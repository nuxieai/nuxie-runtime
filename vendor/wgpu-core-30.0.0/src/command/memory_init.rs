...
impl CommandBufferTextureMemoryActions {
    /// Returns whether a prior store-discard in this command buffer could
    /// require an immediate clear before a later texture use.
    pub(crate) fn has_pending_discards(&self) -> bool {
...
impl BakedCommands {
    ...
    pub(crate) fn initialize_buffer_memory(
        ...
        snatch_guard: &SnatchGuard<'_>,
    ) -> Result<bool, DestroyedResourceError> {
        profiling::scope!("initialize_buffer_memory");
        let mut cleared_memory = false;
