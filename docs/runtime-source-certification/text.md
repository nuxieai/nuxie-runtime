...
The separately owned modifier stream remains red: successive nonzero groups
still layer variation values instead of rebuilding each replacement run from
its original style and swapping the run vector. That correction belongs to the
pinned `text_modifier_group.cpp` pair. No consumer moved; topology remains four
pass, three executable expected-red, and 11 pending.
...
| 36 | `makeStyled` (1053) | Clear; preserve all-run indices while skipping missing style/font/empty text; append scaled runs; optionally apply modifiers; return run nonempty. Defaults: modifiers=true, scale=1. | Runs `text.rs:3076::StaticTextSlice::resolved_runs`, `3149::resolved_dynamic_runs`; buildRenderStyles paint lookup `3238::all_runs_paint_style_index`; first-run seed `3249::first_included_style`; topology `2263::shaped_topology_from_resolved_runs`; evidence `text.rs:6075::cxx_dynamic_text_runs_retain_all_indices_and_make_styled_omissions`, `6203::cxx_styled_text_style_id_wraps_and_drives_all_runs_paint_lookup`, `6302::cxx_shape_measure_and_clip_seed_from_first_included_run_font` | **adapted correction candidate/incomplete**: raw all-runs order and wrapping `u16 styleId` are retained across authored and dynamic entries, including skipped gaps; pinned `buildRenderStyles` paint lookup consumes that ID, while appended TextRun font/metrics come from the actual source style. Every common shaper seed uses the first included run. Modifier application remains incorrect at the separate `text_modifier_group.cpp` boundary: Rust layers successive group variations, whereas pinned replacement runs restart from the original style and `swapRuns` after each nonempty group. |
...
| H3 | `StyledText::swapRuns` (68) | Immutable glyph reconstruction `text.rs:3734::StaticTextSlice::styled_resolved_run_glyphs`; modifier variation owner `3896::styled_text_glyphs_for_style_with_strengths`. | **adapted/incorrect consumer boundary**: immutable reconstruction substitutes for vector swap, but successive nonzero groups currently layer values instead of rebuilding from the original style; the authored `m_runs` lookup in row 18 and the swap correction belong to the modifier source pair. |
...
- literal wrapping `StyledText::TextRun::styleId` and its all-runs lookup are
  retained, but successive modifier groups do not yet reproduce pinned
  original-style replacement and `swapRuns` behavior;
