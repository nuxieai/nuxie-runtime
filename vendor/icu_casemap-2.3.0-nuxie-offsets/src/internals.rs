...
impl<'a, const IS_TITLE_CONTEXT: bool> Writeable for FullCaseWriteable<'a, '_, IS_TITLE_CONTEXT> {
    fn write_to<W: fmt::Write + ?Sized>(&self, sink: &mut W) -> fmt::Result {
        self.write_observed(sink, |_| {}, false)
    }
    fn writeable_length_hint(&self) -> writeable::LengthHint {
...
// Nuxie: observe scalar boundaries in the same contextual mapping pass.
impl<const IS_TITLE_CONTEXT: bool> FullCaseWriteable<'_, '_, IS_TITLE_CONTEXT> {
    fn write_observed<W: fmt::Write + ?Sized>(
        &self,
        sink: &mut W,
        mut observe: impl FnMut(&W),
        granular_tail: bool,
    ) -> fmt::Result {
        let src = self.src;
    ...
    pub(crate) fn with_scalar_offsets(&self) -> (alloc::string::String, alloc::vec::Vec<usize>) {
        #[derive(Default)]
        struct Output {
        ...
        impl fmt::Write for Output {
            fn write_str(&mut self, value: &str) -> fmt::Result {
                ...
                self.scalars += value.chars().count();
                Ok(())
            }
        }
        let mut output = Output::default();
        let mut offsets = alloc::vec![0];
        self.write_observed(&mut output, |output| offsets.push(output.scalars), true)
