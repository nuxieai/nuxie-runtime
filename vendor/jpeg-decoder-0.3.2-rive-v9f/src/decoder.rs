        ...
        loop {
            ...
            match marker {
                ...
                Marker::SOF(..) => {
                    ...
                    let mut frame = parse_sof(&mut self.reader, marker)?;
                    ...
                    frame.apply_rive_v9f_fancy_scaling()?;
...
fn compute_image(
    ...
    if components.len() == 1 {
        ...
        let line_stride = component.block_size.width as usize * component.dct_scale_h;
    ...
    match component_count {
        3 => match color_transform {
            ...
            ColorTransform::YCbCr => {
                #[cfg(feature = "rive_v9f")]
                {
                    Ok(color_convert_line_ycbcr_rive_v9f)
                }
...
// Literal arithmetic owner: rive-app_libjpeg_v9f/jdcolor.c,
// build_ycc_rgb_table() and ycc_rgb_convert().  In particular, the source
// rounds the red and blue contributions before adding Y, but combines the two
// scaled green contributions before its single arithmetic right shift.
#[cfg(feature = "rive_v9f")]
fn color_convert_line_ycbcr_rive_v9f(data: &[Vec<u8>], output: &mut [u8]) {
    assert!(data.len() == 3, "wrong number of components for ycbcr");
    let [y, cb, cr]: &[_; 3] = data.try_into().unwrap();
        ...
        .zip(cr.iter())
    {
        ...
        let cr_to_red = (91_881 * cr + 32_768) >> 16;
        let cb_to_blue = (116_130 * cb + 32_768) >> 16;
        let green = (-22_553 * cb + 32_768 - 46_802 * cr) >> 16;

        chunk[0] = (y + cr_to_red).clamp(0, 255) as u8;
        chunk[1] = (y + green).clamp(0, 255) as u8;
        chunk[2] = (y + cb_to_blue).clamp(0, 255) as u8;
    }
}
