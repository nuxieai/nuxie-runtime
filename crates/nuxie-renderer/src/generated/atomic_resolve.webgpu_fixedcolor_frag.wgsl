...
fn main_1() {
    ...
    if (_e171 <= 1u) {
        phi_1007_ = select(unpack4x8unorm(_e98.y), vec4<f32>(0f, 0f, 0f, 0f), vec4((eh && (_e171 == 0u))));
    } else {
        let _e179 = (_e95 * 8u);
        ...
        let _e201 = phi_993_;
        let _e210 = textureSampleLevel(MD, Ob, vec2<f32>(((clamp(_e201, 0f, 1f) * _e193.z) + _e193.w), bitcast<f32>(_e98.y)), 0f);
        phi_1007_ = _e210;
    }
    let _e212 = phi_1007_;
