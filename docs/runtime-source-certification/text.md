...
no production or test behavior was changed.
...
|---:|---|---|---|---|
| 1 | `TextValueRunProperty::TextValueRunProperty` (23) | Retain core, listener, instance value, property key, and symbol type. | No direct per-property Rust owner. Nearest aggregate: `text/text.rs:15::RuntimeTextValueRunListener::new`. | **adapted/packed**: no per-property dependent retaining the key/type tuple. |
| 2 | `TextValueRunProperty::writeValue` (36) | Text content writes through `CoreRegistry`; style scans paints in order, installs the first as fallback, then the named match; unknown symbol is inert. | `viewmodel/viewmodel_instance_list.rs:132::RuntimeOwnedViewModelListHandle::text_runs`; `text.rs:2935::StaticTextSlice::resolved_runs` | **incorrect/incomplete**: items without `textContent` are dropped by `filter_map`, while C++ still creates an empty run. When content exists but style does not, Rust converts the missing style to an empty name and falls back to the first style; pinned C++ leaves style null and `makeStyled` skips the run. Writes are also polled during reconstruction rather than performed by the listener in source order. |
...
| 17 | `buildRenderStyles` (558) | Eight ordered phases: clear; empty-shape bounds/return; bounds+ellipsis; modifier coverage; bounds/clip; ordered glyph paths/color commands/hit rects; fit/vertical transform plus layout dirt; hit contours. | `text.rs:2238::StaticTextSlice::layout_from_shaped_topology`; `2521::render_data_from_layout`; `4322::static_layout_info`; `4432::static_render_transform`; `draw.rs:17772::runtime_build_text_draw_frame` | **incorrect/incomplete/packed**: paragraph spacing is absent and TextValueRun hit rectangles/contours have no source-equivalent owner. Additionally, pinned empty shape always publishes zero bounds and returns; Rust `text.rs:3839::unshaped_local_bounds` and `477::static_fixed_text_constraint_bounds` can publish the controlled layout box for empty layout-controlled Text. Phase/order is split across immutable reconstruction and retained draw-owner publication. |
...
| 34 | `StyledText::empty` (1033) | Emptiness is run-vector emptiness, not character-vector emptiness. | No direct `StyledText` owner; nearest predicate is `text.rs:2065::StaticTextSlice::render_layout`, which checks `resolved_runs.iter().all(text.empty)`. | **adapted candidate**: because pinned `makeStyled` skips empty source runs before append, the Rust all-empty predicate appears equivalent for the current run stream. |
...
| 36 | `makeStyled` (1053) | Clear; preserve all-run indices while skipping missing style/font/empty text; append scaled runs; optionally apply modifiers; return run nonempty. Defaults: modifiers=true, scale=1. | `text.rs:2935::StaticTextSlice::resolved_runs`; `2140::shaped_topology_from_resolved_runs`; `2238::layout_from_shaped_topology` | **incomplete/packed**: run/style resolution and modifiers exist, but empty/missing runs do not preserve the same style ID space and the StyledText lifecycle is reconstructed. |
| 37 | static `BreakLines` (1085) | Break each paragraph; no-wrap/auto width use -1; auto-width computes global max; then compute spacing/alignment for all paragraphs. | `text.rs:4180::StaticTextSlice::layout_static_text_lines`; `text/line_breaker.rs:1::split_static_text_lines`; `50::static_text_line_iteration` | **adapted/incomplete**: custom HarfRust/renderer annotation breaker replaces `GlyphLine::BreakLines`; paragraph spacing is outside this function but downstream line vertical positions still omit it. |
...
| 39 | `onDirty` (1135; disabled 1420) | WorldTransform notifies all modifiers; Path/Paint invalidates stroke effects on current render styles. | `draw.rs:10197::RuntimeShapeList::on_component_dirty`; Text dirt methods at `draw.rs:10445-10509::RuntimeTextDrawOwner`; no direct per-modifier world-dirty callback. | **incomplete**: no source-ordered per-modifier notification or per-current-render-style stroke-effect invalidation owner. |
...
| 53 | `buildTextStylePaints` (1550) | Lazily cache direct TextStylePaint children once, in child order. | `text.rs:1670::StaticTextSlice::from_graph`; `draw.rs:10427::RuntimeTextDrawOwner::topology_or_build` | **incorrect/incomplete**: topology is lazily cached per occurrence, but construction is bundled with runs/modifiers and errors when there is no style. Pinned legitimately retains an empty style list and later skips null-style runs. |
...
| H20 | `Text::overflowAsFixed` (239) | `text.rs:3799::StaticTextSlice::overflow_as_fixed` | **adapted; inherits row 11 mixed-axis defect** |
...
| H29 | testing `unichars` (279) | No literal observer; nearest projection is `text.rs:163::static_text_layout_debug_report` from retained topology text. | **missing literal public observer** |
...
The exact 18-case topology is **three pass, four executable expected-red, and
11 pending**. Cases 2 and 3 are direct/pass in Wave C7. Case 14 already has an
exact literal Silver entry and passes. Cases 15-18 have executable literal
Silver entries with frozen first differences; they are not missing-owner
pending work. This source audit does not add wrappers or change evidence.
...
| 8 `fitFontSize shrinks...` | **pending**: missing literal shaped run size and `m_transform.xx` observers; row 15 also has a known paragraph-spacing omission. |
...
| 16 `Fit font size` Silver | **executable expected-red**: frozen first difference `frame 2, op 199 (makeRenderPath): expected makeRenderPath, got rewind` (`tools/silver-corpus/generate_manifest.py:1948`); row 15 omission also remains red. |
...
- dynamic list items with content but no style incorrectly fall back to the
  first style instead of remaining null-styled and being skipped;
...
source rows are complete. The four executable reds are honest divergences, not
coverage gaps. A fresh independent reviewer must try to falsify every mapping
before any production correction begins.
