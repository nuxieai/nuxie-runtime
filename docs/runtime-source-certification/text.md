...
style lookup indexes the complete all-runs sequence with that ID. TextRun font,
...
`text.rs::cxx_styled_text_style_id_wraps_and_drives_shaper_style_lookup`,
...
| 36 | `makeStyled` (1053) | Clear; preserve all-run indices while skipping missing style/font/empty text; append scaled runs; optionally apply modifiers; return run nonempty. Defaults: modifiers=true, scale=1. | Runs `text.rs:3076::StaticTextSlice::resolved_runs`, `3149::resolved_dynamic_runs`; literal ID lookup `3238::style_index_from_shaper_id`; first-run seed `3249::first_included_style`; topology `2263::shaped_topology_from_resolved_runs`; evidence `text.rs:6075::cxx_dynamic_text_runs_retain_all_indices_and_make_styled_omissions`, `6203::cxx_styled_text_style_id_wraps_and_drives_shaper_style_lookup`, `6302::cxx_shape_measure_and_clip_seed_from_first_included_run_font` | **adapted correction candidate/incomplete**: raw all-runs order and wrapping `u16 styleId` are retained across authored and dynamic entries, including skipped gaps; paint/style lookup consumes that ID, while appended TextRun font/metrics come from the actual source style. Every common shaper seed uses the first included run. Modifier application remains incorrect at the separate `text_modifier_group.cpp` boundary: Rust layers successive group variations, whereas pinned replacement runs restart from the original style and `swapRuns` after each nonempty group. |
...
| H3 | `StyledText::swapRuns` (68) | Immutable modifier reconstruction `text.rs:3666::StaticTextSlice::styled_resolved_run_glyphs`; group variation owner `3823::styled_text_glyphs_for_style_with_strengths`. | **adapted/incorrect consumer boundary**: immutable replacement substitutes for vector swap, but successive nonzero groups currently layer values instead of rebuilding from the original style; correction belongs to `text_modifier_group.cpp`. |
