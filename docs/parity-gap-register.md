...
| V41 | **CLOSED 2026-08-25 — `paused_nested_artboard_opacity` is exact in both runner lanes.** Two hand-port discrepancies compounded: late ShapePaint attachment read an Artboard's authored render opacity without its separate NestedArtboard host opacity, and layout-driven `TextShape` rebuilds retained TextStylePaint opacity paths that pinned `Text::clearRenderStyles()` clears. Late paint guards now use authored-times-host opacity for Artboard owners, while layout text invalidation replaces derived render paths exactly as C++ does. | `corpus.toml`, samples 0/0.5/1; pinned `artboard.hpp:641-650`, `transform_component.cpp:105-119`, `layout_component.cpp:1116-1124`, `text.cpp:208-219,558-566,1154-1234`, and `text_style_paint.cpp:13-19`; ordinary and scripted C++/Rust side-channel comparisons | **Green:** all three ordinary and scripted samples are byte-identical (68,190-byte stream); row restored to `exact`. |
| V42 | **CLOSED 2026-08-25 — `stateful_component_image_test` is exact in both runner lanes.** Rust had replaced the pinned `ImportStack`'s FileAsset/Artboard displacement and reverse-unwind order with corpus-feature image buckets, causing this file's image to decode before its source paint owner. Source resources now replay the two pinned stack owners directly. | `corpus.toml`, samples 0/0.5/1; pinned `file.cpp:300-662`, `import_stack.hpp:33-85`, and `file_asset_importer.cpp:28-52`; ordinary and scripted C++/Rust side-channel comparisons | **Green:** all three ordinary and scripted samples are exact; all 31 existing exact ImageAsset fixtures remain exact in both lanes (87 samples per lane). |
| V43 | **CLOSED 2026-08-25 — `data_bind_blob_test` is exact in both runner lanes.** Four hand-port discrepancies compounded: Blob used a passive dirt sink instead of the synchronous `ScriptedProperty::valueChanged` listener path; a retained row border box was returned to Taffy as content size and accumulated root insets; hosted fill-width changes reused stale hug-axis sizes; and component-list `takeLayoutData()` never disabled detached root solves, allowing parent-owned `x=16` to be overwritten by `x=0`. Blob listeners now publish at the host mutation boundary, leaf measurement returns content size, authored hug axes are remeasured under their assigned cross-axis constraint, and transferred row roots remain host-owned. | `corpus.toml`, samples 0/0.1/0.5/1/2; pinned `lua_properties.cpp:249-283,1471-1495`, `artboard_component_list.cpp:220-260`, and `artboard.cpp:1245-1252`; ordinary and scripted C++/Rust side-channel comparisons | **Green:** all five ordinary and scripted segments are byte-identical; scripted stream is 10,945,431 bytes. |
| V44 | **CLOSED 2026-08-25 — `artboard_opacity_and_transform_test` is exact in both runner lanes.** The runtime already routed inherited Artboard rotation and opacity through the generic authored DataBind owner; the Rust golden runner's nested-child structural guard omitted those inherited properties and rejected the fixture before execution. | `corpus.toml`, samples 0/0.5/1; ordinary and scripted C++/Rust side-channel comparisons; pinned generated `transform_component_base.hpp` and `world_transform_component_base.hpp` property keys | **Green:** all three ordinary and scripted samples are byte-identical (7,437-byte stream); row restored to `exact`. |
...
| F6 | **Semantics/accessibility** — `semantic_manager` 1,109, `semantic_data` 572, provider, inference registry | 1,926 | CLOSED (FTAIL) | The retained runtime and LT-1 full diff/action/focus side channel are implemented against `4ac7b327`. Nested focus, Simpsons, data_binding_lists, and solo_index_test are exact. Container semantic bounds now preserve C++'s live-geometry observation boundary by pruning collapsed subtrees before Rust's on-demand geometry query can rebuild deferred Solo paths (`semantic_provider.cpp:76-148`; `shape.cpp:374-405`). The mounted Text shaping and component-settlement journal evidence remains exact. |
| F7 | **Unported Lua bindings** — `lua_promise` 1,323, `lua_scripted_context` 583, `lua_buffer_ext` 538, `lua_audio` 507, `lua_data_value` 503, `lua_image_decode` 467, + mesh/color/image/blob/state/data_context/gradient/input | ~6,000 | PARTIAL (by design) | The paired `lua_gpu.cpp` owner audit closes its mixed Canvas 2D and `Image:view` residue through the exact renderer seam. `lua_scripted_context.cpp` has the full Context method-name surface, including the now-backed sized/deferred `canvas` and `gpuCanvas` descriptors; component-derived owner-specific `markNeedsUpdate` remains named residue. Other named Lua families retain their correspondence status and corpus gates. |
| F8 | **ORE scripted GPU host** (GPUBuffer/GPUCanvas contexts) | — | DEFERRED (`deferred-2026-07-19-ore-gpu`) | GPU-prefixed userdata reaches the product renderer under the approved D18 adapter ceiling. The mixed Canvas 2D path now uses the exact mechanically ported RenderCanvas lifecycle, and `Image:view` retains an exact renderer image in the adapter's sampled-texture occurrence. A separate native-ORE product API is still not claimed. |
