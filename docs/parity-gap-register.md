...
| V11 | **Comparator timing fixed; mounted-layout draw remains divergent.** Name-based `ConditionComparisonSelf` sources now resolve through the retained file resolver and Rust reports C++'s two t=0 layer changes. Fresh dense capture still differs after settlement: the mounted child is sized correctly but its x placement is 8px early because Rust republishes the consumed global value into the parent layout. | fresh scripted golden capture; `global_variables_mid_frame_view_model_comparator_matches_cpp_layer_count`; `transition_viewmodel_condition.cpp:49-60`; `state_machine_instance.cpp:2665-2697` | Keep `draw-stream-diverges:V11`; separate same-layer source consumption from parent-layout propagation before promoting to `exact`. |
