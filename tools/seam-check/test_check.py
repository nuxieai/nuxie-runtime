...
TOOL_SPEC = importlib.util.spec_from_file_location("seam_check_tool", TOOL)
assert TOOL_SPEC is not None and TOOL_SPEC.loader is not None
SEAM_CHECK_TOOL = importlib.util.module_from_spec(TOOL_SPEC)
TOOL_SPEC.loader.exec_module(SEAM_CHECK_TOOL)
...
class SeamCheckCliTest(unittest.TestCase):
    ...
    def test_rejects_every_current_product_trust_constructor_name(self) -> None:
        ...
        for method in methods:
            with self.subTest(method=method):
                errors = SEAM_CHECK_TOOL.mixed_facade_source_errors(
    ...
    def test_all_declared_custom_cargo_target_paths_are_discovered(self) -> None:
        ...
        sources = {
            ...
            for path in SEAM_CHECK_TOOL.package_rust_sources(self.package, manifest)
        }
    ...
    def test_rejects_glob_imported_product_host_limit_in_new_file(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("product-host-commands seam debt spread", result.stderr)
    ...
    def test_rejects_stale_internal_debt_exception(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("stale project-data seam debt exception", result.stderr)
    ...
    def test_deleted_internal_debt_exception_fails_in_repository(self) -> None:
        ...
        errors = SEAM_CHECK_TOOL.missing_debt_exception_errors(
            self.root,
            {"example": set()},
            {"example": {missing}},
        )
        ...
        self.assertEqual(
            errors,
            [f"{missing}: missing example seam debt exception file; remove the allowlist entry"],
        )
    ...
    def test_rejects_browser_presentation_debt_in_new_file(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("browser-presentation seam debt spread", result.stderr)
    ...
    def test_rejects_apple_measurement_debt_in_new_file(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("apple-presentation seam debt spread", result.stderr)
    ...
    def test_rejects_binary_authoring_debt_in_new_file(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("binary-authoring seam debt spread", result.stderr)
    ...
    def test_character_literals_do_not_confuse_string_stripping(self) -> None:
        ...
        stripped = SEAM_CHECK_TOOL.strip_rust_non_code(source)

        self.assertIn("let quote", stripped)
        self.assertNotIn("host_commands", stripped)


class SeamCheckWiringTest(unittest.TestCase):
    def test_makefile_exposes_independent_test_check_and_gate_targets(self) -> None:
        makefile = (REPO_ROOT / "Makefile").read_text()

        self.assertIn("seam-contract-test:", makefile)
        self.assertIn("seam-contract-check:", makefile)
        self.assertIn("seam-contract-gate:", makefile)
        self.assertIn('tools/report-all.sh "seam-contract"', makefile)

    def test_ci_and_landing_run_the_seam_contract_gate(self) -> None:
        workflow = (REPO_ROOT / ".github/workflows/ci.yml").read_text()
        landing = (REPO_ROOT / "tools/land.sh").read_text()

        self.assertIn("run: make seam-contract-gate", workflow)
        self.assertIn("seam-contract-gate", landing)
