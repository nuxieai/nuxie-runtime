# Data Binding Graph Formula Imported Asset Mutation Runtime Contract

## Purpose

Close the narrow imported-context source-mutation path for asset sources feeding
`DataConverterFormula` number targets.

Earlier imported asset context slices proved that a file-backed view-model
instance can bind an asset source into the graph before formula conversion.
This slice proves that the public imported-context mutation API refreshes that
same formula source after binding, including fanout to a same-path direct asset
observer.
...
- Imported artboard and view-model pointer source mutations.
...
- Binding the imported context initializes the formula asset source and
  same-path asset observer source.
- Mutating the imported asset source by formula data-bind index updates the
  context override and every same-path asset source node in the graph.
