...
impl RuntimeOwnedViewModelParentRelay {
    ...
    fn add_dependent(&self, sink: &RuntimeCellDirtSink) {
...
impl RuntimeOwnedViewModelViewModel {
    ...
    fn active_scalar_cell_by_property_index(
        ...
        family_cell!(numbers, imported_numbers)
            ...
            .or_else(|| {
                self.active_list_by_property_index(property_index)
                    .map(|list| list.cell.clone())
            })
            .or_else(|| {
                self.active_children()?
                    .iter()
                    .find(|child| child.property_index == property_index)
                    .map(|child| child.endpoint.cell())
            })
    }
        ...
        family_cell!(numbers)
            ...
            .or_else(|| {
                self.view_models
                    .iter()
                    .find(|view_model| view_model.property_index == property_index)
                    .map(|view_model| view_model.endpoint.cell())
            })
    }
