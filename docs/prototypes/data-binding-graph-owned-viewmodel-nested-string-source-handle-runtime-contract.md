# Owned ViewModel Nested String Source Handle Runtime Contract

Purpose: extend owned runtime source handles to generated nested string paths
without admitting the broader owned nested handle family.

Owned generated nested string mutation already resolves a path such as
`child/label` before binding an owned runtime view-model context. This slice
exposes the same generated-child path through
`RuntimeOwnedViewModelStringSourceHandle`, while preserving root handle
...
- Resolving a generated child path such as `child/label` into
  `RuntimeOwnedViewModelStringSourceHandle`.
- Mutating owned generated-child raw string storage through that handle before
  binding the owned context to a state machine.
- C++ probe comparison against
  `ViewModelInstanceRuntime::propertyString("child/label")->value(...)`.
...
Completion condition: resolving and mutating an owned generated nested string
source by handle produces the same state-machine advance and component update
reports as the C++ owned string name-path binding path, no-op repeat writes
