...
impl ImmediateWorker {
    pub fn start_immediate(&mut self, data: RowData) {
        let elements = data.component.block_size.width as usize
            ...
            * data.component.dct_scale_h
            * data.component.dct_scale_v;
    ...
    pub fn component_metadata(&self, index: usize) -> Option<ComponentMetadata> {
        ...
        let line_stride = block_size.width as usize * component.dct_scale_h;
        let dct_scale_h = component.dct_scale_h;
        let dct_scale_v = component.dct_scale_v;
    ...
    pub fn append_row_locked(
        ...
        let mut output_buffer = [0; 16 * 16];
        for i in 0..block_count {
            let x = (i % block_width) * dct_scale_h;
            let y = (i / block_width) * dct_scale_v;
            ...
            let buffered_lines = output_buffer.chunks_mut(dct_scale_h);
            let back_lines = write_back.chunks_mut(line_stride);

            for (buf, back) in buffered_lines.zip(back_lines).take(dct_scale_v) {
                back[..dct_scale_h].copy_from_slice(&buf[..dct_scale_h]);
            }
        }
    }
}
...
impl ComponentMetadata {
    fn bytes_used(&self) -> usize {
        self.block_count * self.dct_scale_h * self.dct_scale_v
    }
}
