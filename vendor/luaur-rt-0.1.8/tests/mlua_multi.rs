...
fn test_result_conversions() -> Result<()> {
    ...
    let err1 = Err::<(), _>("failure1".into_lua_err());
    let multi_err1 = err1.into_lua_multi(&lua)?;
    assert_eq!(multi_err1.len(), 2);
    assert_eq!(multi_err1[0], Value::Nil);
    // DEVIATION: mlua keeps the error message as a `Value::String`. luaur-rt
    // carries a returned Rust error as `Value::Error`; its string form matches.
    assert_eq!(multi_err1[1].to_string()?, "failure1");
...
fn test_multivalue_by_ref() -> Result<()> {
    ...
    // DEVIATION: mlua accepts `&MultiValue` as args; luaur-rt's `IntoLuaMulti`
    // is by-value, so we clone the prepared multi (same observable call).
    f.call::<()>(multi.clone())?;
...
#[test]
fn test_multivalue_index_and_clear() {
    // Adapted from mlua's `value::test_multi_value`.
