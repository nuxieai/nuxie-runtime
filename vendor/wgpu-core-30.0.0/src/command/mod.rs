...
impl InnerCommandEncoder {
    /// Whether the active HAL encoder explicitly permits pre-pass-free render
    /// passes to continue in the same native command buffer.
    fn can_coalesce_render_pass_command_buffers(&self) -> bool {
        self.raw.can_coalesce_render_pass_command_buffers()
    }

    pub(crate) fn can_discard_transition_only_command_buffer(&self) -> bool {
        self.raw.can_discard_transition_only_command_buffer()
    }

    /// Discard the currently open native command buffer without adding it to
    /// this encoder's submission list.
    pub(crate) fn discard_open(&mut self) {
        assert!(self.is_open);
        self.is_open = false;
