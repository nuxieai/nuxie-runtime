...
| V29 | **PARTIAL 2026-08-03 — retained-paint loss is closed; a later text-shaping gap remains.** Rust now completes all samples and paint global 71 remains addressable through its concrete Text occurrence. The next mismatch is glyph path 6 at stream line 105 after t=0.5. | `corpus.toml` milestone V29, samples 0/0.5/1; exact-mode side-channel repro | Reconcile the post-keyframe text layout/shape and restore `exact`. |
...
| V31 | **PARTIAL 2026-08-03 — retained-gradient rematerialization is closed; a later path gap remains.** Gradient 51 is no longer recreated at t=0.5. The next mismatch is draw path 140 at stream line 1920 under the 0.0005 contract. | `corpus.toml` milestone V31, samples 0/0.5/1; tolerant side-channel repro | Reconcile the later animated path verbs/geometry and restore `exact`. |
