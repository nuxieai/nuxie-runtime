...
impl CompactLength {
    ...
    /// The tag indicating a Rive Yoga percentage value.
    ///
    /// Unlike Taffy's native percentages, this retains the authored [0, 100]
    /// value so resolution can preserve Yoga's exact `f32` operation order.
    pub const RIVE_YOGA_PERCENT_TAG: usize = 0b0000_0110;
...
impl CompactLength {
    ...
    /// A Rive Yoga percentage in the range [0.0, 100.0].
    #[inline(always)]
    pub const fn rive_yoga_percent(val: f32) -> Self {
        Self(CompactLengthInner::from_val(
            val,
            Self::RIVE_YOGA_PERCENT_TAG,
        ))
    }
