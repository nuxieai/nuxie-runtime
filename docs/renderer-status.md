...
- Poisoned do-nothing negative control: 1,468 tolerance divergences, zero
  exact or byte-exact passes, and zero gated entries. The poison clear is
  scoped to `--expect-all-fail`; ordinary stub replay remains transparent.
...
The accepted vendored wgpu 30 core/HAL path
coalesces only render passes proved to have no ordered pre-pass work and drops
...
changes are upstream or the full family is published under coordinated names.

The renderer side now also uses a flat move-only MSAA schedule, sparse solid
gradient preparation, grouped clockwise-atomic uploads, C++-style retained
coverage generations, direct midpoint endpoint reads, on-demand direct stroke
batching, a dense multi-contour construction fast path, and per-slot retained
MSAA packing scratch capped at 1 MiB (roughly 3 MiB across the three slots).
Context-owned stroke-preparation scratch now mirrors C++'s resettable
...
The exact final evidence and immutable hashes are recorded in
`docs/renderer-parity-workflow.md`.
...
widening. Universal byte identity is not part of that stop condition.
...
- 2026-07-17: Renderer release exactness remains the reviewed per-row decoded
  ...
  as a non-gating health metric. A same-tier C++ Dawn campaign replaces all 59
  retained comparisons: 55 pass unchanged and four remain concrete Rust/Dawn
...
- 2026-07-24: The sixth user-directed hot-loop slice ports the concrete
  ...
  world-transform state on the `Drawable` object itself; Rust now retains the
  corresponding component index at construction and reads it directly instead
  of rediscovering the component through `component_by_local` on every draw.
  Shape draw also reads the retained component world transform on the ordinary
  non-layout path; the documented layout compatibility boundary remains
  (`artboard.cpp:1652-1698`, `shape.cpp:137-159`,
  ...
  `animation_reset_cases@0` improved from 1.771532x total / 1.840812x advance /
  1.964539x draw to 1.658623x / 1.846382x / 1.931431x, and aggregate moved
  from 1.556598x to 1.518033x
  (`target/perf-hot-loop-drawable-component-owner-current-env.json`).
  ...
  `animation_reset_cases@0.5` at 1.670139x;
  `advance_blend_mode@0.25` remains the worst draw entry at 3.139422x. The
  <=1.0x tier-5 target remains open.
...
- 2026-07-23: The next user-initiated post-RD-C6 checkpoint rebuilt and
  ...
  interval after about four minutes. The sole canonical A-B-B-A run then
  failed closed in `validate-host-load` at 80.60% idle spread against the
  unchanged 12% fence
  (`target/r4-timing-gate/20260723T154251Z-36478`). No comparison,
  `perf-hot-loop`, or retry followed. This is invalid evidence, and RD-C7
  remains blocked pending a valid checkpoint number and user review.
