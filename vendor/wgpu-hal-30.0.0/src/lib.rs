...
pub trait CommandEncoder: WasmNotSendSync + fmt::Debug {
    ...
    /// Returns whether render pass bodies with no real pre-pass work may be
    /// recorded into the command buffer that is already open.
    ///
    /// This is deliberately opt-in. Backends must only return `true` when
    /// their resource transition methods need no native commands and when no
    /// backend-specific command-buffer prologue would be bypassed by
    /// continuation. The answer may change between calls as backend state
    /// changes, so callers must query it for every candidate pass.
    fn can_coalesce_render_pass_command_buffers(&self) -> bool {
        false
    }

    /// Returns whether a transition-only command buffer may be discarded when
    /// core proves that buffer and texture initialization emitted no clears.
    ///
    /// This is deliberately opt-in. Backends must only return `true` when
    /// resource transitions encode no native work and dropping the command
    /// buffer cannot bypass a synchronization or completion prologue.
...
#[test]
fn noop_command_encoder_uses_default_false_coalescing_capability() {
    let encoder = noop::CommandBuffer::new();
    assert!(!CommandEncoder::can_coalesce_render_pass_command_buffers(
        &encoder
    ));
    assert!(!CommandEncoder::can_discard_transition_only_command_buffer(
        &encoder
    ));

    let dyn_encoder: &dyn DynCommandEncoder = &encoder;
    assert!(!dyn_encoder.can_coalesce_render_pass_command_buffers());
    assert!(!dyn_encoder.can_discard_transition_only_command_buffer());
}
