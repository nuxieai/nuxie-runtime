...
const fn render_pass_command_buffer_coalescing_allowed(strict_event_sync_enabled: bool) -> bool {
    !strict_event_sync_enabled
}

const fn transition_only_command_buffer_discard_allowed(strict_event_sync_enabled: bool) -> bool {
    !strict_event_sync_enabled
}
...
mod render_pass_command_buffer_coalescing_tests {
    use super::{
        render_pass_command_buffer_coalescing_allowed,
        transition_only_command_buffer_discard_allowed,
    };

    #[test]
    fn strict_event_sync_rejects_command_buffer_continuation() {
        assert!(render_pass_command_buffer_coalescing_allowed(false));
        assert!(!render_pass_command_buffer_coalescing_allowed(true));
    }

    #[test]
    fn strict_event_sync_preserves_transition_only_command_buffers() {
        assert!(transition_only_command_buffer_discard_allowed(false));
        assert!(!transition_only_command_buffer_discard_allowed(true));
    }

    #[test]
    fn irreversible_relay_epoch_disables_all_later_continuations() {
        // Queue::enable_strict_event_sync stores the relay in a OnceLock, so
        // the only legal epoch transition is absent -> present.
        let relay_epochs = [false, false, true, true];
        let actual = relay_epochs.map(render_pass_command_buffer_coalescing_allowed);
