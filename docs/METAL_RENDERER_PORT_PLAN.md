...
`docs/render-context-metal-fields.tsv` separately accounts for every
state-bearing declaration in `RenderTargetMetal`, `ContextOptions`,
`DrawPipeline`, and `RenderContextMetalImpl`. The campaign checker extracts the
