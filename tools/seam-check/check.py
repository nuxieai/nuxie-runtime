...
See docs/seam-contract.md for the ratified contract and the exact migration
...
def missing_debt_exception_errors(
    ...
    if not (repo_root / ".git").exists():
        return []
    return [
        f"{relative}: missing {family} seam debt exception file; remove the allowlist entry"
    ...
    for family, exceptions in INTERNAL_DEBT_FILES.items():
        for relative in sorted(exceptions):
            if (repo_root / relative).is_file() and relative not in observed_debt[family]:
                errors.append(
                    f"{relative}: stale {family} seam debt exception; remove the "
