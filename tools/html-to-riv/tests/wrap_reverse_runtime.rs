...
#[test]
fn compiled_wrap_reverse_matches_browser_after_resize() {
    let oracle: serde_json::Value =
        serde_json::from_str(include_str!("assets/wrap-reverse-boxes.json")).unwrap();
    ...
    for case in oracle["cases"].as_array().unwrap() {
        ...
        let object = artboard
            ...
            .unwrap();
        use nuxie_runtime::source::generated::{core_registry::CoreRegistry, layout::layout_component_style_base::LayoutComponentStyleBase};
        let style_id = object.with(|o| o.as_layout_component().unwrap().base.style_id()).unwrap();
        let style = artboard.with_artboard(|a| a.objects()[style_id as usize].clone()).unwrap();
        assert_eq!(style.with_mut(|o| CoreRegistry::get_uint(o, i32::from(LayoutComponentStyleBase::FLEX_WRAP_VALUE_PROPERTY_KEY))).unwrap(), 2);
        ...
        requirements.ensure_layout_targets(|target| output.source_map.iter().any(|n| n.object_id == target)).unwrap();
        ...
        for entry in &requirements.layout_align_self {
            ...
            let policy=match entry.alignment {
                AlignSelf::Auto=>CssAlignSelf::Auto, AlignSelf::FlexStart=>CssAlignSelf::FlexStart,
                AlignSelf::Center=>CssAlignSelf::Center, AlignSelf::FlexEnd=>CssAlignSelf::FlexEnd,
                AlignSelf::Stretch=>CssAlignSelf::Stretch, AlignSelf::Baseline=>CssAlignSelf::Baseline,
            };
            assert!(LayoutComponent::set_css_align_self_occurrence(&target,Some(policy)));
        }
            ...
            for node in &output.source_map {
                ...
                for (index, key) in ["x", "y", "width", "height"].iter().enumerate() {
                    ...
                    if (actual[index] - expected).abs() >= 0.1 {
                        mismatches.push(format!("{} {} {} at {width}, {} {key}: {} vs {expected}",
                            case["direction"],case["alignment"],case["mode"],node.id,actual[index]));
                    }
                }
            }
        }
    ...
    assert!(mismatches.is_empty(), "{} mismatched coordinates:\n{}", mismatches.len(), mismatches.join("\n"));
}
