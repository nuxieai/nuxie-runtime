...
fn should_discard_transition_command_buffer(
    hal_allows_discard: bool,
    buffer_clear_work: bool,
    texture_clear_work: bool,
) -> bool {
    hal_allows_discard && !buffer_clear_work && !texture_clear_work
}

#[cfg(test)]
mod transition_command_buffer_tests {
    use super::should_discard_transition_command_buffer;

    #[test]
    fn discards_only_when_hal_allows_it_and_no_clear_work_was_encoded() {
        for bits in 0_u8..8 {
            let hal_allows_discard = bits & 1 != 0;
            let buffer_clear_work = bits & 2 != 0;
            let texture_clear_work = bits & 4 != 0;
            assert_eq!(
                should_discard_transition_command_buffer(
                    hal_allows_discard,
                    buffer_clear_work,
                    texture_clear_work,
                ),
...

            {
                if !command_buffers.is_empty() {
                    ...
                    for command_buffer in command_buffers {
                        ...
                        let mut trackers = self.device.trackers.lock();
                        let buffer_clear_work = match baked
                            .initialize_buffer_memory(&mut trackers, &submission.snatch_guard)
                        {
                            Ok(work) => work,
                            Err(e) => break 'error Err(e.into()),
                        };
                        let texture_clear_work = match baked.initialize_texture_memory(
                            &mut trackers,
                            &self.device,
                            &submission.snatch_guard,
                        ) {
                        ...
                        let discard_transition_buffer = should_discard_transition_command_buffer(
                            baked.encoder.can_discard_transition_only_command_buffer(),
                            buffer_clear_work,
                            texture_clear_work,
                        );
                        if discard_transition_buffer {
                            baked.encoder.discard_open();
                        } else if let Err(e) = baked.encoder.close_and_push_front() {
                            break 'error Err(e.into());
                        }
