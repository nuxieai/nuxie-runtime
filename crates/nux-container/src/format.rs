...
pub fn write_package(model: &NuxPackageModel<'_>) -> Result<Vec<u8>> {
    ...
    manifest.journey.sha256 = sha256_hex(model.journey);
    manifest.journey.size_bytes = usize_to_u64(model.journey.len())?;
    ...
    manifest.members = build_inventory(&manifest, model.scene, model.journey, &embedded)?;
    ...
    members.push((JOURNEY_MEMBER, model.journey));
