...
use femtovg::{
    ...
    ImageFilter as FemtovgImageFilter, ImageFlags, ImageId, ImageSource, LineCap, LineJoin, Paint,
...
impl WebGl2Factory {
    ...
    fn make_rgba_image(
        ...
        Ok(Box::new(WebGl2Image {
            ...
            height,
            pixels,
            ids: RefCell::new(BTreeMap::from([(ImageSampler::LINEAR_CLAMP.as_key(), id)])),
            mesh_textures: RefCell::new(BTreeMap::new()),
...
impl WebGl2Frame {
    ...
    fn close_clip_layers_to(&mut self, depth: usize) {
        while self.clip_layers.len() > depth {
            ...
            self.pending_image_deletes
                .borrow_mut()
                .extend([layer.mask, layer.content]);
        }
        self.state.clip_depth = self.clip_layers.len();
    }
...
impl Renderer for WebGl2Frame {
    ...
    fn draw_image(
        ...
        if webgl2_image_flags(sampler).is_none() {
...
struct WebGl2Image {
    ...
    height: u32,
    pixels: Vec<RGBA8>,
    ids: RefCell<BTreeMap<u8, ImageId>>,
    mesh_textures: RefCell<BTreeMap<u8, web_sys::WebGlTexture>>,
...
impl Drop for WebGl2Image {
    fn drop(&mut self) {
        if let Some(pending_deletes) = self.pending_deletes.upgrade() {
            pending_deletes
                .borrow_mut()
                .extend(self.ids.get_mut().values().copied());
        }
        if let Some(pending_deletes) = self.pending_mesh_texture_deletes.upgrade() {
            pending_deletes
                .borrow_mut()
                .extend(std::mem::take(self.mesh_textures.get_mut()).into_values());
        }
    }
}

fn webgl2_image_flags(sampler: ImageSampler) -> Option<ImageFlags> {
    if sampler.wrap_x == ImageWrap::Mirror || sampler.wrap_y == ImageWrap::Mirror {
        return None;
    }
    let mut flags = ImageFlags::empty();
    if sampler.filter == ImageFilter::Nearest {
        flags |= ImageFlags::NEAREST;
    }
    if sampler.wrap_x == ImageWrap::Repeat {
        flags |= ImageFlags::REPEAT_X;
    }
    if sampler.wrap_y == ImageWrap::Repeat {
        flags |= ImageFlags::REPEAT_Y;
    }
    Some(flags)
}
...
void main() {
    ...
    color = vec4(sampled.rgb * alpha, alpha);
}
"#,
            "image mesh fragment",
        ) {
    ...
    fn texture_for(
        ...
        if let Some(texture) = image.mesh_textures.borrow().get(&sampler.as_key()) {
            return Ok(texture.clone());
        }
        let texture = self.gl.create_texture().ok_or(())?;
        ...
        image
            .mesh_textures
            .borrow_mut()
            .insert(sampler.as_key(), texture.clone());
        Ok(texture)
    }
