...
StyledText production-correction candidate note: rows 33-36 and header rows
H1-H3 now name the concrete immutable Rust topology rather than treating a
fresh vector as if it were no owner. `resolved_runs` retains raw all-runs order
and style identity separately from whether `makeStyled` appended a run;
null-style, font-null, and empty source runs are omitted without advancing
StyledText offsets. Appended valid UTF-8 decodes only through the first NUL,
retains the resulting per-append relative character count, and both shaping
routes consume only that prefix. Rebuilding produces a fresh Unicode/run
topology before publication, so no prior Unicode or run remains. Focused
real-owner evidence is
...
Direct `style_local` identity replaces C++'s indirect
`styleId = allRuns index` lookup. The separately owned modifier stream remains
incorrect: successive nonzero groups currently layer variation values instead
of rebuilding each replacement run from its original style and swapping the
run vector. That discrepancy is deferred to the pinned
`text_modifier_group.cpp` pair; it is not repaired or hidden here. No consumer
...
| 33 | `StyledText::clear` (1027) | Clear Unicode values then runs. | Immutable rebuild `text.rs:3041::StaticTextSlice::resolved_runs` and `2228::StaticTextSlice::shaped_topology_from_resolved_runs`; retained publication `draw.rs::RuntimeTextDrawOwner::shaped_topology_or_build`; evidence `text.rs:6050::cxx_styled_text_append_decodes_to_nul_and_rebuild_clears_prior_topology` | **adapted correction candidate**: Rust constructs and publishes a fresh Unicode/run topology, observably removing all prior Unicode and runs before the replacement is visible. The two internal `clear()` calls have no separately observable Rust mutation order. |
| 34 | `StyledText::empty` (1033) | Emptiness is run-vector emptiness, not character-vector emptiness. | Inclusion state `text.rs:981::StaticResolvedRun`; predicate `3964::StaticTextSlice::has_styled_text_from_resolved_runs`; insertion-point consumer `text.rs::StaticTextSlice::static_line_metrics` | **adapted correction candidate**: emptiness follows whether `makeStyled` appended any font-backed, nonempty-source run, including a zero-character run produced by a leading NUL; it is not inferred from concatenated Unicode length. |
| 35 | `StyledText::append` (1035) | Decode UTF-8 until NUL into Unicode values, count code points, append one `TextRun` with supplied style metadata. | `text.rs:1012::cxx_styled_text_prefix`; `3041::StaticTextSlice::resolved_runs`; shaping consumers `3666::styled_resolved_run_glyphs`, `3748::styled_resolved_run_glyphs_bidi`; evidence `text.rs:6050::cxx_styled_text_append_decodes_to_nul_and_rebuild_clears_prior_topology` | **adapted correction candidate**: valid UTF-8 is decoded to the first NUL and each appended run retains its relative character count; direct `style_local` carries supplied style identity while cumulative `char_start` is separate. Invalid UTF-8 is safely rejected instead of reproducing unchecked C++ decoding. |
| 36 | `makeStyled` (1053) | Clear; preserve all-run indices while skipping missing style/font/empty text; append scaled runs; optionally apply modifiers; return run nonempty. Defaults: modifiers=true, scale=1. | `text.rs:3041::StaticTextSlice::resolved_runs`; `3109::resolved_dynamic_runs`; `2228::shaped_topology_from_resolved_runs`; evidence `text.rs:5929::cxx_dynamic_text_runs_retain_all_indices_and_make_styled_omissions`, `6050::cxx_styled_text_append_decodes_to_nul_and_rebuild_clears_prior_topology`, `text.rs::cxx_empty_line_metrics_ignore_runs_omitted_by_make_styled` | **adapted correction candidate/incomplete**: raw all-runs order is retained; null-style, font-null, and empty source runs are omitted without consuming offsets; direct style identity replaces indirect `styleId = allRuns index`; scaled style metadata is consumed by shaping/layout owners. Modifier application remains incorrect at the separate `text_modifier_group.cpp` boundary: Rust layers successive group variations, whereas pinned replacement runs restart from the original style and `swapRuns` after each nonempty group. |
...
|---:|---|---|---|
| H1 | `StyledText::unichars` (65) | Retained value `text/fully_shaped_text.rs:22::StaticShapedTextTopology::text`, built at `text.rs:2228::StaticTextSlice::shaped_topology_from_resolved_runs`. | **exact value under immutable-topology adaptation; no public accessor** |
| H2 | `StyledText::runs` (66) | Retained raw/inclusion metadata `text/fully_shaped_text.rs:23::StaticShapedTextTopology::resolved_runs`; shaped output `:24::contextual_glyphs`. | **adapted representation**: direct style identity and per-run relative length replace a literal public `TextRun` span. |
...
- dynamic all-run positions and direct style identity replace the literal
  `StyledText::TextRun::styleId` indirection, but successive modifier groups do
  not yet reproduce pinned original-style replacement and `swapRuns` behavior;
