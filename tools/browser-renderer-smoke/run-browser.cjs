...
const fullCases = [
  {
    ...
    expected: [
      ...
      "canvas-lease=exclusive rejection=typed retained-frame=blocking generations=2 presented=2 configure=2 unconfigure=2 getCurrentTexture=2",
      "retained-frame-removal=explicit+direct frame-b=parent-only surface=2 mapRead=2 putImageData=0",
      "resize=webgpu in-flight=rejected extent=13x9 presented=true",
      "surface-acquisition-loss=recreated retry=same-present alpha=premultiplied acquisitions=2 surfaces=2",
      "surface-acquisition-persistent-loss=typed retry=bounded acquisitions=2 surfaces=2",
