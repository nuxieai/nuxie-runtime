# Formula Owned Color Mutation Runtime Contract

Purpose: extend non-number owned-context source mutation to color sources
...
- After binding, `StateMachineInstance::set_owned_view_model_context_color_source_for_data_bind`
  mutates that owned root color source by state-machine data-bind index.
- A direct `DataConverterFormula` number bind using that color source reapplies
  on the next state-machine data-bind application and matches C++'s fallback
  behavior.
- A direct observer bind for the same owned color source updates alongside the
  formula fallback bind.
- The C++ probe retains the active owned color context and exposes
  `--runtime-set-owned-view-model-source-color` for parity comparison.

## Covered Case

- Direct formula fallback with an owned root color source, a post-bind owned
  source mutation, a zero-second advance, and a subsequent positive advance.

## Out Of Scope

- Number, symbol-list-index, boolean, and enum owned source mutation, covered
  by their own contracts.
- String, trigger, list, asset, artboard, or view-model owned source mutation
  APIs.
...
- Focused C++ probe test:
  - `state_machine_owned_viewmodel_color_formula_source_mutation_matches_cpp_probe`
