...
nested traversal reads mounted child occurrences; component lists retain row
identity and draw-index order; scripted drawables/layouts dispatch directly.
Ordinary `Artboard::draw`/`drawInternal` walks those live families with lazy
clipping, occurrence-owned frame origin, and no current-object
`RuntimeDrawCommand` materialization. The diagnostic command-stream
compatibility surface still represents an initially resolved authored nested
reference when it is intentionally built from only one graph; live renderer
traversal requires the mounted child identity. Whole-file manifest rows remain
`pending-verification` until the orchestrator's independent run. RD-C7 remains
blocked by the measured user checkpoint and no scene-cache deletion has begun.
