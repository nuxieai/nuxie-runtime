# Data Binding Graph StringRemoveZeros Main-To-Target Two-Way Target Dirty Runtime Contract

## Purpose

Pin C++ state-machine target-dirty behavior for direct
`DataConverterStringRemoveZeros` string-to-string binds on
...
- Direct `DataConverterStringRemoveZeros` conversion.
...
- Subsequent normal state-machine advancement overwriting the string target
  from the unchanged source through C++ remove-zero conversion.
- Exact C++ `stringBindings` probe reporting for the mutating string source and
  target after each explicit runtime action.

## Out Of Scope

- `DataConverterStringPad`, converter groups, and trim behavior beyond the
  already covered direct trim slice.
...
- A `TwoWay` direct remove-zeros bind applies the converted source before
  target mutation.
...
- Normal state-machine advancement after the explicit data-context step
  overwrites the target from the unchanged source through C++ remove-zero
  conversion, including when elapsed time is zero.
