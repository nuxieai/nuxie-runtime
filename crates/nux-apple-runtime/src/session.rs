...
/// One app-clock advance. The first delta after create/resume is zero. The
/// Apple drawable is borrowed for the synchronous perform call. The completion
/// context remains live until its exactly-once callback. The runtime never
/// invokes the callback synchronously or waits for it. After submission is
/// skipped, fails, or Metal signals completion, the callback is deferred to a
/// system dispatch queue and may begin before perform returns. Until return, it
/// must not access this operation or the result output, re-enter the runtime
/// with the session, or release the session or dependent handles. A higher-level
/// wrapper must gate or marshal user completion onto its own safe executor. The
/// callback must not call UIKit.
#[repr(C)]
#[derive(Debug, Clone, Copy)]
...
impl Drop for OwnedAdvanceOperation {
    fn drop(&mut self) {
        if let Some(callback) = self.completion_callback.take() {
            defer_frame_completion(callback, self.completion_context_identity);
        }
    }
}
...
mod configured_session_seam {
    ...
    pub(super) fn perform(
        ...
        match token.worker.call(Some(session_id), move |state| {
...
/// Ordered outputs are returned in the owned result. State batches are atomic
/// and pointer batches preserve immediate subcycles inside their returned
/// `cycle` values. An advance completion follows the asynchronous timing and
/// re-entry rules documented by `NuxScreenAdvanceOperation`.
///
/// # Safety
///
...
/// An advance completion may begin concurrently but must not use or release the
...
pub unsafe extern "C" fn nux_screen_session_perform(
    ...
    ffi_guard_with_session_result(
        ...
        || {
            ...
            let (status, result) = configured_session_seam::perform(session_token, operation);
            let result_status = replace_session_result(out_result, result);
            if result_status == NuxStatus::Ok {
                status
            } else {
...
mod tests {
    ...
    #[cfg(feature = "apple-product")]
    fn wait_for_completion(counter: &AtomicUsize) {
    ...
    #[cfg(feature = "apple-product")]
    #[test]
    fn rejected_advance_completion_cannot_block_perform_return() {
        struct CompletionGate {
            released: AtomicBool,
            calls: AtomicUsize,
        }

        unsafe extern "C" fn await_release(context: *mut c_void) {
            let gate = unsafe { Arc::from_raw(context.cast::<CompletionGate>()) };
        ...
        let callback_context = Arc::into_raw(Arc::clone(&gate)).expose_provenance();
        ...
        let caller = std::thread::spawn(move || {
            let advance = NuxScreenAdvanceOperation {
                ...
                render: 1,
                apple_drawable: ptr::dangling_mut(),
                completion_context: ptr::with_exposed_provenance_mut(callback_context),
                completion_callback: Some(await_release),
            };
            let mut request = operation(NUX_SCREEN_SESSION_OPERATION_KIND_ADVANCE);
            request.advance = &advance;
            let mut result = ptr::null_mut();
        ...
        let returned_without_callback = status_receiver.recv_timeout(Duration::from_secs(1));
        let did_return_without_callback = returned_without_callback.is_ok();
        gate.released.store(true, Ordering::SeqCst);
        let status = returned_without_callback.unwrap_or_else(|_| {
            status_receiver
                .recv_timeout(Duration::from_secs(1))
                .expect("perform returns once the synchronous callback is released")
        });
        caller.join().expect("perform caller does not panic");
        wait_for_completion(&gate.calls);

        assert_eq!(status, NuxStatus::NullArgument);
        assert_eq!(gate.calls.load(Ordering::SeqCst), 1);
        assert!(
            did_return_without_callback,
            "perform waited for its completion callback instead of returning independently"
        );
    }
