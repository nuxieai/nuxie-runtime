...
// source and local-tooling roots. Watching them catches file creation without
// recursively watching `.git`, the workspace `target` directory, or generated
// tool output. `.cargo` is kept present by its committed README watch anchor.
const IDENTITY_SOURCE_ROOTS: &[&str] = &["crates", "vendor", ".cargo"];
...
fn emit_identity_rerun_inputs(repo_root: &std::path::Path) {
    ...
    for source_root in IDENTITY_SOURCE_ROOTS {
        println!(
            ...
            repo_root.join(source_root).display()
        );
    }
