...
This document narrows the formal goal for `rive-schema` and `rive-binary`.
It is the operating contract for finishing binary import parity without letting
`rive-binary` grow into the whole Rive runtime.

## Formal Goal

Finish `rive-schema` and `rive-binary` to C++ parity for schema generation and
...
out of scope for `rive-binary`.

## Scope Lock

`rive-binary` owns file loading. It does not own runtime execution.

The external seam for `rive-binary` is:

- Input: `.riv` bytes plus generated schema metadata from `rive-schema`.
...
Everything past that seam belongs to `rive-graph` or later runtime crates unless
it is needed to prove immediate import parity.

## Owned By `rive-schema`

`rive-schema` is complete for this goal when it covers:
...
`rive-schema` does not need to generate concrete runtime behavior structs for this
goal. That belongs to runtime crates.

## Owned By `rive-binary`

`rive-binary` is complete for this goal when it covers:
...
These relationships are allowed in `rive-binary` only when they are immediate
...
than the stable public interface of `rive-binary`.
...
helpers to `rive-binary`.

When adding a new C++ probe assertion, classify it as one of:

- Import-owned: proves a fact `rive-binary` must expose or preserve.
- Test-only: protects assumptions but should not become public API.
- Runtime-owned: belongs in `rive-graph` or a later runtime crate.
- Out-of-scope: useful later, but not needed for this goal.

## Out Of Scope For `rive-binary`

The following are explicitly not part of the `rive-schema`/`rive-binary`
completion goal:
...
If the answer is no to all four, do not add it to `rive-binary`.
...
- `rive-schema` has schema parity coverage for the metadata used by binary import.
- `rive-binary` passes focused synthetic tests for header, primitive, object,
...
- Each public `RuntimeFile` helper is classified as import-owned, test-supporting,
  or a candidate to move out of `rive-binary`.
- Any remaining C++ runtime behavior discovered during audits is recorded under
  "out of scope" or moved to a later `rive-graph`/runtime plan.
...
should be triaged against this contract before blocking `rive-binary` completion.

## What Actually Remains

The remaining work should be closure work:

- Audit the current `rive-binary` public surface against this contract.
...
- Avoid adding new runtime lifecycle behavior to `rive-binary` unless the admission
  rule classifies it as import-owned.

The likely end state is not a much larger `rive-binary`; it is a clearer one.
