...
- Keep the renderer's opaque `WgpuPresentationSurface` and
  `WgpuPresentationFrame` boundary as the only browser presentation mechanism
  owned by this repository.
- Let the browser owner construct its canvas-presentation adapter around
  `WgpuFactory`; `BrowserFactory`, canvas lifecycle, resizing, and bounded
  recovery are product policy and live in nuxie-dev.
...
- Keep active-frame resize protection in the browser-owned adapter.
- Make ordinary browser presentation submit directly to the WebGPU canvas
...
- The browser-owned adapter recovers an `Outdated` surface by reconfiguring it
  once and a `Lost` surface by recreating it from the retained canvas. Runtime
  only exposes typed opaque acquisition and presentation operations.
- Reserve renderer readback for explicit pixel capture. It returns exactly
  `width * height * 4` RGBA bytes and does not present.
...
- Simplify `tools/browser-renderer-smoke` to exercise runtime-owned WebGPU
  behavior only, including Core/Compatibility admission, direct opaque
  presentation, premultiplied-alpha composition, stream replay, GPU-canvas,
  path clip, explicit readback, and explicit failure when WebGPU is unavailable.
  Canvas lifecycle and recovery coverage belongs to nuxie-dev.
...
4. `make browser-renderer-smoke`
   proves direct presentation calls neither `GPUBuffer.mapAsync` nor
   `CanvasRenderingContext2D.putImageData`, while explicit readback performs
   one mapped readback and returns exact RGBA pixels. Browser-owned lifecycle
   and bounded recovery are tested in nuxie-dev.
