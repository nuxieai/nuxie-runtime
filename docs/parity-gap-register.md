...
| V10 | **Perf gate is non-blocking and thin**: CI `perf-json` is `continue-on-error`; corpus is 6 hardcoded files; null-renderer only. Renderer timing gate exists but is separate. | `tools/perf-compare`, `.github/workflows/ci.yml`; five-largest-fixture timing and static-archive evidence: `docs/perf-size-evidence.md` | Make the hot-loop gate blocking at ratio ≤ 1.0 on a broadened (≥20-file) perf corpus; publish the ratio per commit like `exact-segments`. |
