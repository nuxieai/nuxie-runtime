...
pub(crate) fn contour_to_path<C: PointCoord>(
    ...
    match style {
        ...
        PathStyle::HarfBuzz => {
            contour_to_path_harfbuzz(points, first_point, last_point, pen)
        }
    }
}
...
fn contour_to_path_harfbuzz<C: PointCoord>(
    ...
    // HarfBuzz rotates contours whose serialized first point is off-curve so
    // that the final serialized point is consumed first. See
    // OT/glyf/glyf.hh::glyf_accelerator_t::get_points. This ordering is
    // observable through hb_font_draw_glyph and must happen before applying
    // path-builder.hh's forward-scan rules.
    if first_point.flags.is_off_curve_quad() {
        let point_count = points.len();
        let rotated = core::iter::once(last_point).chain(points.take(point_count - 1));
        return contour_to_path_harfbuzz_forward(rotated, last_point, pen);
    }

    contour_to_path_harfbuzz_forward(points, first_point, pen)
}

fn contour_to_path_harfbuzz_forward<C: PointCoord>(
...
mod tests {
    ...
    fn start_off_curve_to_path_scan_forward() {
        assert_off_curve_path_to_svg(
            "M2.0,2.0 Q10.0,2.0 4.0,1.0 Q10.0,1.0 6.0,1.5 Q2.0,2.0 2.0,2.0 Z",
            PathStyle::HarfBuzz,
            false,
        );
    }
