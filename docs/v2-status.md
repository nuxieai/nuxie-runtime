...
19. M8 OPENED (user decision 2026-07-09; scope in porting-map-v2 #V2-9).
    ...
    (a) Scripting seam: runtime-owned ScriptingVm/ScriptInstance/
        ...
        `ScriptInputArtboard` values. `replace_view_model.riv` is exact at two
        samples after equal-ID context replacement gained one-shot child
        replacement and instance-revision render caches.
        `viewmodel_from_instance.riv` is exact at two samples after root cold
        user `init` stopped observing a bound `Context` before C++ exposes it;
        the retained selected model still feeds global Data, generators, and
        pre-bind setup. This closes the resource-allocation subgroup.
...
- 2026-07-09: [M8] Promoted `relative_data_bind_path.riv` to scripted exact
  ...
  `ScriptAsset` its C++ file-asset importer scope, so its payload no longer
  overwrites the valid manifest; non-scripting import keeps its separate C++
  stack behavior. Nested artboard instances retain
  `DataBindPath.isRelative` and resolve those paths through manifest names
  against each parent context.
  Ordinary nested-artboard properties remain in the generic generated-setter
  binding path while lifecycle properties keep their specialized handling;
  unresolved name-based colors resolve during full root-context binding
  without entering the reduced nested-context-only binding graph used by the
  regular runner. Scripted compare moves to exact=22 / exact-segments=28 /
  diverges=4 / unsupported-feature=1 with M8=1. Next is the smallest remaining
  stream, `databind_viewmodel.riv`.

- 2026-07-09: [M8] Promoted `viewmodel_from_instance.riv` to scripted exact
  and widened it from one to two samples. C++ preserves the selected model for
  global Data, generator construction, and later binding, but root script user
  `init` does not see that model as its `Context` during cold construction.
  The scripted runner now makes that lifecycle distinction while leaving
  already-bound nested scripts unchanged, removing two premature child
  instances and their four paints. Scripted compare moves to exact=21 /
  exact-segments=26 / diverges=5 / unsupported-feature=1 with M8=1. Next is
  the focused transform/context gap in `relative_data_bind_path.riv`.

- 2026-07-09: [M8] Promoted `replace_view_model.riv` to scripted exact and
  widened it from one to two samples. Nested-artboard artboard-value bindings
  now distinguish initial context establishment from replacement of an
  already-retained context, force one equal-ID child rebuild on the latter,
  and remain idempotent across subsequent data-bind passes. Same-graph child
  replacements carry an instance revision in paint/path cache identity, so
  their fresh render resources match C++ without invalidating changed-graph
  or unrelated caches. Scripted compare moves to exact=20 /
  exact-segments=24 / diverges=6 / unsupported-feature=1 with M8=1. Full
  regular and scripted compares, `cargo test --workspace`, corpus
  regeneration, formatting, and diff checks pass. Next is the separate
  `viewmodel_from_instance.riv` allocation lifecycle gap.
...
- 2026-07-09: [M8] Admitted `ScriptInputArtboard`, `GamepadInput`, and
  ...
  They are scripting/listener metadata and do not participate in text layout;
  their runtime paths were already implemented. `gamepad_test.riv` now emits
  a 20 KB Rust stream and advances to runnable divergence. Scripted compare
  reports exact=11 / exact-segments=11 / diverges=15 /
  unsupported-feature=1. Full compare reports exact=263 /
  exact-segments=584 / diverges=26 / unsupported-feature=6 with parked M8=1 /
  gated=5; `cargo test --workspace`, corpus regeneration, formatting, and diff
  checks pass. `script_create_text_runs.riv` and its authored text-run API are
  the sole remaining M8 unsupported entry.

- 2026-07-09: [M8] Aligned nested-child and static-text preflights with the
  runtime binding matrix exercised by `data_viz_demo.riv`: range-mapped trim
  offset and follow-path distance, computed shape length, normalized shape
  scale, formula/operation-bound linear-gradient coordinates, direct cubic
  mirrored vertex distance, and the concrete `CubicWeight` sibling are now
  admitted. Each path already routes through generated property setters and
  implemented converters; no evaluation behavior was added. The 450 KB Rust
  stream now executes and the file moves to runnable divergence. The asset is
  `scripted-runner-only`: the default C++ debug runner aborts in
  `AABB::inset` on `r.width() >= 0`, while the scripted release runner emits
  the 460 KB reference stream. Scripted compare reports exact=11 /
  exact-segments=11 / diverges=14 /
  unsupported-feature=2. Full compare reports exact=263 /
  exact-segments=584 / diverges=25 / unsupported-feature=7 with parked M8=2 /
  gated=5; `cargo test --workspace`, corpus regeneration, formatting, and diff
  checks pass. The remaining M8 unsupported queue is the two concrete text
  surfaces in `gamepad_test.riv` and `script_create_text_runs.riv`.

- 2026-07-09: [M8] Admitted the exact nested-child property paths already
  implemented by the runtime: formula-converted `CustomPropertyNumber`,
  inherited `NestedArtboard` x/y, and `NestedArtboard.artboardId`.
  `relative_data_bind_path.riv` and `replace_view_model.riv` advance to
  runnable divergence, while `data_viz_demo.riv` reaches the sharper
  `nested-trim-path-data-bind` diagnostic. Scripted compare reports exact=11 /
  exact-segments=11 / diverges=13 / unsupported-feature=3. Full compare reports
  exact=263 / exact-segments=584 / diverges=24 / unsupported-feature=8 with
  parked M8=3 / gated=5; `cargo test --workspace`, corpus regeneration,
  formatting, and diff checks pass. Next target is one of the three concrete
  M8 blockers: nested trim-path data binding or the two text surfaces.
...
- 2026-07-09: [M8] Narrowed the static text preflight for the corpus-backed
  ...
  `scripted_data_converter_bound_input.riv` to execution. The focused stream
  now reaches real converter differences in bound position and fill color, so
  this is a diagnostic ratchet from unsupported to runnable divergence, not a
  claim that scripted converter behavior is complete. Scripted compare reports
  exact=11 / exact-segments=11 / diverges=8 / unsupported-feature=7. Full
  compare reports exact=263 / exact-segments=584 / diverges=19 /
  unsupported-feature=13 with parked M8=7 / gated=6; `cargo test --workspace`,
  corpus regeneration, formatting, and diff checks pass. Next target is the
  highest-leverage remaining M8 blocker from the seven-file manifest queue.
...
- 2026-07-09: [M8] Promoted `scripted_as_path.riv` to scripted-mode exact by
  ...
  and `ScriptedNode::asPaint` surface from C++ `src/lua/lua_artboards.cpp`.
  The runtime seam returns VM-neutral path/paint snapshots; Luau wraps them as
  node, path, and read-only paint-data userdata. Script-artboard input paint
  allocation now follows C++ lifecycle order: source paints, script
  generator/init paints, selected-root clone paints, then pending child clone
  paints. Scripted compare reports exact=9 / exact-segments=9 / diverges=0 /
  unsupported-feature=17. Full compare reports exact=263 /
  exact-segments=584 / diverges=9 / unsupported-feature=23, and
  `cargo test --workspace` passes. Next target is the tied-highest bounded
  `scripted-path-effects` queue (3 files); the four-file view-model queue
  remains parked behind its recorded vertical-slice design requirement.
...
- 2026-07-09: [M8 scout] Rejected an oversized first attempt at the
  ...
  `viewmodel_from_instance.riv` established that its first `Data.Child`
  failure is not a bounded constructor-only slice: reaching the next stable
  diagnostic requires file-level `Data` constructors, mutable string/number
  property userdata, `context:viewModel()` lifecycle handoff, named
  `ScriptedViewModel:instance`, and binding a constructed view model through
  `Artboard:instance(viewModel)`. A roughly 500-line cross-crate prototype
  exercised that complete chain through the artboard-instance call but still
  ended on a later nil property and moved no exact segment. It was fully
  backed out rather than committed without an end condition. The clean-tree
  `make scripted-golden-compare` baseline remains exact=7 /
  exact-segments=7 / diverges=0 / unsupported-feature=19. Do not restart this
  queue as independent API stubs; return with one corpus-backed vertical
  artboard/view-model binding design. Next target is the bounded two-file
  `script-init-paint` queue.
