...
| P06 | Linear gradients | active | Public v26 candidate; 7 native/WASM parity tests, 272 initial/composition original-clone frames and 48 realistic native-glyph lifecycle frames pass with audited visual coverage. Corrected ordinary Atomics has 54 audited composition views and affine image controls. Frozen broad regression passes 8,062 checks / 8,052 reviewed pairs; 21 normal integration views have exact visual-transfer receipts. Final numeric boundaries, renderer migration, upstream integration and retained-frame performance remain. See validation/linear-gradient-implementation-notes.md. |
