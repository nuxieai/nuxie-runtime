...
pub struct Component {
    ...
    pub dct_scale_h: usize,
    pub dct_scale_v: usize,
...
impl FrameInfo {
    pub(crate) fn update_idct_size(&mut self, idct_size: usize) -> Result<()> {
        for component in &mut self.components {
            component.dct_scale_h = idct_size;
            component.dct_scale_v = idct_size;
        }
    ...
    pub(crate) fn apply_rive_v9f_fancy_scaling(&mut self) -> Result<()> {
        // Literal owner: rive-app_libjpeg_v9f/jdmaster.c,
        // jpeg_calc_output_dimensions().  Rive decodes renderer images at the
        // source's default full-size minimum DCT scale with fancy upsampling.
        let min_dct_h_scaled_size = 8;
        let min_dct_v_scaled_size = 8;
        let max_h_samp_factor = self
            .components
            .iter()
            .map(|component| usize::from(component.horizontal_sampling_factor))
            .max()
            .unwrap();
        let max_v_samp_factor = self
            .components
            .iter()
            .map(|component| usize::from(component.vertical_sampling_factor))
            .max()
            .unwrap();

        for component in &mut self.components {
            let mut ssize = 1;
            while min_dct_h_scaled_size * ssize <= 8
                && max_h_samp_factor
                    % (usize::from(component.horizontal_sampling_factor) * ssize * 2)
                    == 0
            {
                ssize *= 2;
            }
            component.dct_scale_h = min_dct_h_scaled_size * ssize;

            ssize = 1;
            while min_dct_v_scaled_size * ssize <= 8
                && max_v_samp_factor % (usize::from(component.vertical_sampling_factor) * ssize * 2)
                    == 0
            {
                ssize *= 2;
            }
            component.dct_scale_v = min_dct_v_scaled_size * ssize;

            if component.dct_scale_h > component.dct_scale_v * 2 {
                component.dct_scale_h = component.dct_scale_v * 2;
            } else if component.dct_scale_v > component.dct_scale_h * 2 {
                component.dct_scale_v = component.dct_scale_h * 2;
            }
        }
        update_component_sizes(self.image_size, &mut self.components)?;
        Ok(())
