...
fn rotate(v: Vec2D, angle: f32) -> Vec2D {
    ...
    Vec2D::new(cos.mul_add(v.x, -sin * v.y), sin.mul_add(v.x, cos * v.y))
}

fn cross(a: Vec2D, b: Vec2D) -> f32 {
...
fn cubic_tangent_roots(p: [Vec2D; 4], target: Vec2D) -> Vec<f32> {
    let a = Vec2D::new(
        3.0 * (p[3].x - 3.0 * p[2].x + 3.0 * p[1].x - p[0].x),
        3.0 * (p[3].y - 3.0 * p[2].y + 3.0 * p[1].y - p[0].y),
    );
    let b = Vec2D::new(
        6.0 * (p[2].x - 2.0 * p[1].x + p[0].x),
        6.0 * (p[2].y - 2.0 * p[1].y + p[0].y),
    );
    let c = Vec2D::new(3.0 * (p[1].x - p[0].x), 3.0 * (p[1].y - p[0].y));
    let aa = cross(a, target);
    let bb = cross(b, target);
    let cc = cross(c, target);
    ...
    if aa.abs() < 1e-7 {
        ...
        let discriminant = bb.mul_add(bb, -4.0 * aa * cc);
        if discriminant >= 0.0 {
            let root = discriminant.sqrt();
            roots.extend([(-bb - root) / (2.0 * aa), (-bb + root) / (2.0 * aa)]);
        }
    }
...
fn chop_uniform_rotation(
    ...
    if count <= 0 {
        path.cubic(p[1], p[2], p[3]);
        return;
    }
    let mut roots = Vec::new();
    let mut target = tangents[0];
    let mut minimum = 1e-4;
    for _ in 0..count {
        target = rotate(target, rotation);
        if let Some(root) = cubic_tangent_roots(p, target)
            .into_iter()
            .find(|root| *root > minimum && *root < 1.0 - 1e-4)
        {
            roots.push(root);
            minimum = root;
        }
    }
    for curve in crate::draw::chop_cubic_at_values(p, &roots) {
        path.cubic(curve[1], curve[2], curve[3]);
    }
}

fn assert_no_quadratic_segments(path: &RawPath) {
    if path.verbs().contains(&PathVerb::Quad) {
        RIVE_UNREACHABLE();
    }
}

pub(crate) fn coarse_area(path: &RawPath) -> f32 {
    // RawPath::computeCoarseArea() has the same intentional source
    // precondition as makeSoftenedCopyForFeathering(): quads are not valid
    // renderer geometry and hit RIVE_UNREACHABLE rather than being converted.
    assert_no_quadratic_segments(path);
    crate::draw::path_coarse_area(path)
}

pub(crate) fn softened_copy(
    source: &RiveRenderPath,
    feather: f32,
    matrix_max_scale: f32,
) -> RiveRenderPath {
    assert_no_quadratic_segments(&source.m_rawPath);
    // `draw::softened_path_for_feathering` is the literal source translation
    // of this same C++ routine, including its stable q-form quadratic root,
    // root-order filtering, and cusp straddling. Its radius argument is the
    // source `find_feather_radius(feather)` value.
    let mut result = crate::draw::softened_path_for_feathering(
        &source.m_rawPath,
        feather * 1.5,
        matrix_max_scale,
    );
...
impl RiveRenderPath {
    ...
    pub fn update_bounds_and_area(&mut self) {
        self.m_bounds.set(
            self.m_rawPath
                .bounds()
                .unwrap_or(nuxie_render_api::Aabb::new(0.0, 0.0, 0.0, 0.0)),
        );
        self.m_coarseArea.set(coarse_area(&self.m_rawPath));
        self.m_rawPathMutationID.set(self.getRawPathMutationID());
        self.m_dirt.set(0);
    }
    pub fn addRenderPathSource(&mut self, path: &RiveRenderPath, matrix: Mat2D) {
        let verb_start = self.m_rawPath.verbs().len();
        let point_start = self.m_rawPath.points().len();
        self.m_rawPath.add_path(&path.m_rawPath, matrix);
