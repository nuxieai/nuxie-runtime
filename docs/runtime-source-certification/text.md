...
remain adaptations; literal `StyledText::TextRun::styleId` ownership is now
handled by the later rejection-correction unit below. No `text_test.cpp`
consumer moved; this remains an author candidate pending independent semantic
review.
...
That evidence uses two authored groups and four ranges, observes every clear
and group-publication boundary in order, and exercises `clampChanged` through
both shape-modifier and paint-only groups while retaining their maps.
No consumer moved; topology remains four pass, three executable expected-red,
and 11 pending.

Control-size production-correction candidate note: row 10 now has one
occurrence-owned five-field retained state and one callback that compares the
complete tuple, writes it, then invokes `markShapeDirty(false)`. Direct
LayoutComponent propagation, the LayoutParticipant initial solve, and its
animated advance all call that owner. Direct LayoutComponent interpolation
also calls it after each intermediate size write and the terminal write;
controlled sizing reads the retained tuple instead of repolling a newer Taffy
result. Focused evidence
`text.rs::cxx_control_size_retains_all_fields_before_no_layout_shape_dirt`
covers each field and the identical no-op, while
`draw.rs::layout_size_change_does_not_control_text_behind_a_solo` covers the
real direct-child propagation boundary and
`draw.rs::animated_layout_control_size_reaches_text_at_intermediate_and_completion`
covers the live animation caller and preferred retained constraint. The
disabled empty body remains red.
No consumer moved; topology remains four pass, three executable expected-red,
and 11 pending.
...
Text on-dirty production-correction candidate note: row 39 now runs at the
real `Component::addDirt` callback boundary, before Artboard dirty publication
and before `Text::update`. WorldTransform visits every current retained
modifier group in authored order; follow-path groups re-enter the same callback
by adding Path to the accumulated mask. Path or Paint visits only the distinct
current render styles retained in draw-command order and invalidates every
occurrence-owned paint's stroke-effect chain. The previous later generic Text
path, which invalidated every imported TextStylePaint whether current or not,
has been removed. Focused real-owner evidence is
`draw.rs::cxx_text_on_dirty_visits_current_modifiers_then_current_style_effects`;
it covers three modifier groups, two current render styles with concrete effect
owners, WorldTransform, Path, Paint, and their combined source order. The
...
StyledText rejection-correction candidate note: the four blockers in independent
review `fa99cacfa` are now modeled at their real owners. `resolved_runs` retains
raw source bytes, truncates at the first NUL in byte space before UTF-8
validation, and therefore accepts `[0x00, 0xff]` as a nonempty source with a
zero-count appended run; invalid bytes in the consumed prefix remain a named
Rust safety rejection. Every authored and dynamic all-runs entry retains its
literal wrapping `u16 styleId`, skipped entries consume IDs, and glyph paint
lookup indexes the complete all-runs sequence with that ID. TextRun font,
size, line-height, and letter-spacing still come from the actually appended
run, as pinned. All common topology, bounds, fit, measure, render, and clip
shaper seeds now use the first included run rather than the first imported
TextStylePaint. Finally, live occurrence evidence drives a real TextValueRun
write through `RuntimeTextDrawOwner` invalidation and update publication,
proving the prior topology remains until update and is then replaced by the
leading-NUL zero-run and subsequent prefix topology. Evidence is
`text.rs::cxx_dynamic_text_runs_retain_all_indices_and_make_styled_omissions`,
`text.rs::cxx_styled_text_style_id_wraps_and_drives_all_runs_paint_lookup`,
`text.rs::cxx_styled_text_append_decodes_to_nul_and_rebuild_clears_prior_topology`,
`text.rs::cxx_shape_measure_and_clip_seed_from_first_included_run_font`, and
`draw.rs::text_write_replaces_live_retained_styled_text_topology_on_update`.
The separately owned modifier stream is now audited in
`runtime-source-certification/text-modifier-group.md`. Successive nonzero
groups restart from the authored style font and replace the preceding group's
coordinates under the immutable-run adaptation. The modifier pair still has
separate line-range prepass and failed-inverse follow-path reds. No consumer
moved; topology remains four pass, three executable expected-red, and 11
pending.
...
| 10 | `controlSize` (160; disabled 1446) | On any width/height/scale/direction change, retain all five values then `markShapeDirty(false)`; disabled is inert. | Retained owner `components.rs::RuntimeTextState::retain_control_size`; callback `text/text.rs::control_size`; shared direct caller `draw.rs::ArtboardInstance::propagate_runtime_layout_text_control_size`; solve/animation boundaries `draw.rs::ArtboardInstance::retain_runtime_layout_component_bounds`, `artboard.rs::ArtboardInstance::advance_layout_component_entry`; participant caller `artboard.rs::ArtboardInstance::invalidate_runtime_layout_text_host`; retained consumer `draw.rs::ArtboardInstance::runtime_text_layout_constraint`; evidence `text.rs::cxx_control_size_retains_all_fields_before_no_layout_shape_dirt`, `draw.rs::layout_size_change_does_not_control_text_behind_a_solo`, `draw.rs::animated_layout_control_size_reaches_text_at_intermediate_and_completion` | **exact enabled-path correction candidate; disabled path missing**: one occurrence-owned tuple compares and publishes all five fields before no-layout shape dirt; direct solve, direct intermediate/terminal animation, and participant Taffy seams call it, and controlled consumers prefer that retained value. The disabled no-op body remains red. |
...
| 15 | `fitFontScale` (389) | Find max nonempty styled run size; early return; binary-search integer top size; each probe makes styled text, shapes, breaks, includes paragraph spacing, and checks width/height. | `text.rs:3383::fit_font_scale`; `3434::fit_font_scale_for_max_size`; `3189::static_text_total_height`; complete Silver stream `tools/silver-corpus/tests/silver_backfill_cases.rs:37::text_fit_font_size_source_correction_is_exact`; first-included-run evidence `text.rs::cxx_shape_measure_and_clip_seed_from_first_included_run_font` | **adapted correction candidate with exact consumer evidence**: each binary-search height probe now includes every paragraph's spacing in pinned order and seeds from the first appended TextRun font; the seven-frame `fit_font_size_test` stream is exact. Shaping/line-break representation remains adapted. |
...
| 22 | `addModifierGroup` (970; disabled 1416) | Append group in child/import order; disabled is inert. | construction parent validation `artboard.rs:1361::ArtboardInstance::build_component_occurrence_relations`; authored collection `text.rs:2099-2105::StaticTextSlice::from_graph` | **adapted candidate**: malformed direct parents fail before occurrence publication; immutable modifier collection remains pending order evidence. |
...
| 25 | `modifierShapeDirty` (993; disabled 1427) | Add Path only. | `text/text.rs:139::modifier_shape_dirty`; range dispatch `text/text_modifier_group.rs:377::range_changed`; bool owner `text/text_modifier_group.rs::text_modifier_group_bool_property_changed`; evidence `text.rs:7031::cxx_text_shape_dirty_clears_retained_range_maps_before_group_and_world_dirt` | **exact enabled-path candidate; disabled path missing**: the distinct enabled owner adds Path only and deliberately preserves retained range maps, group coverage dirt, WorldTransform, and layout state. `unitsValueChanged`, `clampChanged`, and shape-modifier-backed range changes route through the pinned Text-before-group callback order. The disabled no-op body remains red. |
...
| 33 | `StyledText::clear` (1027) | Clear Unicode values then runs. | Immutable rebuild `text.rs:3076::StaticTextSlice::resolved_runs` and `2263::StaticTextSlice::shaped_topology_from_resolved_runs`; retained owner `draw.rs::RuntimeTextDrawOwner::shaped_topology_or_build`; live evidence `draw.rs:27432::text_write_replaces_live_retained_styled_text_topology_on_update` | **adapted correction candidate**: a TextValueRun write leaves the old retained publication intact until the update owner invalidates it, then publishes a fresh Unicode/run topology with no prior values. The two internal C++ clear calls have no separately observable Rust mutation order. |
| 34 | `StyledText::empty` (1033) | Emptiness is run-vector emptiness, not character-vector emptiness. | Inclusion state `text.rs::StaticResolvedRun::styled_text_included`; predicate `text.rs::StaticTextSlice::has_styled_text_from_resolved_runs`; evidence `text.rs:6250::cxx_styled_text_append_decodes_to_nul_and_rebuild_clears_prior_topology` and `draw.rs:27432::text_write_replaces_live_retained_styled_text_topology_on_update` | **adapted correction candidate**: emptiness follows whether `makeStyled` appended any font-backed, nonempty-source run, including `[0x00, 0xff]` as a zero-character but nonempty run; it is not inferred from Unicode length. |
| 35 | `StyledText::append` (1035) | Decode UTF-8 until NUL into Unicode values, count code points, append one `TextRun` with supplied style metadata. | Byte prefix `text.rs:1017::cxx_styled_text_prefix`; run owner `3076::StaticTextSlice::resolved_runs`; shaping owner `3734::styled_resolved_run_glyphs`; evidence `text.rs:6250::cxx_styled_text_append_decodes_to_nul_and_rebuild_clears_prior_topology` | **adapted correction candidate**: byte truncation precedes UTF-8 validation, so unread invalid suffixes are ignored exactly; each appended run retains relative character count, source style metadata, and literal styleId. Invalid UTF-8 inside the consumed prefix remains the named Rust safety rejection. |
| 36 | `makeStyled` (1053) | Clear; preserve all-run indices while skipping missing style/font/empty text; append scaled runs; optionally apply modifiers; return run nonempty. Defaults: modifiers=true, scale=1. | Runs `text.rs:3089::StaticTextSlice::resolved_runs`, `3162::resolved_dynamic_runs`; buildRenderStyles paint lookup `3251::all_runs_paint_style_index`; first-run seed `3262::first_included_style`; topology `2276::shaped_topology_from_resolved_runs`; modifier replacement `text.rs:3909::styled_text_glyphs_for_style_with_strengths`; evidence `text.rs::cxx_successive_modifier_groups_restart_from_the_authored_style_font` and `runtime-source-certification/text-modifier-group.md` | **adapted correction candidate**: raw all-runs order and wrapping `u16 styleId` are retained across authored and dynamic entries, including skipped gaps; pinned `buildRenderStyles` paint lookup consumes that ID, while appended TextRun font/metrics come from the actual source style. Every common shaper seed uses the first included run. Successive nonzero modifier groups now restart from the authored style font and replace preceding coordinates; immutable glyph reconstruction substitutes for mutable `swapRuns`. |
| 37 | static `BreakLines` (1085) | Break each paragraph; no-wrap/auto width use -1; auto-width computes global max; then compute spacing/alignment for all paragraphs. | `text.rs:4318::StaticTextSlice::layout_static_text_lines`; `text/line_breaker.rs:1::split_static_text_lines`; `52::static_text_line_iteration`; downstream metrics `text.rs:3231::static_line_metrics`; focused insertion evidence `text.rs:6091::cxx_empty_line_metrics_ignore_runs_omitted_by_make_styled` | **adapted**: custom HarfRust/renderer annotation breaker replaces `GlyphLine::BreakLines`; paragraph boundaries and styled-run insertion ownership now drive downstream metrics, but the shaped line-break representation remains nonliteral. |
| 38 | `modifierRangesNeedShape` (1123; disabled 1428) | Return true on first modifier needing shape, else false; disabled false. | No direct Text member; nearest predicate is `text/text_modifier_group.rs:363::StaticTextModifierGroup::has_shape_modifiers`. | **incorrect**: the current predicate omits line-unit ranges and there is no pinned pre-shape wrapped-line coverage pass; see `runtime-source-certification/text-modifier-group.md` row 28. |
| 39 | `onDirty` (1135; disabled 1420) | WorldTransform notifies all modifier groups; Path/Paint invalidates stroke effects on current render styles. | addDirt callback `artboard.rs::ArtboardInstance::dispatch_component_on_dirty`; occurrence targets `draw.rs::RuntimeDrawableList::text_on_dirty_targets`, `draw.rs::RuntimeTextDrawOwner::on_dirty_targets`, and `text.rs::StaticTextSlice::world_dirty_modifier_groups`; ordered dispatch `draw.rs::RuntimeTextOnDirtyTargets::visit`; effect owner `draw.rs::RuntimeShapeList::invalidate_container_stroke_effects`; evidence `draw.rs::cxx_text_on_dirty_visits_current_modifiers_then_current_style_effects` | **exact enabled-path correction candidate under retained-owner adaptation; disabled path missing**: accumulated WorldTransform visits every current group in authored order and a follow-path group re-enters with Path; Path/Paint then invalidates only distinct current render styles in retained draw-command order, including the combined-mask order. Rust's immutable topology/replay identifies the current pointer lists, while concrete occurrence-owned ShapePaint effects receive the invalidation. The disabled no-op body remains red. |
...
| 41 | `measure` (1259) | Make styled; choose width from authored sizing; choose wrap from max constraint; shape/break; baseline/ellipsis/paragraph spacing/trim; authored-sizing bounds; clamp to max; empty returns zero. | `text/text_engine.rs:54::static_text_layout_measure_bounds`; `text.rs:2790::measure_bounds_with_layout_constraint`; spacing evidence `text.rs:5539::cxx_paragraph_spacing_order_covers_empty_paragraphs_trim_measure_fit_and_render`; first-run evidence `text.rs::cxx_shape_measure_and_clip_seed_from_first_included_run_font` | **adapted correction candidate**: measurement includes inter-paragraph spacing, removes final trailing spacing in auto bounds, and seeds the common shaper from the first appended TextRun font. Taffy projection and fallback behavior remain adaptations. |
...
|---:|---|---|---|
| H1 | `StyledText::unichars` (65) | Retained value `text/fully_shaped_text.rs:22::StaticShapedTextTopology::text`, built at `text.rs:2263::StaticTextSlice::shaped_topology_from_resolved_runs`; live publication evidence `draw.rs:27432::text_write_replaces_live_retained_styled_text_topology_on_update`. | **exact value under immutable-topology adaptation; no public accessor** |
| H2 | `StyledText::runs` (66) | Retained source bytes/inclusion/relative length/literal styleId at `text.rs::StaticResolvedRun`; shaped output `text/fully_shaped_text.rs:24::StaticShapedTextTopology::contextual_glyphs`. | **adapted representation with literal identity**: Rust retains the complete run contract but exposes no public `TextRun` span. |
| H3 | `StyledText::swapRuns` (68) | Immutable glyph reconstruction `text.rs:3747::StaticTextSlice::styled_resolved_run_glyphs`; modifier variation owner `3909::styled_text_glyphs_for_style_with_strengths`. | **adapted correction candidate**: immutable reconstruction substitutes for vector swap; successive nonzero groups now rebuild from the authored style and replace the preceding group's coordinates. The separate authored `m_runs` lookup remains row 18. |
...
| H23 | `Text::runs` (248) | No public complete vector; `text.rs:954::StaticTextSlice::runs` plus `3041::resolved_runs`. | **adapted** |
...
- literal wrapping `StyledText::TextRun::styleId`, its all-runs lookup, and
  successive modifier-group original-style replacement are retained under the
  immutable-run adaptation; the separately audited modifier owner still lacks
  the line-range prepass and failed-inverse retained follow-path behavior;
