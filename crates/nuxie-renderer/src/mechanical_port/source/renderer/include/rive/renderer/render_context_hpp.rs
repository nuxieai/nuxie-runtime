...
impl RenderContext {
    /// Controlled construction of the complete pinned source owner graph.
    /// No caller can publish an independently assembled base/member pair.
    pub(crate) fn pinFromOwner(
        owner: RenderContextImplOwner,
        max_path_id: usize,
    ) -> Pin<Box<Self>> {
        Box::pin(Self {
