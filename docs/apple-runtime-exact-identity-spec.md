...
3. Flow-runtime context creation requires that binding and validates it before
   reading any request pointer. Context and session handles inherit the
   validated identity, so later operations need no repeated identity fields.
...
8. Published SDK and XCFramework releases are atomic: the SDK pins the exact
   artifact URL/checksum and validates the exact runtime identity. No
   compatible-minor range is accepted.
9. The expected identity is checked into the SDK independently from the
   XCFramework's headers. A whole-framework swap must not supply both the
   actual and expected identity.
10. The SwiftPM checksum remains the artifact-byte authority. The identity
    binding independently proves that the linked SDK and runtime were released
    together.
...
- release the new breaking interface as runtime `0.2.0`; existing immutable
  `0.1.x` clients remain pinned to their existing artifacts
...
- replace `NuxieRuntimeABI` in `nuxie-ios` with exact runtime identity;
