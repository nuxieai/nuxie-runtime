# Data Binding Graph ToString Number Main-To-Target Two-Way Target Dirty Runtime Contract

## Purpose

Pin C++ state-machine target-dirty behavior for direct
`DataConverterToString` number-to-string binds on main-`ToTarget | TwoWay`
data binds.
...
back to the number source. Explicit `advancedDataContext()` preserves the
manual target edit, and the next normal `StateMachineInstance::advance()`
reapplies source-to-target conversion from the unchanged number source.
...
- Direct `DataConverterToString` with imported number formatting flags and
  decimals.
...
- Subsequent normal state-machine advancement overwriting the string target
  from the unchanged number source through C++ number-to-string formatting.
...
- String trim, remove-zero, pad, converter-group, color, boolean, trigger,
...
- A `TwoWay` direct number-to-string bind applies the formatted number value
  before target mutation.
- Mutating the string target does not write back to the number source.
