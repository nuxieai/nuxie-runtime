...
fn invalid_table_shapes_and_unsupported_values_are_transactionally_rejected() {
    ...
    let accepted: (bool, bool, bool, bool, bool, bool, bool) = vm
        .eval(
            r#"
                local nuxie = require("nuxie")
                ...
                local oversizedIndex = pcall(function()
                    nuxie.response.set("invalid", { [4294967297] = "value" })
                end)
                return mixed, sparse, unsupported,
                    emptyTrigger, emptyField, emptyObjectKey, oversizedIndex
            "#,
        )
        .unwrap();

    assert_eq!(accepted, (false, false, false, false, false, false, false));
