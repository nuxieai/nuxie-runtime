...
/// Number of exact constraint results retained for each sizing category.
const CACHE_WAYS: usize = 32;
...
pub struct Cache {
    ...
    measure_entries: [[Option<CacheEntry<Size<f32>>>; CACHE_WAYS]; CACHE_SIZE],
...
impl Cache {
    ...
    pub const fn new() -> Self {
        Self { final_layout_entry: None, measure_entries: [[None; CACHE_WAYS]; CACHE_SIZE], is_empty: true }
    }
    ...
    pub fn store(&mut self, input: &LayoutInput, layout_output: LayoutOutput) {
        ...
        match input.run_mode {
            ...
            RunMode::ComputeSize => {
                ...
                let entries = &mut self.measure_entries[cache_slot];
                let index = entries.iter().position(Option::is_none).unwrap_or_else(|| {
                    ((key.kd_available_space ^ key.parent_size) as usize) % CACHE_WAYS
                });
                entries[index] = Some(CacheEntry { key, content: layout_output.size });
            }
    ...
    pub fn is_empty(&self) -> bool {
        self.final_layout_entry.is_none()
            && !self.measure_entries.iter().flatten().any(|entry| entry.is_some())
    }
}
...
mod tests {
    ...
    fn retains_multiple_exact_measurements_in_one_constraint_category() {
        ...
        let inputs: [_; 24] = core::array::from_fn(|index| {
        ...
        for (index, input) in inputs.iter().enumerate() {
            cache.store(
                ...
                LayoutOutput::from_outer_size(Size {
                    width: index as f32 + 100.0,
                    height: index as f32 + 200.0,
                }),
            );
        }
