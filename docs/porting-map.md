...
Current #12 update: graph formula random object fallback explicit target-to-source
Direct graph-owned `DataConverterFormula` object-like sources now cover
explicit `advanceDataContext()` target-to-source scheduling for
`FunctionType::random` output tokens with `randomModeValue` values `0`, `1`,
and `2`. Rust preserves the unchanged object-like source after the edited
number target is converted, matches C++'s single random-provider call during
that conversion, and makes no extra random calls when the unchanged source is
reapplied through the fallback path. The contract is
`docs/prototypes/data-binding-graph-formula-random-object-fallback-explicit-target-to-source-runtime-contract.md`.
Random object public-update and target-dirty scheduling, imported/owned
...
Current #12 update: graph formula random object fallback target-dirty
Direct graph-owned `DataConverterFormula` object-like sources now cover main
`ToTarget | TwoWay` target-dirty scheduling for `FunctionType::random` output
tokens with `randomModeValue` values `0`, `1`, and `2`. Rust preserves the
manual number-target edit through explicit data-context advancement, then
reapplies C++'s `0.0` fallback from the unchanged object-like source on later
normal state-machine advancement, with zero random-provider calls on both
sides. The contract is
`docs/prototypes/data-binding-graph-formula-random-object-fallback-main-to-target-two-way-target-dirty-runtime-contract.md`.
Imported/owned contexts, source mutation APIs, relative/parent/nested lookup,
...
Current #12 update: graph formula artboard imported/owned contexts
Direct graph-owned `DataConverterFormula` artboard sources now cover imported
and owned root view-model contexts. Rust refreshes the artboard source when
rebinding to an alternate serialized `ViewModelInstance` or a mutated owned
context, then preserves C++'s formula object fallback by writing `0.0` to the
number target while a same-path artboard observer reports the rebound artboard
id. The contract is
`docs/prototypes/data-binding-graph-formula-artboard-context-runtime-contract.md`.
View-model pointer formula contexts, random/function-token asset/artboard
contexts, source mutation APIs, relative/parent/nested lookup, listener-owned
data binding, nested artboard propagation, and full dirty-list scheduler
parity remain follow-up `#12` slices.

Current #12 update: graph formula view-model imported/owned contexts
Direct graph-owned `DataConverterFormula` view-model pointer sources now cover
imported and owned root view-model contexts. Rust refreshes the pointer source
when rebinding to an alternate serialized root `ViewModelInstance` or a
relinked owned context, then preserves C++'s formula object fallback by
writing `0.0` to the number target while a same-path view-model observer
reports the rebound child instance index. The contract is
`docs/prototypes/data-binding-graph-formula-viewmodel-context-runtime-contract.md`.
Random/function-token object contexts, source mutation APIs, relative/parent/
nested lookup, listener-owned data binding, nested artboard propagation, and
full dirty-list scheduler parity remain follow-up `#12` slices.

Current #12 update: graph formula random asset imported/owned contexts
Direct graph-owned `DataConverterFormula` asset sources now cover imported and
owned root view-model contexts when the formula output token is
`FunctionType::random` with `randomModeValue` values `0`, `1`, and `2`. Rust
refreshes the rebound asset source, preserves C++'s object fallback by writing
`0.0` to the number target, keeps a same-path asset observer on the rebound
asset id, and reports zero random-provider calls just like C++. The contract
is
`docs/prototypes/data-binding-graph-formula-random-asset-context-runtime-contract.md`.
Random/function-token artboard and view-model pointer contexts, source
mutation APIs, relative/parent/nested lookup, listener-owned data binding,
nested artboard propagation, and full dirty-list scheduler parity remain
follow-up `#12` slices.

Current #12 update: graph formula random artboard imported/owned contexts
Direct graph-owned `DataConverterFormula` artboard sources now cover imported
and owned root view-model contexts when the formula output token is
`FunctionType::random` with `randomModeValue` values `0`, `1`, and `2`. Rust
refreshes the rebound artboard source, preserves C++'s object fallback by
writing `0.0` to the number target, keeps a same-path artboard observer on the
rebound artboard id, and reports zero random-provider calls just like C++. The
contract is
`docs/prototypes/data-binding-graph-formula-random-artboard-context-runtime-contract.md`.
Random/function-token view-model pointer contexts, source mutation APIs,
relative/parent/nested lookup, listener-owned data binding, nested artboard
propagation, and full dirty-list scheduler parity remain follow-up `#12`
slices.
...
Current #12 update: graph formula owned asset source mutation
Direct graph-owned `DataConverterFormula` asset sources now cover post-bind
owned root asset source mutation by state-machine data-bind index. The
same-path direct asset observer and formula fallback number target both
refresh after
`StateMachineInstance::set_owned_view_model_context_asset_source_for_data_bind`.
The C++ probe now retains active owned asset contexts for this comparison. The
contract is
`docs/prototypes/data-binding-graph-formula-owned-asset-mutation-runtime-contract.md`.
Artboard and view-model pointer owned source mutations, imported/default
source mutation APIs, relative/parent/nested lookup, listener-owned data
binding, nested artboard propagation, and full dirty-list scheduler parity
remain follow-up `#12` slices.

Current #12 update: graph formula owned artboard source mutation
Direct graph-owned `DataConverterFormula` artboard sources now cover post-bind
owned root artboard source mutation by state-machine data-bind index. The
same-path direct artboard observer and formula fallback number target both
refresh after
`StateMachineInstance::set_owned_view_model_context_artboard_source_for_data_bind`.
The C++ probe now retains active owned artboard contexts for this comparison.
The contract is
`docs/prototypes/data-binding-graph-formula-owned-artboard-mutation-runtime-contract.md`.
View-model pointer owned source mutations, imported/default source mutation
APIs, relative/parent/nested lookup, listener-owned data binding, nested
artboard propagation, and full dirty-list scheduler parity remain follow-up
`#12` slices.

Current #12 update: graph formula owned view-model pointer source mutation
Direct graph-owned `DataConverterFormula` view-model pointer sources now cover
post-bind owned root pointer source mutation by formula data-bind index. Formula
number source nodes retain the referenced view-model instance id table needed
for `StateMachineInstance::set_owned_view_model_context_view_model_source_for_data_bind`.
The same-path direct view-model observer and formula fallback number target
both refresh after the pointer relink. The C++ probe now retains active owned
view-model pointer contexts for this comparison and uses the same
`ViewModelInstanceRuntime::replaceViewModelByName` path for pointer mutation as
it uses for initial owned-context binding. The contract is
`docs/prototypes/data-binding-graph-formula-owned-viewmodel-mutation-runtime-contract.md`.
Imported/default source mutation APIs, relative/parent/nested lookup,
listener-owned data binding, nested artboard propagation, and full dirty-list
scheduler parity remain follow-up `#12` slices.
...
Current #12 update: graph formula default asset source mutation
Direct graph-owned `DataConverterFormula` asset sources now cover post-bind
default-context root asset source mutation by formula data-bind index. The
existing default asset source mutation path already fans out to every matching
same-path graph asset source node, so the formula fallback number target and
direct asset observer both refresh after
`StateMachineInstance::set_default_view_model_asset_source_for_data_bind`.
The contract is
`docs/prototypes/data-binding-graph-formula-default-asset-mutation-runtime-contract.md`.
Default artboard and view-model pointer source mutations, relative/parent/
nested lookup, listener-owned data binding, nested artboard propagation, and
full dirty-list scheduler parity remain follow-up `#12` slices.
...
Current #12 update: graph formula default view-model pointer source relink
Direct graph-owned `DataConverterFormula` view-model pointer sources now cover
post-bind default-context root pointer relink by formula data-bind index. The
existing default view-model pointer relink path already fans out to every
matching same-path graph view-model source node, so the formula fallback
number target and direct view-model observer both refresh after
`StateMachineInstance::relink_default_view_model_view_model_source_for_data_bind`.
The contract is
`docs/prototypes/data-binding-graph-formula-default-viewmodel-relink-runtime-contract.md`.
Relative/parent/nested lookup, listener-owned data binding, nested artboard
propagation, and full dirty-list scheduler parity remain follow-up `#12`
slices.

Current #12 update: default object-source formula fallback random
source-change behavior now has an explicit C++ parity boundary. Direct
default-context asset/artboard mutation and view-model pointer relink with a
`DataConverterFormula` `FormulaTokenFunction(random)` fallback and
`randomModeValue == 2` update the same-path object observer rows while keeping
the runtime random provider call count at zero, including the explicit
data-context advance required by the view-model pointer relink path. The
contract is
`docs/prototypes/data-binding-graph-formula-default-object-random-source-change-runtime-contract.md`.
Imported/owned post-bind object random-inert mutation checks, grouped
object-source fallback permutations, relative/parent/nested lookup,
listener-owned data binding, nested artboard propagation, and full dirty-list
scheduler parity remain follow-up `#12` slices.
...
`DataConverterGroup<FormulaFallback, OperationValue>` shape. Asset and artboard
source mutation and view-model pointer relink update same-path object observer
rows while reapplying the grouped converter to the number target, and
...
`DataConverterGroup<FormulaFallback, OperationValue>` shape. Imported alternate
contexts and API-owned contexts preserve asset, artboard, and view-model pointer
sources while matching C++ number target values, and
`FormulaTokenFunction(random)` fallback modes `0`, `1`, and `2` remain
random-inert with zero runtime random provider calls. The contract is
...
fallback source-to-target behavior now covers the three-child
...
Asset, artboard, and view-model pointer sources preserve C++ admission into
number targets. Formula-first longer groups remain random-inert for object
sources, while operation-before-formula longer groups match C++'s one initial
random provider call for `FormulaTokenFunction(random)` modes `0`, `1`, and
`2` with no extra call on a later no-change advance. The contract is
`docs/prototypes/data-binding-graph-formula-default-object-random-longer-group-runtime-contract.md`.
Additional longer group permutations (four-or-more-child C++ oracle coverage,
multiple formulas, non-operation children, imported/owned context variants,
mutation/relink, and target-to-source/public-update paths),
relative/parent/nested lookup, listener-owned data binding, nested artboard
propagation, and full dirty-list scheduler parity remain follow-up `#12`
slices.

Current #12 update: default-context longer grouped object-source formula
fallback source-to-target C++ oracle coverage now also covers four-child groups
with the formula in every position:
...
Formula-first four-child groups remain random-inert for object sources, while
all operation-before-formula positions match C++'s one initial random provider
call for `FormulaTokenFunction(random)` modes `0`, `1`, and `2` with no extra
call on a later no-change advance. The contract remains
`docs/prototypes/data-binding-graph-formula-default-object-random-longer-group-runtime-contract.md`.
Additional longer group permutations (five-or-more-child C++ oracle coverage,
multiple formulas, non-operation children, imported/owned context variants,
mutation/relink, and target-to-source/public-update paths),
relative/parent/nested lookup, listener-owned data binding, nested artboard
propagation, and full dirty-list scheduler parity remain follow-up `#12`
...
Current #12 update: owned runtime string source mutation now preserves the
`DataConverterOperationViewModel` non-number secondary operand fallback for
direct converters and for
`DataConverterGroup<OperationValue, OperationViewModel>`. The same-path string
...
Current #12 update: owned runtime enum source mutation now preserves the
`DataConverterOperationViewModel` non-number secondary operand fallback for
direct converters and for
`DataConverterGroup<OperationValue, OperationViewModel>`. The same-path enum
observer updates, while the operation-viewmodel operand stays on C++'s `0.0`
fallback. The contract is
`docs/prototypes/data-binding-graph-operation-viewmodel-owned-enum-mutation-runtime-contract.md`.

Current #12 update: owned runtime trigger source mutation now preserves the
`DataConverterOperationViewModel` non-number secondary operand fallback for
direct converters and for
`DataConverterGroup<OperationValue, OperationViewModel>`. The same-path
trigger observer updates, while the operation-viewmodel operand stays on
C++'s `0.0` fallback. The contract is
`docs/prototypes/data-binding-graph-operation-viewmodel-owned-trigger-mutation-runtime-contract.md`.

Current #12 update: owned runtime list source mutation now preserves the
`DataConverterOperationViewModel` non-number secondary operand fallback for
direct converters and for
`DataConverterGroup<OperationValue, OperationViewModel>`. The same-path list
observer updates, while the operation-viewmodel operand stays on C++'s `0.0`
fallback. The contract is
...
Current #12 update: owned runtime artboard source mutation now preserves the
`DataConverterOperationViewModel` non-number secondary operand fallback for
direct converters and for
`DataConverterGroup<OperationValue, OperationViewModel>`. Rust exposes
`StateMachineInstance::set_owned_view_model_context_artboard_source_for_data_bind`,
and the C++ probe retains active owned contexts for
`--runtime-set-owned-view-model-source-artboard`. The same-path artboard
observer updates, while the operation-viewmodel operand stays on C++'s `0.0`
fallback. The contract is
`docs/prototypes/data-binding-graph-operation-viewmodel-owned-artboard-mutation-runtime-contract.md`.

Current #12 update: owned runtime view-model pointer source mutation now
preserves the `DataConverterOperationViewModel` non-number secondary operand
fallback for direct converters and for
`DataConverterGroup<OperationValue, OperationViewModel>`. Rust exposes
`StateMachineInstance::set_owned_view_model_context_view_model_source_for_data_bind`,
and the C++ probe retains active owned contexts for
`--runtime-set-owned-view-model-source-viewmodel`. The same-path view-model
observer updates to the selected referenced instance, while the
operation-viewmodel operand stays on C++'s `0.0` fallback. The contract is
`docs/prototypes/data-binding-graph-operation-viewmodel-owned-viewmodel-mutation-runtime-contract.md`.
Longer and exotic converter group permutations, relative/name converter paths,
owned-context source mutation APIs beyond root number, symbol-list-index,
boolean, string, color, enum, trigger, list, asset, artboard, and view-model
pointer OperationViewModel operands,
mutation-driven recompute for other converter families, broader dirty/update
queues, listener-owned data binding, and nested artboard propagation remain
follow-up `#12` slices.
