# Operation ViewModel Owned Trigger Mutation Runtime Contract

Purpose: pin owned-context trigger source mutation for
...
- A same-path trigger observer bind updates alongside the mutation so the
  source mutation itself is still observable.

## Covered Cases

- Direct `DataConverterOperationViewModel` with an owned trigger secondary
  source mutation after owned context binding.
- Grouped `DataConverterGroup<OperationValue, OperationViewModel>` with the
  same owned trigger secondary source mutation after owned context binding.

## Out Of Scope

- Number, symbol-list-index, boolean, string, color, and enum owned mutation,
  covered by their own contracts.
- Asset, artboard, list, or view-model OperationViewModel secondary operand
  mutation.
...
- Focused C++ probe tests:
  - `operation_viewmodel_owned_trigger_source_mutation_fallback_matches_cpp_probe`
  - `operation_viewmodel_group_owned_trigger_source_mutation_fallback_matches_cpp_probe`
