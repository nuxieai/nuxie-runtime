...
fn main_1() {
    ...
    let _e93 = X1_1;
    let _e94 = textureSample(JC, U5_, _e93);
    phi_1250_ = 1f;
    if fh {
        let _e95 = M0_1;
    ...
    if (_e189 <= 1u) {
        ...
        let _e224 = phi_1202_;
        let _e233 = textureSampleLevel(MD, Ob, vec2<f32>(((clamp(_e224, 0f, 1f) * _e216.z) + _e216.w), bitcast<f32>(_e116.y)), 0f);
        phi_1229_ = 0u;
        phi_1227_ = _e233;
    }
    let _e235 = phi_1229_;
...
@fragment
fn main(@builtin(position) gl_FragCoord: vec4<f32>, @location(0) X1_: vec2<f32>, @location(1) M0_: vec4<f32>, @location(4) @interpolate(flat, either) x3_: u32, @location(3) @interpolate(flat, either) H1_: f32, @location(5) @interpolate(flat, either) A1_: u32) -> @location(0) vec4<f32> {
