...
fn main_1() {
    ...
    var phi_1143_: f32;
    var phi_1167_: u32;
    var phi_1165_: vec4<f32>;
    var phi_1178_: vec3<f32>;
    ...
    if ((_e117.x & 768u) != 0u) {
        ...
        phi_796_ = dh;
        if dh {
            phi_796_ = ((_e117.x & 512u) != 0u);
        }
        let _e125 = phi_796_;
        phi_1138_ = _e121;
        if _e125 {
            phi_1138_ = (1f - abs(((fract((_e121 * 0.5f)) * 2f) + -1f)));
        }
        let _e133 = phi_1138_;
        phi_1137_ = _e133;
    }
    ...
    if Zg {
        ...
        if (_e138 != 0u) {
            ...
            if (_e138 == (_e142 >> bitcast<u32>(16i))) {
                phi_1139_ = min(_e136, unpack2x16float(_e142).x);
            } else {
                phi_1139_ = 0f;
            }
            let _e150 = phi_1139_;
    ...
    if _e158 {
        ...
        phi_1144_ = min(_e154, clamp((min(_e181.x, _e181.y) + 0.5f), 0f, 1f));
    }
    let _e189 = phi_1144_;
    let _e190 = (_e117.x & 15u);
    if (_e190 <= 1u) {
        let _e195 = (Zg && (_e190 == 0u));
        phi_1168_ = 0u;
        if _e195 {
            phi_1168_ = (_e117.y | pack2x16float(vec2<f32>(_e189, 0f)));
        }
        let _e200 = phi_1168_;
        phi_1167_ = _e200;
        phi_1165_ = select(unpack4x8unorm(_e117.y), vec4<f32>(0f, 0f, 0f, 0f), vec4(_e195));
    } else {
        let _e203 = (_e97 * 4u);
        ...
        if (_e190 == 2u) {
            ...
            phi_1143_ = length(_e220);
        }
        let _e225 = phi_1143_;
        let _e234 = textureSampleLevel(LD, Mb, vec2<f32>(((clamp(_e225, 0f, 1f) * _e217.z) + _e217.w), bitcast<f32>(_e117.y)), 0f);
        phi_1167_ = 0u;
        phi_1165_ = _e234;
    }
    let _e236 = phi_1167_;
    ...
    let _e251 = n.A3_;
    if (gh && (_e240 != 0f)) {
        phi_1178_ = (vec3(((fract((52.982918f * fract(((0.06711056f * _e57.x) + (0.00583715f * _e57.y))))) * _e249) + _e251)) + _e247);
    } else {
        phi_1178_ = _e247;
    }
    let _e267 = phi_1178_;
