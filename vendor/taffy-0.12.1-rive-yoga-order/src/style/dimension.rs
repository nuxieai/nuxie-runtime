...
impl Dimension {
    ...
    ///
    /// This adapter retains the authored percentage so resolution can mirror
    /// Yoga's `value * owner_size * 0.01` operation order exactly.
    #[inline(always)]
    pub const fn rive_yoga_percent(val: f32) -> Self {
        Self(CompactLength::rive_yoga_percent(val))
    }
