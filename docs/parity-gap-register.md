...
| F6 | **Semantics/accessibility** — `semantic_manager` 1,109, `semantic_data` 572, provider, inference registry | 1,926 | PARTIAL (LT1SC) | The retained runtime and LT-1 full diff/action/focus side channel are implemented against `4ac7b327`. The nested focus provider oracle is exact. The action fixture's draw stream emits a Rust radial gradient where C++ creates render paint; the Simpsons runner stops at unsupported `layout-component-paint` global 347; and `solo_index_test` reports Rust root bounds `(-44,-44,437,341)` vs C++ `(333,263,437,341)`. Those three corpus rows carry the concrete blockers. |
