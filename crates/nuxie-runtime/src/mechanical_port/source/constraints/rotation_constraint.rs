    ...
    pub fn constrain(&mut self, component: &mut TransformComponent) {
        ...
        if self.base.target().is_none() {
            ...
            if !self.base.does_copy() {
                self.components_b.set_rotation(if self.base.dest_space() == TransformSpace::Local { 0.0 } else { self.components_a.rotation() });
            } else {
                self.components_b.set_rotation(self.components_b.rotation() * self.base.copy_factor());
                if self.base.offset() { self.components_b.set_rotation(self.components_b.rotation() + component.rotation()); }
            }
        ...
        if self.base.max() && self.components_b.rotation() > self.base.max_value() { self.components_b.set_rotation(self.base.max_value()); }
        if self.base.min() && self.components_b.rotation() < self.base.min_value() { self.components_b.set_rotation(self.base.min_value()); }
        if clamp_local {
            transform_b = Mat2D::compose(self.components_b);
            transform_b = *get_parent_world(component) * transform_b;
            self.components_b = transform_b.decompose();
        ...
        self.components_b.set_rotation(self.components_a.rotation() + diff * self.base.strength());
