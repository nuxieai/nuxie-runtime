struct Ne {
    d2_: array<vec2<u32>>,
}

struct h0Ed {
    d2_: array<u32>,
}

struct Oe {
    d2_: array<vec4<f32>>,
}

struct DC {
    hc: f32,
    rd: f32,
    kf: f32,
    lf: f32,
    p6_: u32,
    Mg: u32,
    Ve: u32,
    We: u32,
    U7_: vec4<i32>,
    Ig: vec2<f32>,
    sd: vec2<f32>,
    c2_: u32,
    Ng: f32,
    d6_: u32,
    R2_: f32,
    td: f32,
    Qe: u32,
    B3_: f32,
    C3_: f32,
    ud: f32,
    Fg: u32,
}

struct v4Ed {
    d2_: array<u32>,
}

@id(7) override mh: bool = true;
@id(4) override jh: bool = true;
@id(0) override fh: bool = true;
@id(1) override gh: bool = true;

@group(0) @binding(3)
var<storage> BD: Ne;
@group(2) @binding(1)
var<storage, read_write> h0_: h0Ed;
@group(0) @binding(4)
var<storage> RB: Oe;
var<private> gl_FragCoord_1: vec4<f32>;
@group(0) @binding(8)
var MD: texture_2d<f32>;
@group(3) @binding(8)
var Pb: sampler;
@group(0) @binding(0)
var<uniform> m: DC;
@group(2) @binding(3)
var<storage, read_write> v4_: v4Ed;
var<private> B0_1: u32;
var<private> i1_1: f32;
var<private> C1_: vec4<f32>;
@group(3) @binding(9)
var aa: sampler;
@group(0) @binding(9)
var YC: texture_2d<f32>;
@group(1) @binding(11)
var JC: texture_2d<f32>;
@group(1) @binding(13)
var V5_: sampler;

fn main_1() {
    var phi_1130_: u32;
    var phi_791_: bool;
    var phi_1135_: f32;
    var phi_1134_: f32;
    var phi_1136_: f32;
    var phi_1139_: f32;
    var phi_1138_: f32;
    var phi_828_: bool;
    var phi_1141_: f32;
    var phi_1167_: u32;
    var phi_1140_: f32;
    var phi_1166_: u32;
    var phi_1164_: vec4<f32>;
    var phi_1181_: u32;
    var phi_1177_: vec4<f32>;
    var phi_1178_: vec3<f32>;

    let _e55 = gl_FragCoord_1;
    let _e56 = _e55.xy;
    let _e59 = bitcast<vec2<u32>>(vec2<i32>(floor(_e56)));
    let _e61 = m.p6_;
    let _e90 = bitcast<i32>((((((_e59.y >> bitcast<u32>(5u)) * (((_e61 + 31u) & 4294967264u) << bitcast<u32>(5u))) + ((_e59.x >> bitcast<u32>(5u)) << bitcast<u32>(10u))) + (((_e59.x & 28u) << bitcast<u32>(5u)) + ((_e59.y & 28u) << bitcast<u32>(2i)))) + (((_e59.y & 3u) << bitcast<u32>(2i)) + (_e59.x & 3u))));
    let _e93 = v4_.d2_[_e90];
    let _e95 = (_e93 >> bitcast<u32>(17u));
    let _e96 = B0_1;
    if (_e95 == _e96) {
        phi_1130_ = _e93;
    } else {
        phi_1130_ = ((_e96 << bitcast<u32>(17u)) + 65536u);
    }
    let _e102 = phi_1130_;
    let _e103 = i1_1;
    v4_.d2_[_e90] = (_e102 + bitcast<u32>(i32(round((_e103 * 2048f)))));
    phi_1181_ = 0u;
    phi_1177_ = vec4<f32>(0f, 0f, 0f, 0f);
    if (_e95 != _e96) {
        let _e113 = ((f32((_e93 & 131071u)) * 0.00048828125f) + -32f);
        let _e116 = BD.d2_[_e95];
        phi_1134_ = _e113;
        if ((_e116.x & 768u) != 0u) {
            let _e120 = abs(_e113);
            phi_791_ = jh;
            if jh {
                phi_791_ = ((_e116.x & 512u) != 0u);
            }
            let _e124 = phi_791_;
            phi_1135_ = _e120;
            if _e124 {
                phi_1135_ = (1f - abs(((fract((_e120 * 0.5f)) * 2f) + -1f)));
            }
            let _e132 = phi_1135_;
            phi_1134_ = _e132;
        }
        let _e134 = phi_1134_;
        let _e135 = clamp(_e134, 0f, 1f);
        phi_1138_ = _e135;
        if fh {
            let _e137 = (_e116.x >> bitcast<u32>(16u));
            phi_1139_ = _e135;
            if (_e137 != 0u) {
                let _e141 = h0_.d2_[_e90];
                if (_e137 == (_e141 >> bitcast<u32>(16i))) {
                    phi_1136_ = min(_e135, unpack2x16float(_e141).x);
                } else {
                    phi_1136_ = 0f;
                }
                let _e149 = phi_1136_;
                phi_1139_ = _e149;
            }
            let _e151 = phi_1139_;
            phi_1138_ = _e151;
        }
        let _e153 = phi_1138_;
        phi_828_ = gh;
        if gh {
            phi_828_ = ((_e116.x & 1024u) != 0u);
        }
        let _e157 = phi_828_;
        phi_1141_ = _e153;
        if _e157 {
            let _e158 = (_e95 * 8u);
            let _e162 = RB.d2_[(_e158 + 2u)];
            let _e173 = RB.d2_[(_e158 + 3u)];
            let _e178 = _e173.zw;
            let _e180 = ((abs(((mat2x2<f32>(vec2<f32>(_e162.x, _e162.y), vec2<f32>(_e162.z, _e162.w)) * _e56) + _e173.xy)) * _e178) - _e178);
            phi_1141_ = min(_e153, clamp((min(_e180.x, _e180.y) + 0.5f), 0f, 1f));
        }
        let _e188 = phi_1141_;
        let _e189 = (_e116.x & 15u);
        if (_e189 <= 1u) {
            let _e194 = (fh && (_e189 == 0u));
            phi_1167_ = 0u;
            if _e194 {
                phi_1167_ = (_e116.y | pack2x16float(vec2<f32>(_e188, 0f)));
            }
            let _e199 = phi_1167_;
            phi_1166_ = _e199;
            phi_1164_ = select(unpack4x8unorm(_e116.y), vec4<f32>(0f, 0f, 0f, 0f), vec4(_e194));
        } else {
            let _e202 = (_e95 * 8u);
            let _e205 = RB.d2_[_e202];
            let _e216 = RB.d2_[(_e202 + 1u)];
            let _e219 = ((mat2x2<f32>(vec2<f32>(_e205.x, _e205.y), vec2<f32>(_e205.z, _e205.w)) * _e56) + _e216.xy);
            if (_e189 == 2u) {
                phi_1140_ = _e219.x;
            } else {
                phi_1140_ = length(_e219);
            }
            let _e224 = phi_1140_;
            let _e233 = textureSampleLevel(MD, Pb, vec2<f32>(((clamp(_e224, 0f, 1f) * _e216.z) + _e216.w), bitcast<f32>(_e116.y)), 0f);
            phi_1166_ = 0u;
            phi_1164_ = _e233;
        }
        let _e235 = phi_1166_;
        let _e237 = phi_1164_;
        let _e239 = (_e237.w * _e188);
        let _e241 = (_e237.xyz * _e239);
        phi_1181_ = _e235;
        phi_1177_ = vec4<f32>(_e241.x, _e241.y, _e241.z, _e239);
    }
    let _e247 = phi_1181_;
    let _e249 = phi_1177_;
    let _e250 = _e249.xyz;
    let _e253 = m.B3_;
    let _e255 = m.C3_;
    if (mh && (_e249.w != 0f)) {
        phi_1178_ = (vec3(((fract((52.982918f * fract(((0.06711056f * _e55.x) + (0.00583715f * _e55.y))))) * _e253) + _e255)) + _e250);
    } else {
        phi_1178_ = _e250;
    }
    let _e271 = phi_1178_;
    let _e277 = vec4<f32>(_e271.x, _e249.y, _e249.z, _e249.w);
    let _e283 = vec4<f32>(_e277.x, _e271.y, _e277.z, _e277.w);
    C1_ = vec4<f32>(_e283.x, _e283.y, _e271.z, _e283.w);
    if (_e247 != 0u) {
        h0_.d2_[_e90] = _e247;
    }
    return;
}

@fragment
fn main(@builtin(position) gl_FragCoord: vec4<f32>, @location(1) @interpolate(flat, either) B0_: u32, @location(0) @interpolate(flat, either) i1_: f32) -> @location(0) vec4<f32> {
    gl_FragCoord_1 = gl_FragCoord;
    B0_1 = B0_;
    i1_1 = i1_;
    main_1();
    let _e7 = C1_;
    return _e7;
}
