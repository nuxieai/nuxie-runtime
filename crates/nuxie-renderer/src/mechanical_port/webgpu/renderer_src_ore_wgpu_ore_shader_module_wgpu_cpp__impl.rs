...
//! include-only `renderer/src/ore/wgpu/ore_shader_module_wgpu.cpp` owner.

pub(crate) const PINNED_SOURCE: &str =
    include_str!("source/renderer_src_ore_wgpu_ore_shader_module_wgpu.cpp");
pub(crate) const SOURCE_INCLUDE_COUNT: usize = 1;
pub(crate) const SOURCE_OUT_OF_LINE_DEFINITION_COUNT: usize = 0;
const _: [(); 69] = [(); PINNED_SOURCE.len()];
...
mod tests {
    ...
    fn include_only_translation_unit_is_preserved() {
        ...
        assert!(PINNED_SOURCE.contains("#include \"ore_shader_module_wgpu.hpp\""));
