# Data Binding Graph StringPad Main-To-Target Two-Way Target Dirty Runtime Contract

## Purpose

Pin C++ state-machine target-dirty behavior for direct
`DataConverterStringPad` string-to-string binds on main-`ToTarget | TwoWay`
data binds.
...
- Direct `DataConverterStringPad` conversion through imported `length`, `text`,
...
- Subsequent normal state-machine advancement overwriting the string target
  from the unchanged source through C++ pad conversion.
...
- A `TwoWay` direct string-pad bind applies the padded source before target
  mutation.
...
- Normal state-machine advancement after the explicit data-context step
  overwrites the target from the unchanged source through C++ pad conversion,
