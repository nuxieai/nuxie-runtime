...
/// Factory-late realization of `SliceMesh::update` for every live NSlicer.
/// The C++ owner can calculate and upload immediately; Rust retains the exact
/// update event on the occurrence until the render image and backend context
/// are both available.
...
pub(super) fn runtime_prepare_slice_meshes(
    ...
    for details in graph.n_slicer_details.iter().filter(|details| {
        ...
        let Some(image_local) = super::n_slicer::runtime_nslicer_image_local(instance, details)
        else {
            continue;
        };
