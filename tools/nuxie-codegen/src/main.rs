...
fn main() -> Result<()> {
    ...
    let schema = Schema::load_with_extensions(&args.defs, &args.extensions)?;
    ...
    fn parse(raw: Vec<String>) -> Result<Self> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                "--extensions" => {
                    extensions =
                        PathBuf::from(args.next().context("--extensions requires a path")?);
                }
                "--defs" => {
                ...
                "--help" | "-h" => {
                    println!(
                        "usage: nuxie-codegen --defs <defs-dir> --out <schema.rs> [--extensions <nuxie-defs-dir>]"
                    );
                    std::process::exit(0);
                }
...
impl Schema {
    fn load_with_extensions(upstream: &Path, extensions: &Path) -> Result<Self> {
        let mut schema = Self::load(upstream)?;
        schema.validate_namespace(false)?;
        let mut files = Vec::new();
        collect_json_files(extensions, extensions, &mut files)?;
        let mut owned = Self {
            defs: BTreeMap::new(),
        };
        for relative in files {
            let path = extensions.join(&relative);
            let mut def: RawDefinition = serde_json::from_str(&fs::read_to_string(&path)?)
                .with_context(|| format!("parsing Nuxie definition {}", path.display()))?;
            let file = format!("nuxie/{relative}");
            def.file = file.clone();
            owned.defs.insert(file, def);
        }
        owned.validate_namespace(true)?;
        for (file, def) in owned.defs {
            if schema.defs.insert(file.clone(), def).is_some() {
                bail!("extension definition shadows upstream file {file}");
            }
        }
    ...
    fn validate_namespace(&self, extension: bool) -> Result<()> {
        for (file, def) in &self.defs {
            let keys = def
                .key
                .iter()
                .chain(def.properties.values().filter_map(|p| p.key.as_ref()));
            for key in keys {
                for value in key
                    .int
                    .into_iter()
                    .chain(key.alternates.iter().filter_map(|k| k.int))
                {
                    if extension && !(60000..=65535).contains(&value) {
                        bail!("{file}: Nuxie key {value} must be in 60000..=65535");
                    }
                    if !extension && value >= 60000 {
                        bail!("{file}: upstream key {value} overlaps the reserved Nuxie band");
                    }
                }
            }
        }
    ...
    fn render(&self) -> Result<String> {
        let mut defs = self
            ...
            .map(|(file, def)| {
                ...
                Ok(RuntimeDefinition {
                    ...
                    mixins: def.mixin.iter().chain(&def.mixins).cloned().collect(),
                    runtime_mixins: self.runtime_mixin_names(def),
        ...
        // Runtime mixins generate interface metadata, not ObjectKind tags or
        // standalone Core definitions (definition.dart::_generateMixinInterfaceHeader).
        let (mut mixins, mut defs): (Vec<_>, Vec<_>) = defs.drain(..).partition(|def| def.is_mixin);
        defs.sort_by_key(|def| def.key);
        mixins.sort_by_key(|def| def.key);
        validate_variants(&defs)?;

        let mut out = String::new();
        out.push_str("// @generated by `nuxie-codegen`; do not edit by hand.\n");
        out.push_str("// Source: Rive runtime `dev/defs` and Nuxie `defs/nuxie` JSON.\n\n");
        out.push_str(
            "use crate::{BitmaskPassthrough, CoreRegistryFieldKind, Definition, FieldKind, Key, Property, RuntimeMixin};\n\n",
        );
        ...
        out.push_str(&self.render_core_registry_lookup_tables(&defs, &mixins)?);
        ...
        out.push_str(&self.render_runtime_mixins(&defs, &mixins)?);
        out.push_str(&self.render_hierarchy_property_lookup_table(&defs, &mixins)?);
        out.push_str(&self.render_bitmask_passthrough_lookup_table(&defs, &mixins)?);
    ...
    fn runtime_mixin_names(&self, def: &RawDefinition) -> Vec<String> {
        def.mixin
            .iter()
            .chain(&def.mixins)
            // Editor-only mixin files are absent from the runtime defs tree.
            .filter_map(|file| self.defs.get(file))
            .filter(|mixin| mixin.runtime && mixin.is_mixin)
            .filter_map(|mixin| mixin.name.clone())
            .collect()
    }

    fn bitmask_passthrough<'a>(
        &self,
        property: RawPropertyEntry<'a>,
        def_file: &str,
    ) -> Option<RenderedBitmask<'a>> {
        let mut bitmask = property.raw.bitmask_passthrough()?;
        let def = self.defs.get(def_file)?;
        bitmask.host_provided = def.is_mixin
            && !def
                .runtime_properties()
                .any(|candidate| candidate.name == bitmask.target);
        Some(bitmask)
    }

    fn render_runtime_mixins(
        &self,
        defs: &[RuntimeDefinition<'_>],
        mixins: &[RuntimeDefinition<'_>],
    ) -> Result<String> {
        let mut out = String::new();
        for (index, mixin) in mixins.iter().enumerate() {
            out.push_str(&format!(
                "static MIXIN_{index}_PROPERTIES: &[Property] = &[\n"
            ));
            for property in sorted_runtime_properties(mixin) {
                out.push_str(&self.render_property(&property, &mixin.file)?);
            }
            out.push_str("];\n\n");
        }
        out.push_str("pub static RUNTIME_MIXINS: &[RuntimeMixin] = &[\n");
        for (index, mixin) in mixins.iter().enumerate() {
            out.push_str(&format!(
                "    RuntimeMixin {{ name: {}, file: {}, properties: MIXIN_{index}_PROPERTIES }},\n",
                rust_string(&mixin.name), rust_string(&mixin.file),
            ));
        }
        out.push_str("];\n\n");
        out.push_str(
        ...
        for (index, mixin) in mixins.iter().enumerate() {
            out.push_str(&format!(
                "        {} => Some(&RUNTIME_MIXINS[{index}]),\n",
                rust_string(&mixin.name)
            ));
        }
        out.push_str("        _ => None,\n    }\n}\n\n");
        ...
        for (definition_index, def) in defs.iter().enumerate() {
            let interfaces = def
                ...
                .filter_map(|name| {
                    mixins
                        .iter()
                        .position(|mixin| &mixin.name == name)
                        .map(|index| format!("&RUNTIME_MIXINS[{index}]"))
                })
                .collect::<Vec<_>>();
            if !interfaces.is_empty() {
                out.push_str(&format!(
                    "static DEF_{definition_index}_RUNTIME_MIXINS: &[&RuntimeMixin] = &[{}];\n\n",
                    interfaces.join(", ")
                ));
                consumers.push((def.key, definition_index));
            }
        }
        out.push_str(
            "pub fn runtime_mixins_for_type_key(key: u16) -> &'static [&'static RuntimeMixin] {\n",
        );
        out.push_str("    match key {\n");
        for (type_key, definition_index) in consumers {
            out.push_str(&format!(
                "        {type_key} => DEF_{definition_index}_RUNTIME_MIXINS,\n",
            ));
        }
        out.push_str("        _ => &[],\n    }\n}\n\n");
        Ok(out)
    ...
    fn render_core_registry_lookup_tables(
        &self,
        defs: &[RuntimeDefinition<'_>],
        mixins: &[RuntimeDefinition<'_>],
    ) -> Result<String> {
        let mut first_properties = first_properties_by_key(defs);
        for (key, property) in first_properties_by_key(mixins) {
            first_properties.entry(key).or_insert(property);
        }
        let mut out = String::new();
    ...
    fn render_hierarchy_property_lookup_table(
        ...
        // Mixin resolver dispatch is over exact consuming coreType values;
        // it does not make an interface a Core ancestor or copy its properties.
        for def in defs {
            for (mixin_index, mixin) in mixins.iter().enumerate() {
                if !def.runtime_mixins.contains(&mixin.name) {
                    continue;
                }
                for (property_index, property) in
                    sorted_runtime_properties(mixin).into_iter().enumerate()
                {
                    for property_key in generated_property_keys(property) {
                        out.push_str(&format!(
                            "        ({}, {property_key}) => Some(({}, &MIXIN_{mixin_index}_PROPERTIES[{property_index}])),\n",
                            def.key,
                            rust_string(&mixin.name),
                        ));
                    }
    ...
    fn render_bitmask_passthrough_lookup_table(
        ...
        for def in defs {
            ...
            for owner_index in owner_indices {
                for property in properties_by_definition[owner_index].iter().copied() {
                    let Some(bitmask) = self.bitmask_passthrough(property, &defs[owner_index].file)
                    else {
                        continue;
                    };
                    for property_key in generated_property_keys(property) {
            ...
            for mixin in mixins {
                ...
                for property in sorted_runtime_properties(mixin) {
                    if let Some(bitmask) = self.bitmask_passthrough(property, &mixin.file) {
                        for property_key in generated_property_keys(property) {
                            entries.entry((def.key, property_key)).or_insert(bitmask);
                        }
                    }
        ...
        for ((type_key, property_key), bitmask) in entries {
            out.push_str(&format!(
                "        ({type_key}, {property_key}) => Some(BitmaskPassthrough {{ target: {}, bit: {}, width: {}, host_provided: {} }}),\n",
                rust_string(bitmask.target),
                bitmask.bit,
                bitmask.width,
                bitmask.host_provided,
            ));
        }
        out.push_str("        _ => None,\n");
        out.push_str("    }\n");
    ...
    fn render_property(&self, property: &RawPropertyEntry<'_>, def_file: &str) -> Result<String> {
        ...
        let bitmask = self.bitmask_passthrough(*property, def_file);
        ...
        match bitmask {
            Some(bitmask) => {
                out.push_str(&format!(
                    "Some(BitmaskPassthrough {{ target: {}, bit: {}, width: {}, host_provided: {} }}),\n",
                    rust_string(bitmask.target),
                    bitmask.bit,
                    bitmask.width,
                    bitmask.host_provided,
        ...
        for (file, def) in self.runtime_definitions() {
            ...
            for property in &properties {
                ...
                let Some(target) = target else {
                    if def.is_mixin {
                        // The host supplies this mask. Source overlap checking
                        // only applies when a same-def target was resolved.
                        continue;
                    }
                    bail!(
...
#[cfg(test)]
mod extension_namespace_tests {
    use super::*;

    fn schema(json: &str) -> Schema {
        let def = serde_json::from_str(json).unwrap();
        Schema {
            defs: BTreeMap::from([("test.json".into(), def)]),
        }
    }
    #[test]
    fn upstream_types_and_alternate_properties_cannot_enter_owned_band() {
        for json in [
            r#"{"name":"FutureUpstream","key":{"int":60000}}"#,
            r#"{"name":"Ordinary","key":{"int":10},"properties":{"p":{"type":"uint","key":{"int":20,"alternates":[{"int":60020,"string":"old"}]}}}}"#,
        ] {
            assert!(schema(json).validate_namespace(false).is_err());
        }
    }
    #[test]
    fn extensions_cannot_claim_upstream_or_out_of_width_keys() {
        for key in [3, 59999, 65536] {
            let json = format!(r#"{{"name":"Video","key":{{"int":{key}}}}}"#);
            assert!(schema(&json).validate_namespace(true).is_err());
        }
        assert!(
            schema(r#"{"name":"Video","key":{"int":60001}}"#)
                .validate_namespace(true)
                .is_ok()
        );
    }
    #[test]
    fn extension_collision_is_rejected_before_rendering() {
        let def = r#"{"name":"Video","key":{"int":60001}}"#;
        let mut s = schema(def);
        s.defs
            .insert("other.json".into(), serde_json::from_str(def).unwrap());
        assert!(s.validate_definition_keys().is_err());
    }
}
