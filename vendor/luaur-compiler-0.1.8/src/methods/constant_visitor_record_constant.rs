...
impl<'a> ConstantVisitor<'a> {
    pub fn record_constant<T>(
        ...
        T: Clone
            + Eq
            + core::hash::Hash
            + luaur_common::records::dense_hash_table::DenseDefault
            + Default
            + 'static,
    {
            ...
            + 'static,
    {
        ...
        if let Some(expr_key) = key_any.downcast_ref::<*mut AstExpr>() {
            let m = unsafe {
                core::mem::transmute::<
                    &mut DenseHashMap<T, Constant>,
                    &mut DenseHashMap<*mut AstExpr, Constant>,
                >(map)
            };
            self.log_change_dense_hash_map_ast_expr_constant_ast_expr_constant(
                m, *expr_key, existing,
            );
            ...
            let m = unsafe {
                core::mem::transmute::<
                    &mut DenseHashMap<T, Constant>,
                    &mut DenseHashMap<*mut AstLocal, Constant>,
                >(map)
            };
            self.log_change_dense_hash_map_ast_local_constant_ast_local_constant(
                m, *local_key, existing,
            );
        }
    }
}
