...
- `post_build_dependencies`: project C++ initialized `m_DrawTargets` as static `draw_target_dependency_edges`, `draw_target_order`, and `draw_target_cycles`, including parent-ordered draw-rule groups, synthetic root target dependencies for resolved target drawables, and target-to-target dependencies from each target drawable's flattened rules. Active target linked-list mutation, placement splicing, clipping-stack operations, renderer commands, and GPU work remain out of scope.
