...
const pagePath = path.join(repo, "tools/webgl2-renderer-replay/index.html");
const jsPath = path.join(
  repo,
  "tools/webgl2-renderer-replay/pkg/webgl2_renderer_replay.js",
);
const wasmPath = path.join(
  repo,
  "tools/webgl2-renderer-replay/pkg/webgl2_renderer_replay_bg.wasm",
);
...
const pageUrl = `http://127.0.0.1:${port}/tools/webgl2-renderer-replay/index.html`;
...
async function loadCandidatePage() {
  ...
  await page.evaluate(() => window.webgl2CandidateReady);
  rendersSinceReload = 0;
}

async function render(payload) {
  if (!page) throw new Error("browser WebGL2 candidate is not ready");
  if (!payload || payload.backend !== "rust-webgl2-exact") {
    throw new Error("candidate only accepts backend rust-webgl2-exact");
  }
  ...
  if (rendersSinceReload >= 16) await loadCandidatePage();
  const browserErrors = [];
  ...
  try {
    const result = await page.evaluate(
      async ({ stream, mode, frame }) =>
        window.runWebgl2Candidate({ stream, mode, frame }),
