...
#[test]
fn public_gradients_record_original_and_clone_resize_frames() {
    let oracle: serde_json::Value =
        serde_json::from_str(include_str!("assets/linear-gradient-initial-oracle.json")).unwrap();
    assert_eq!(oracle["browser"], "153.0.8010.12");
    let recording =
        std::env::var_os("NUXIE_GRADIENT_PUBLIC_RECORD_DIR").map(std::path::PathBuf::from);
