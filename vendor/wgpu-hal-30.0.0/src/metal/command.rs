...
impl crate::CommandEncoder for super::CommandEncoder {
    ...
    fn can_coalesce_render_pass_command_buffers(&self) -> bool {
        // Metal resource transitions are implicit, so a pre-pass-free render
        // body can remain in the current MTLCommandBuffer. Strict event sync
        // is the exception: begin_encoding must add the relay wait prologue to
        // every command buffer, and continuing an older command buffer would
        // bypass that prologue for the appended body.
        //
        // The relay lives in a OnceLock and can only transition from absent to
        // present. Querying it for every candidate pass therefore refuses
        // continuation after strict sync is enabled, including when an older
        // encoder is already open. A concurrent enable immediately after this
        // check remains part of the queue API's pre-existing requirement that
        // callers coordinate strict-sync enablement with command encoding.
        super::render_pass_command_buffer_coalescing_allowed(
            self.queue_shared.relay.get().is_some(),
        )
    }

    fn can_discard_transition_only_command_buffer(&self) -> bool {
        // Metal resource transitions are implicit. An otherwise empty Transit
        // buffer has no native work unless strict event sync installed a wait
        // prologue in `begin_encoding`.
        super::transition_only_command_buffer_discard_allowed(
            self.queue_shared.relay.get().is_some(),
        )
    }
