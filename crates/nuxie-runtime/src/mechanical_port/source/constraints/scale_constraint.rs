    ...
    pub fn constrain(&mut self, component: &mut TransformComponent) {
        ...
        if self.base.target().is_none() {
            ...
            if !self.base.does_copy() {
                self.components_b.set_scale_x(if self.base.dest_space() == TransformSpace::Local { 1.0 } else { self.components_a.scale_x() });
            } else {
                self.components_b.set_scale_x(self.components_b.scale_x() * self.base.copy_factor());
                if self.base.offset() { self.components_b.set_scale_x(self.components_b.scale_x() * component.scale_x()); }
            }
            if !self.base.does_copy_y() {
                self.components_b.set_scale_y(if self.base.dest_space() == TransformSpace::Local { 1.0 } else { self.components_a.scale_y() });
            } else {
                self.components_b.set_scale_y(self.components_b.scale_y() * self.base.copy_factor_y());
                if self.base.offset() { self.components_b.set_scale_y(self.components_b.scale_y() * component.scale_y()); }
            }
        ...
        if self.base.max() && self.components_b.scale_x() > self.base.max_value() { self.components_b.set_scale_x(self.base.max_value()); }
        if self.base.min() && self.components_b.scale_x() < self.base.min_value() { self.components_b.set_scale_x(self.base.min_value()); }
        if self.base.max_y() && self.components_b.scale_y() > self.base.max_value_y() { self.components_b.set_scale_y(self.base.max_value_y()); }
        if self.base.min_y() && self.components_b.scale_y() < self.base.min_value_y() { self.components_b.set_scale_y(self.base.min_value_y()); }
        if clamp_local {
            transform_b = Mat2D::compose(self.components_b);
            transform_b = *get_parent_world(component) * transform_b;
            self.components_b = transform_b.decompose();
        ...
        self.components_b.set_scale_x(self.components_a.scale_x() * ti + self.components_b.scale_x() * t);
        self.components_b.set_scale_y(self.components_a.scale_y() * ti + self.components_b.scale_y() * t);
