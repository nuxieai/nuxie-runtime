...
fn main_1() {
    ...
    var phi_1004_: f32;
    var phi_992_: f32;
    var phi_1006_: vec4<f32>;
    var phi_1008_: vec3<f32>;
    ...
    if ((_e98.x & 768u) != 0u) {
        ...
        phi_681_ = dh;
        if dh {
            phi_681_ = ((_e98.x & 512u) != 0u);
        }
        let _e106 = phi_681_;
        phi_987_ = _e102;
        if _e106 {
            phi_987_ = (1f - abs(((fract((_e102 * 0.5f)) * 2f) + -1f)));
        }
        let _e114 = phi_987_;
        phi_986_ = _e114;
    }
    ...
    if Zg {
        ...
        if (_e119 != 0u) {
            ...
            if (_e119 == (_e123 >> bitcast<u32>(16i))) {
                phi_988_ = min(_e117, unpack2x16float(_e123).x);
            } else {
                phi_988_ = 0f;
            }
            let _e131 = phi_988_;
    ...
    if _e139 {
        ...
        phi_1004_ = min(_e135, clamp((min(_e162.x, _e162.y) + 0.5f), 0f, 1f));
    }
    let _e170 = phi_1004_;
    let _e171 = (_e98.x & 15u);
    if (_e171 <= 1u) {
        phi_1006_ = select(unpack4x8unorm(_e98.y), vec4<f32>(0f, 0f, 0f, 0f), vec4((Zg && (_e171 == 0u))));
    } else {
        let _e179 = (_e95 * 4u);
        ...
        if (_e171 == 2u) {
            ...
            phi_992_ = length(_e196);
        }
        let _e201 = phi_992_;
        let _e210 = textureSampleLevel(LD, Mb, vec2<f32>(((clamp(_e201, 0f, 1f) * _e193.z) + _e193.w), bitcast<f32>(_e98.y)), 0f);
        phi_1006_ = _e210;
    }
    let _e212 = phi_1006_;
    ...
    let _e225 = n.A3_;
    if (gh && (_e214 != 0f)) {
        phi_1008_ = (vec3(((fract((52.982918f * fract(((0.06711056f * _e51.x) + (0.00583715f * _e51.y))))) * _e223) + _e225)) + _e221);
    } else {
        phi_1008_ = _e221;
    }
    let _e241 = phi_1008_;
