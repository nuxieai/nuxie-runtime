...
- Closed Editor-owned supported-WebGPU stale golden: on 2026-07-27 the user
  accepted the reviewed expected image for `LOC-012`. The required-WebGPU
  visual/spacing gate passes 2/2 with the authored `#13253d` background,
  border, and clip-radius details. P19-C08 remains historical linkage only;
  COR-07 retains the non-reproducible historical artifact as optional
  provenance work. No runtime/renderer repair or C++ pixel-parity claim is
...
- closed `RT-ED-004` remains historical-only and retains its exact old
  WebGL2 failure without a repair claim. `LOC-012` is closed by the user's
  acceptance of the evidence-backed Editor stale-golden WebGPU image; COR-07
  remains optional historical provenance and does not block that decision.
  Closed `LOC-013` and `LOC-014` have exact C++/Rust WebGPU-equivalent
  stale-oracle dispositions, and `LOC-006` completed as a no-repair stale
...
### Q3 — Verify the mapped retained-ViewModel family after FL-D
...
- `F-ED-10`: `LOC-012` is closed after the user accepted the qualified
  Editor-owned required-WebGPU golden on 2026-07-27. Keep P19-C08 as
  historical linkage, keep the executable fixture qualified, and retain
  COR-07 only for optional historical provenance; make no runtime/renderer
  repair or C++ pixel-parity claim.
  `LOC-014` is independently closed as a stale oracle after the exact same
  180-by-124 typed Feather scene produced zero differing C++/Rust pixels.
