...
fn main_1() {
    ...
    if (_e124 > 0u) {
        let _e131 = FD.d2_[(max(_e124, 1u) - 1u)][2u];
        phi_1774_ = _e131;
    } else {
        phi_1774_ = 0u;
    }
    ...
    if ((_e145 != 0f) && (bitcast<f32>(_e141.z) == 0f)) {
        ...
        let _e343 = local_4;
        let _e345 = local_5;
        let _e355 = textureSampleLevel(YC, aa, vec2<f32>((0.5f * min(min((1f - (_e339 * 0.31830987f)), (((dot(_e343, _e345) / (_e208 * _e208)) - 1f) * 0.5f)), 0.99f)), 1f), 0f);
    ...
    N4_ = vec4<f32>((_e475 - abs((_e93 - _e115.x))), _e475, f32(((_e121 << bitcast<u32>(10i)) | _e417)), _e464);
    let _e485 = VC_1;
    O4_[0u] = _e485.x;
    O4_[1u] = _e485.y;
    if (_e121 > 1u) {
        ...
        if (_e495 == 0f) {
            ...
            phi_1995_ = clamp((dot(_e438, _e491) * inverseSqrt(_e495)), -1f, 1f);
        }
        let _e501 = phi_1995_;
        let _e503 = f32(_e121);
        phi_2003_ = _e503;
        ...
        let _e508 = phi_2003_;
        let _e509 = (acos(_e501) / _e508);
        phi_2004_ = _e509;
        if (determinant(mat2x2<f32>(_e438, _e491)) < 0f) {
            phi_2004_ = -(_e509);
        }
        let _e514 = phi_2004_;
        O4_[2u] = _e514;
