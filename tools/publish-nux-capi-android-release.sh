...
expected_tag="android-runtime-v0.3.6"

if [[ "${1:-}" == "--plan" ]]; then
    printf '%s\n' \
        '1. require clean source commit equal to origin/main' \
        '2. require local and remote android-runtime-v0.3.6 tags at that source commit' \
...
if [[ -z "${release_tag}" || $# -gt 2 ]]; then
    echo "usage: $0 android-runtime-v0.3.6 [artifact-directory] | --plan" >&2
    exit 2
fi
...
gh release create "${release_tag}" \
    ...
    --draft \
    --verify-tag \
    --title "Nuxie Android runtime 0.3.6" \
    --notes-file "${notes}"
