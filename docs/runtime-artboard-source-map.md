...
| `mesh_vertex.hpp`, `mesh_vertex.cpp` | `shapes/mesh_vertex.rs` | authored-order Mesh registration and inherited x/y geometry dirt | partial: relation and property callbacks extracted with schema inheritance; render geometry remains in `draw.rs` |
| `contour_mesh_vertex.hpp` (generated base + behaviorless concrete subclass) | `shapes/mesh_vertex.rs` | inherits `MeshVertex::onAddedDirty` and `markGeometryDirty` | generated/helper boundary: no distinct C++ behavior owner exists, so Rust uses the schema inheritance relation instead of duplicating an empty module |
