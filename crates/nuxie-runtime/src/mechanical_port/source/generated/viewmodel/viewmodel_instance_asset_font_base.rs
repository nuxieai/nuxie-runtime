use crate::mechanical_port::source::viewmodel::viewmodel_instance_asset_font::ViewModelInstanceAssetFont;

use crate::mechanical_port::source::{
    core::binary_reader::BinaryReader, viewmodel::viewmodel_instance_asset::ViewModelInstanceAsset,
};

pub struct ViewModelInstanceAssetFontBase {
    pub base: ViewModelInstanceAsset,
}

impl Default for ViewModelInstanceAssetFontBase {
    fn default() -> Self {
        Self {
            base: ViewModelInstanceAsset::default(),
        }
    }
}

impl ViewModelInstanceAssetFontBase {
    pub const TYPE_KEY: u16 = 1035;

    pub fn is_type_of(type_key: u16) -> bool {
        matches!(type_key, Self::TYPE_KEY | 586 | 428 | 10)
    }
    pub fn core_type(&self) -> u16 {
        Self::TYPE_KEY
    }
    pub fn clone_into(&self) -> ViewModelInstanceAssetFont {
        let mut cloned = ViewModelInstanceAssetFont::default();
        let mut base = std::mem::take(&mut cloned.base);
        base.copy(self, &mut cloned);
        cloned.base = base;
        cloned
    }
}

impl std::ops::Deref for ViewModelInstanceAssetFontBase {
    type Target = ViewModelInstanceAsset;

    fn deref(&self) -> &Self::Target {
        &self.base
    }
}

impl std::ops::DerefMut for ViewModelInstanceAssetFontBase {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.base
    }
}
