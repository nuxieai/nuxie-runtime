use crate::mechanical_port::source::{
    constraints::{
        draggable_constraint::DraggableProxy,
...
impl ThumbDraggableProxy {
    pub fn new(constraint: &mut ScrollBarConstraint, hittable: &mut Drawable) -> Self {
        Self { constraint, hittable, last_position: Vec2D::default() }
    }
}
...
impl DraggableProxy for ThumbDraggableProxy {
    ...
    fn start_drag(&mut self, mouse_position: Vec2D, time_stamp: f32) -> bool {
        ...
        if let Some(physics) = scroll.physics_mut() { physics.accumulate(Vec2D::default(), time_stamp); }
        true
    }
...
pub struct TrackDraggableProxy { constraint: *mut ScrollBarConstraint, hittable: *mut Drawable }
impl TrackDraggableProxy {
    pub fn new(constraint: &mut ScrollBarConstraint, hittable: &mut Drawable) -> Self { Self { constraint, hittable } }
}
impl DraggableProxy for TrackDraggableProxy {
    fn start_drag(&mut self, mouse_position: Vec2D, _time_stamp: f32) -> bool { unsafe { (*self.constraint).hit_track(mouse_position) }; true }
    fn drag(&mut self, _mouse_position: Vec2D, _time_stamp: f32) -> bool { true }
    fn end_drag(&mut self, _mouse_position: Vec2D, _time_stamp: f32) -> bool { true }
    fn hittable(&mut self) -> Option<&mut Drawable> { Some(unsafe { &mut *self.hittable }) }
}
