...
namespace gpu {
namespace glsl {
const char common[] = R"===(#define B3 3.14159265359
...
#endif
e c G0(c x){return x;}e E A2(E xy){return xy;}e E A2(c x,c y){E S;S.x=x,S.y=y;return S;}e E A2(c x){E S;S.x=x,S.y=x;return S;}e d J6(float x){return d(x,x);}e v Q0(c x,c y,c z){v S;S.x=x,S.y=y,S.z=z;return S;}e v Q0(c x){v S;S.x=x,S.y=x,S.z=x;return S;}e i C0(c x,c y,c z,c w){i S;S.x=x,S.y=y,S.z=z,S.w=w;return S;}e i C0(v xyz,c w){i S;S.xyz=xyz;S.w=w;return S;}e i C0(c x){i S;S.x=x,S.y=x,S.z=x,S.w=x;return S;}e i C0(i x){return x;}e D4 kf(bool b){return D4(b,b);}e V6 Th(v m,v b,v G1){V6 S;S[0]=m;S[1]=b;S[2]=G1;return S;}e W6 Uh(v m,v b){W6 S;S[0]=m;S[1]=b;return S;}e h5 Vh(i m,i b,i G1,i lf){h5 S;S[0]=m;S[1]=b;S[2]=G1;S[3]=lf;return S;}e g0 l2(g x){return g0(x.xy,x.zw);}e uint Qb(N x){return x;}e d Y5(d m,d b,float t){return(b-m)*t+m;}e c o8(uint nc,uint Z5){return nc==0u?.0:unpackHalf2x16((nc+mf)*Z5).x;}e float oc(d h2){h2=normalize(h2);float f1=acos(clamp(h2.x,-1.,1.));return h2.y>=.0?f1:-f1;}e i Wh(i j){return C0(j.xyz*j.w,j.w);}e v C6(i ca){return ca.xyz*(ca.w!=.0?1./ca.w:.0);}e c f3(E X6){return min(X6.x,X6.y);}e c f3(v pc){return min(f3(pc.xy),pc.z);}e c f3(i qc){E X6=min(qc.xy,qc.zw);c nf=min(X6.x,X6.y);return nf;}e c J5(E Y6){return max(Y6.x,Y6.y);}e c J5(v rc){return max(J5(rc.xy),rc.z);}e c J5(i sc){E Y6=max(sc.xy,sc.zw);c of=max(Y6.x,Y6.y);return of;}e float E9(d x){return abs(x.x)+abs(x.y);}e c da(c x,c ea,c fa){
#if defined(IF)||defined(DD)
#ifdef DD
if(DD)
#endif
...
#if 0
e c Xh(d K0,float B2,float m3){int x=int(K0.x);int y=int(K0.y);int uc=(x^y);int b=(y>>1)&1;b|=(uc&2);b|=(y&1)<<2;b|=(uc&1)<<3;float rf=float(b);c sf=S4(rf)/16.0;return(sf*B2)+m3;}e c Yh(d K0,float B2,float m3){K0.y*=0.5;K0.x=fract(K0.x*0.5+K0.y);K0.y=fract(K0.y);float N3=(K0.y*0.5+K0.x);return(N3*B2)+m3;}
#endif
#ifdef LB
