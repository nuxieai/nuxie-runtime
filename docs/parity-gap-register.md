...
| V14 | **`artboard_list_overrides`: post-zero list layout height diverges.** Rust clips at height 724 where C++ clips at 1074. | `corpus.toml` milestone V14, samples 0/0.5/1 | Reconcile list override layout propagation and restore `exact`. **2026-08-04 attempt (commit 4f790a3e, reverted): retaining hosted layout draw owners closed this row in the scripted lane but regressed silver transition_duration_bind_list (frame 7, op 742: drawPath became rewind) — the dirt condition must satisfy both oracles; diagnose the two-oracle tension before the next attempt.** **Coupled mounted-layout family with V11 and the per-host fence refinement (patch in c12): the pieces validate only together — partial landings regressed silver transition_duration_bind_list or global_variables_test. Next attempt lands the family atomically against both oracles.** |
