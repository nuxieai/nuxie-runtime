...
- Deterministic `DataConverterFormula` owned enum mutation slice: post-bind
  owned root enum source mutation by state-machine data-bind index now
  refreshes the direct formula fallback number target and a same-path direct
  enum observer. The Rust runtime exposes
  `set_owned_view_model_context_enum_source_for_data_bind`, and the C++ probe
  retains active owned enum contexts plus
  `--runtime-set-owned-view-model-source-enum` for parity comparison.
  Color/string/trigger/list owned mutation, random formulas,
  reverse/public-update/target-dirty scheduling, relative/parent/nested lookup,
  listener-owned data binding, nested artboard propagation, and full
  dirty-list scheduler parity remain follow-up slices. The contract is
  `docs/prototypes/data-binding-graph-formula-owned-enum-mutation-runtime-contract.md`.
- Deterministic `DataConverterFormula` imported/owned color-context slice:
  imported and owned runtime view-model color contexts now feed rebound source
  values into the direct formula fallback path before normal state-machine
  advancement. The fixture includes a same-path direct color observer, so
  imported-context color mutation now fans out to every same-path color source
  node in the active graph. String/trigger and list formula contexts,
  same-path imported fanout for non-boolean/non-enum/non-color source kinds,
  random formulas, reverse/public-update/target-dirty scheduling, generated
  list item identity, relative/parent/nested lookup, listener-owned data
...
- Deterministic `DataConverterFormula` owned string mutation slice: post-bind
  owned root string source mutation by state-machine data-bind index now
  refreshes the direct formula fallback number target and a same-path direct
  string observer. The Rust runtime exposes
  `set_owned_view_model_context_string_source_for_data_bind`, and the C++
  probe retains active owned string contexts plus
  `--runtime-set-owned-view-model-source-string` for parity comparison.
  Trigger/list owned mutation, random formulas, reverse/public-update/
...
- Deterministic `DataConverterFormula` imported/owned trigger-context slice:
  imported and owned runtime view-model trigger contexts now feed rebound
  source values into the direct formula fallback path before normal
  state-machine advancement. The fixture includes a same-path direct trigger
  observer, so imported-context trigger mutation now fans out to every
  same-path trigger source node in the active graph. List formula contexts,
  same-path imported fanout for non-boolean/non-enum/non-color/non-string/
  non-trigger source kinds, random formulas, reverse/public-update/
  target-dirty scheduling, generated list item identity, relative/parent/
...
- `DataConverterFormula` random call-count explicit target-to-source slice:
  ...
  source-change random modes. Grouped explicit target-to-source call counts are
  covered separately below. List/non-number paths, imported/owned contexts,
  secondary dependency invalidation, real RNG generation/seeding, and full
  dirty-list scheduler parity remain follow-up slices. Public-update and
  target-dirty call counts are covered separately below. The contract is
  `docs/prototypes/data-binding-graph-formula-random-target-to-source-call-count-runtime-contract.md`.
- `DataConverterFormula` random grouped call-count explicit target-to-source
  slice: default-context number sources feeding number targets through
...
- `DataConverterFormula` random call-count public-update slice:
  ...
  and source-change random modes. Grouped public-update call counts are covered
  separately below. List/non-number paths, imported/owned contexts, secondary
  dependency invalidation, real RNG generation/seeding, and full dirty-list
  scheduler parity remain follow-up slices. Target-dirty call counts are
  covered separately below. The contract is
  `docs/prototypes/data-binding-graph-formula-random-public-update-call-count-runtime-contract.md`.
- `DataConverterFormula` random grouped call-count public-update slice:
  default-context number sources feeding number targets through
  `DataConverterGroup<OperationValue, Formula(random)>` now expose
  host-supplied random-stream pull counts for public
  `update_data_binds_apply_target_to_source` scheduling in default, always,
...
- `DataConverterFormula` list fallback bindable-list target slice:
  ...
  `FormulaTokenInput` converter now enter the runtime data-bind graph, report
  ...
  list targets, deterministic target-to-source, and deterministic/random
  target-dirty behavior are covered separately below. Generated list items,
...
- `DataConverterFormula` random list fallback bindable-list target slice:
  ...
  items, imported/owned contexts, real random generation, secondary
  dependency invalidation, and full dirty-list scheduler parity remain
  follow-up slices. Random explicit/public-update target-to-source behavior,
  random target-dirty behavior, and random call counts are covered separately
...
- `DataConverterFormula` list fallback bindable-list explicit
  ...
  target-to-source behavior is covered separately below, and random
  public-update list-target target-to-source behavior is covered separately
  below. The deterministic and random target-dirty paths are covered
  separately below. Generated list items, imported/owned contexts, real random
...
- `DataConverterFormula` list fallback bindable-list target-dirty slice:
  default-context list sources feeding state-machine
  `BindablePropertyList.propertyValue` targets through a deterministic
  `FormulaTokenInput` converter now preserve the manually edited list-target
  scalar during explicit `advanceDataContext()`, keep reporting the imported
  source list size, and reapply the numeric formula fallback on later normal
  state-machine advancement. Random target-dirty behavior is covered
  separately below. Generated list items, imported/owned contexts, real random
  generation, secondary dependency invalidation, and full dirty-list
  scheduler parity remain follow-up slices. The contract is
...
- `DataConverterFormula` random list fallback bindable-list target-dirty
  slice: default-context list sources feeding state-machine
  `BindablePropertyList.propertyValue` targets through `FunctionType::random`
  output tokens with `randomModeValue` values `0`, `1`, and `2` now preserve
  the manually edited list-target scalar during explicit
  `advanceDataContext()`, keep reporting the imported source list size, ignore
  supplied random values for this observable fallback, and reapply the numeric
  formula fallback on later normal state-machine advancement. Generated list
  items, imported/owned contexts, real random generation, secondary
  dependency invalidation, and full dirty-list scheduler parity remain
  follow-up slices. Random call counts are covered separately below. The
  contract is
...
- `DataConverterFormula` random list fallback bindable-list explicit
  ...
  remain follow-up slices. Random call counts are covered separately below.
  The contract is
...
- `DataConverterFormula` random list fallback bindable-list public-update
  target-to-source slice: default-context list sources feeding state-machine
  `BindablePropertyList.propertyValue` targets through `FunctionType::random`
  output tokens with `randomModeValue` values `0`, `1`, and `2` now keep
  reporting the imported source list size during public `updateDataBinds(true)`
  target-to-source, ignore supplied random values for this observable reverse
  fallback, and reapply the numeric formula fallback to the list target during
  the same public update, matching C++. Random target-dirty behavior is
  covered separately above. Generated list items, imported/owned contexts,
  real random generation, secondary dependency invalidation, and full
  dirty-list scheduler parity remain follow-up slices. Random call counts are
  covered by the list call-count slice below. The contract is
...
- `DataConverterFormula` random list fallback call-count slice:
  default-context list sources flowing through direct `FunctionType::random`
  fallback tokens now expose Rust's host-supplied random-stream pull count
...
- `DataConverterFormula` list fallback explicit target-to-source slice:
  ...
  The deterministic formula list target explicit behavior is covered
  separately above. Deterministic and random target-dirty behavior is covered
  separately below. Symbol-list-index explicit behavior is covered separately
  below; imported/owned contexts, random formulas, secondary dependency
  invalidation, and broader dirty-list scheduling remain follow-up slices. The
  contract is
  `docs/prototypes/data-binding-graph-formula-list-fallback-explicit-target-to-source-runtime-contract.md`.
- `DataConverterFormula` list fallback target-dirty slice: default-context
  list sources feeding main-`ToTarget | TwoWay` number targets through a
  deterministic `FormulaTokenInput` converter now preserve the manually edited
  number target during explicit `advanceDataContext()`, keep reporting the
  imported source list size, and reapply the numeric formula fallback on later
  normal state-machine advancement. Random target-dirty behavior is covered
  separately below. Imported/owned contexts, secondary dependency
  invalidation, and full dirty-list scheduler parity remain follow-up slices.
  The contract is
  `docs/prototypes/data-binding-graph-formula-list-fallback-main-to-target-two-way-target-dirty-runtime-contract.md`.
- `DataConverterFormula` random list fallback target-dirty slice:
  default-context list sources feeding main-`ToTarget | TwoWay` number targets
  through `FunctionType::random` output tokens with `randomModeValue` values
  `0`, `1`, and `2` now preserve the manually edited number target during
  explicit `advanceDataContext()`, keep reporting the imported source list
  size, ignore supplied random values for this observable fallback, and
  reapply the numeric formula fallback on later normal state-machine
  advancement. Imported/owned contexts, real random generation, random
  call-count parity, secondary dependency invalidation, and full dirty-list
  scheduler parity remain follow-up slices. The contract is
...
- `DataConverterFormula` symbol-list-index function target-dirty slice:
  default-context symbol-list-index sources feeding main-`ToTarget | TwoWay`
  number targets through deterministic `FormulaTokenFunction` output tokens
  now preserve the manually edited number target during explicit
  `advanceDataContext()`, keep the symbol-list-index source unchanged, and
  reapply the C++ function-token formula value on later normal state-machine
  advancement. Imported/owned contexts, secondary dependency invalidation,
  and full dirty-list scheduler parity remain follow-up slices. The contract
  is
...
- `DataConverterFormula` random symbol-list-index source-to-target slice:
  ...
  Always-mode, source-change-mode, target-to-source behavior, and target-dirty
  behavior are covered separately below. List formulas, other non-number
  random formulas, imported/owned contexts, and real random generation remain
  follow-up slices.
- `DataConverterFormula` random symbol-list-index always-mode source-to-target
  ...
  and target-dirty behavior are covered separately below. List formulas, other
...
- `DataConverterFormula` random non-number fallback slice: default-context
  ...
  separately below. List formulas, imported/owned contexts, real random
  generation, secondary dependency invalidation, and full dirty-list scheduler
  parity remain follow-up slices. Enum/color/string/trigger target-to-source
  and target-dirty behavior are covered separately below. Source-to-target and
  target-to-source call counts are covered separately below. The
  contract is
  `docs/prototypes/data-binding-graph-formula-random-non-number-fallback-runtime-contract.md`.
- `DataConverterFormula` random non-number source-to-target call-count slice:
...
- `DataConverterFormula` random boolean fallback target-to-source slice:
  ...
  covered separately below; enum/color/string/trigger target-dirty behavior is
  covered separately below. Imported/owned contexts, real random generation,
  secondary dependency invalidation, and full dirty-list scheduler parity
  remain follow-up slices. Boolean target-to-source call counts are covered
...
- `DataConverterFormula` random boolean target-to-source call-count slice:
  default-context boolean sources feeding number targets now expose Rust's
  host-supplied random-stream pull count for explicit
...
- `DataConverterFormula` random boolean fallback target-dirty slice:
  ...
  number target edit through explicit data-context advancement, keep the
  boolean source unchanged, reapply C++'s numeric formula fallback on later
  normal state-machine advancement, and keep Rust's host-supplied
  random-stream call count at zero for `randomModeValue` values `0`, `1`, and
  `2`. Enum/color/string/trigger target-dirty behavior is covered separately
  below. Imported/owned contexts, real random generation, secondary dependency
  invalidation, and full dirty-list scheduler parity remain follow-up slices.
  The contract is
...
- `DataConverterFormula` remaining random fallback target-dirty slice:
  default-context enum, color, string, and trigger sources feeding number
  targets now preserve a manual number target edit through explicit
  data-context advancement, keep enum/color/string sources unchanged, reapply
  C++'s numeric formula fallback on later normal state-machine advancement,
  and keep Rust's host-supplied random-stream call count at zero for
  `randomModeValue` values `0`, `1`, and `2`. Trigger source count/reset
  behavior is outside this mixed-source number-target contract because the
  C++ probe does not expose trigger bindings for this bind shape.
  Imported/owned contexts, real random generation, secondary dependency
  invalidation, and full dirty-list scheduler parity remain follow-up slices.
  The contract is
...
- `DataConverterFormula` remaining random fallback target-to-source slice:
  ...
  main-`ToSource | TwoWay` and main-`ToTarget | TwoWay` number targets now
  preserve the unchanged source when random-function formula conversion
  produces a number, then reapply that unchanged source through the formula
  ...
  behavior is covered separately below; enum/color/string/trigger
  target-to-source call counts are covered separately below. Imported/owned
...
- `DataConverterFormula` random symbol-list-index target-to-source slice:
  ...
  draw before the visible reapply value. Target-dirty behavior is covered
  separately below. Grouped symbol-list-index random formulas, imported/owned
  contexts, real random generation, and random call-count parity remain
  follow-up slices. The contract is
  `docs/prototypes/data-binding-graph-formula-random-symbol-list-index-target-to-source-runtime-contract.md`.
- `DataConverterFormula` random symbol-list-index explicit target-to-source
  call-count slice: default-context symbol-list-index sources feeding
  main-`ToSource | TwoWay` number targets now expose Rust's host-supplied
  random-stream pull count during explicit `advanceDataContext()`
...
- `DataConverterFormula` random symbol-list-index public-update call-count
  ...
  TwoWay` number targets now expose Rust's host-supplied random-stream pull
  count during public `updateDataBinds(true)` target-to-source scheduling and
...
- `DataConverterFormula` random symbol-list-index target-dirty slice:
  default-context symbol-list-index sources feeding main-`ToTarget | TwoWay`
  number targets now preserve a manual number target edit through explicit
  data-context advancement, then reapply the unchanged symbol-list-index
  source through direct random formula scheduling on later normal
  ...
  separately below. Grouped target-dirty behavior is covered separately below.
...
- `DataConverterFormula` random symbol-list-index group source-to-target
  ...
  separately below. Grouped public-update target-to-source and target-dirty
  behavior are covered separately below. List formulas, remaining non-number
  random scheduling, imported/owned contexts, real random generation, and
  grouped target-to-source, public-update, and target-dirty call-count parity
  remain follow-up slices. The contract is
...
- `DataConverterFormula` random symbol-list-index group non-default
  ...
  target-to-source and target-dirty behavior are covered separately below. List
  formulas, remaining non-number random scheduling, imported/owned contexts,
  real random generation, and grouped target-to-source, public-update, and
  target-dirty call-count parity remain follow-up slices. The contract is
...
- `DataConverterFormula` random symbol-list-index group source-to-target
  ...
  targets through `DataConverterGroup<OperationValue, Formula(random)>` now
  expose Rust's host-supplied random-stream pull count during grouped
  source-to-target scheduling. Default mode consumes one initial draw and
  reuses it across later advancement, always mode consumes one draw per
...
- `DataConverterFormula` random symbol-list-index group explicit
  ...
  number targets through main-`ToSource | TwoWay`
  ...
  contexts, real random generation, and grouped public-update and target-dirty
...
- `DataConverterFormula` random symbol-list-index group explicit
  target-to-source call-count slice: default-context symbol-list-index sources
  feeding number targets through main-`ToSource | TwoWay`
  `DataConverterGroup<OperationValue, Formula(random)>` now expose Rust's
  host-supplied random-stream pull count during explicit
  `advance_data_context` target-to-source scheduling and later normal
...
- `DataConverterFormula` random symbol-list-index group target-dirty slice:
  ...
  `DataConverterGroup<OperationValue, Formula(random)>` now preserve a manual
  number target edit through explicit data-context advancement, then reapply
...
- `DataConverterFormula` random symbol-list-index group target-dirty
  ...
  targets through main-`ToTarget | TwoWay`
  `DataConverterGroup<OperationValue, Formula(random)>` now expose Rust's
  host-supplied random-stream pull count for target-dirty preservation and
...
- `DataConverterFormula` random always-mode explicit target-to-source slice:
  ...
  reapplication, and no additional values on later normal advances in this
  direct fixture, matching the C++ probe reports. Target-dirty, grouped
  ...
  random seeding/queueing, cache invalidation, and broader call counts remain
  follow-up slices. Direct explicit target-to-source call counts are covered
  separately by
  `docs/prototypes/data-binding-graph-formula-random-target-to-source-call-count-runtime-contract.md`.
- `DataConverterFormula` random always-mode public update target-to-source
  ...
  target-to-source source write, and same-update source-to-target
  reapplication without pulling again on later normal advances in this direct
  fixture, matching the C++ probe reports. Target-dirty, grouped
  target-to-source, grouped public-update, grouped target-dirty, list,
  remaining non-number `RandomMode::always`, real Rust random generation, C++
  random seeding/queueing, cache invalidation, and broader call counts remain
  follow-up slices. Direct public-update call counts are covered separately by
  `docs/prototypes/data-binding-graph-formula-random-public-update-call-count-runtime-contract.md`.
- `DataConverterFormula` random always-mode target-dirty slice:
  ...
  advancement, consumes one fresh value on the first later normal reapply, and
  does not pull again on the second later normal advance in this direct
  fixture, matching C++. Grouped target-to-source, grouped public-update,
  ...
  broader call counts remain follow-up slices. Direct target-dirty call counts
...
- `DataConverterFormula` random source-change target-to-source slice:
  ...
  same-pass source-to-target reapplication without pulling again on later
  normal advances in this direct fixture, matching C++. Public update,
  ...
  seeding/queueing, and broader call counts remain follow-up slices. Direct
  explicit target-to-source call counts are covered separately by
  `docs/prototypes/data-binding-graph-formula-random-target-to-source-call-count-runtime-contract.md`.
- `DataConverterFormula` random source-change public-update target-to-source
  ...
  same-update source-to-target reapplication without pulling again on later
  normal advances in this direct fixture, matching C++. Target-dirty,
  ...
  seeding/queueing, and broader call counts remain follow-up slices. Direct
  public-update call counts are covered separately by
  `docs/prototypes/data-binding-graph-formula-random-public-update-call-count-runtime-contract.md`.
- `DataConverterFormula` random source-change target-dirty slice:
  ...
  dependency invalidation, real Rust random generation, C++ random
  seeding/queueing, and broader call counts remain follow-up slices. Direct
  target-dirty call counts are covered separately by
  `docs/prototypes/data-binding-graph-formula-random-target-dirty-call-count-runtime-contract.md`.
...
- `DataConverterFormula` random group source-to-target slice:
  ...
  state-machine advancement. List formulas, cache invalidation, imported/owned
  contexts, and real random generation remain follow-up slices. Grouped
  non-number source-to-target, target-to-source, public-update, and
  target-dirty scheduling is covered separately below.
...
- `DataConverterFormula` random group always-mode target-to-source slice:
  ...
  reapplication. Later normal state-machine advances in this fixture do not
  reschedule the grouped formula without another source or target mutation.
  Grouped public-update, grouped target-dirty, source-change grouped
  public-update/target-dirty, list formulas, non-number, cache invalidation,
...
- `DataConverterFormula` random group always-mode public-update
  ...
  state-machine advances in this fixture do not reschedule the grouped formula
  without another source or target mutation. List formulas, non-number, cache
  invalidation, call counts, imported/owned contexts, and real random
  generation remain follow-up slices. Grouped target-dirty and source-change
...
- `DataConverterFormula` random group source-change source-to-target slice:
  ...
  formulas, secondary converter dependency invalidation, cache invalidation,
  imported/owned contexts, and real random generation remain follow-up slices.
  Grouped non-number source-to-target, target-to-source, public-update, and
...
- `DataConverterFormula` random group non-number source-to-target slice:
  ...
  source-change `[1, 2, 2]` source-to-target schedules. List paths,
  imported/owned contexts, secondary dependency invalidation, and real random
  generation remain follow-up slices. Grouped non-number explicit
  target-to-source, public-update, and target-dirty behavior are covered
  separately below.
- `DataConverterFormula` random group non-number explicit target-to-source
  ...
  target-to-source schedules. List paths, imported/owned contexts, secondary
  dependency invalidation, and real random generation remain follow-up slices.
  Grouped non-number public-update and target-dirty behavior are covered
  separately below.
- `DataConverterFormula` random group non-number public-update slice:
  ...
  update schedules. List paths, imported/owned contexts, secondary dependency
  invalidation, and real random generation remain follow-up slices. Grouped
  non-number target-dirty behavior is covered separately below.
- `DataConverterFormula` random group non-number target-dirty slice:
  default-context boolean, enum, color, string, and trigger sources feeding
  number targets through a main-`ToTarget | TwoWay`
  `DataConverterGroup<OperationValue, Formula(random)>` now preserve a manual
  number target edit during explicit data-context advancement, then match C++'s
  later source-to-target reapply for `randomModeValue` values `0`, `1`, and
  `2`. C++ and Rust random call counts match for default `[1, 1, 1, 1]`,
  always `[1, 1, 2, 2]`, non-trigger source-change `[1, 1, 1, 1]`, and trigger
  source-change `[1, 1, 2, 2]` schedules. The trigger source-change case clears
  the nested grouped formula random cache when the bound trigger reset changes
  the source. List paths, imported/owned contexts, secondary dependency
...
- Operation-view-model grouped context recompute slice:
...
- Operation-view-model owned symbol mutation slice:
  mutating an already-bound owned symbol-list-index source by state-machine
  data-bind index now updates the ordinary symbol-list-index bind while direct
  and grouped `DataConverterOperationViewModel` operands keep the C++ `0.0`
...
- Operation-view-model owned string mutation slice:
  mutating an already-bound owned string source by state-machine data-bind
  index now updates the ordinary string bind while direct and grouped
  `DataConverterOperationViewModel` operands keep the C++ `0.0` fallback.
...
- Operation-view-model owned trigger mutation slice:
  mutating an already-bound owned trigger source by state-machine data-bind
  index now updates the ordinary trigger bind while direct and grouped
  `DataConverterOperationViewModel` operands keep the C++ `0.0` fallback.
  This uses the existing owned trigger mutation command rather than admitting
  non-number operation-viewmodel operands. The contract is
  `docs/prototypes/data-binding-graph-operation-viewmodel-owned-trigger-mutation-runtime-contract.md`.
- Operation-view-model owned list mutation slice:
  mutating an already-bound owned list source item count by state-machine
  data-bind index now updates the ordinary list bind while direct and grouped
  `DataConverterOperationViewModel` operands keep the C++ `0.0` fallback.
  This uses the existing owned list mutation command rather than admitting
  non-number operation-viewmodel operands. The contract is
  `docs/prototypes/data-binding-graph-operation-viewmodel-owned-list-mutation-runtime-contract.md`.
- Operation-view-model owned asset mutation slice:
  mutating an already-bound owned asset source by state-machine data-bind index
  now updates the ordinary asset bind while direct and grouped
  `DataConverterOperationViewModel` operands keep the C++ `0.0` fallback.
