...
//! `src/data_bind/context/context_value.cpp`.
//!
//! Context values retain typed source/target snapshots, validity,
//! conversion/reverse-conversion state, and occurrence-local converter state.
...
pub(crate) struct RuntimeDataBindGraphSourceNode {
    ...
    /// Sources formerly retained by a Formula converter. Pinned C++ replaces
    /// `DataConverterFormula::m_source` on a non-null A→B rebind without
    /// removing the converter dependent from A. Dirt from A subsequently
    /// clears source-change randoms but does not dirty the outer DataBind.
    retained_formula_sources: RuntimeRetainedFormulaSources,
    /// Every OperationViewModel operand ever bound by this converter
    /// occurrence. Pinned C++ never removes the outer DataBind from departed
    /// operands, including across later unbind/default-context transitions.
    retained_operation_sources: Vec<RuntimeViewModelCell>,
    /// Primary Formula source detached by the graph-wide context reset. It is
    /// promoted to the converter's historical dependency only if the
    /// following non-null resolution binds a different source; an unresolved
    /// bind follows Formula::unbind and drops it.
    departed_formula_source: Option<RuntimeViewModelCell>,
    pub(crate) converter: Option<RuntimeDataBindGraphConverter>,
...
struct RuntimeRetainedFormulaSources {
    cells: Vec<RuntimeViewModelCell>,
    sink: RuntimeCellDirtSink,
}

impl Clone for RuntimeRetainedFormulaSources {
    fn clone(&self) -> Self {
        let mut cloned = Self::default();
        cloned.retain(self.cells.clone());
        cloned.sink.add_dirt(self.sink.peek_dirt());
        cloned
    }
}

impl Drop for RuntimeRetainedFormulaSources {
    fn drop(&mut self) {
...
impl RuntimeRetainedFormulaSources {
    fn retain(&mut self, cells: Vec<RuntimeViewModelCell>) {
        for cell in cells {
            if self.cells.iter().any(|current| current.ptr_eq(&cell)) {
                continue;
            }
...
fn matching_graph_source_value(
    ...
    match current {
        RuntimeDataBindGraphValue::Number(_) => crate::context_value_number::matching(next),
        RuntimeDataBindGraphValue::Boolean(_) => crate::context_value_boolean::matching(next),
        RuntimeDataBindGraphValue::String(_) => crate::context_value_string::matching(next),
        RuntimeDataBindGraphValue::Color(_) => crate::context_value_color::matching(next),
        RuntimeDataBindGraphValue::Enum(_) => crate::context_value_enum::matching(next),
        RuntimeDataBindGraphValue::SymbolListIndex(_) => {
            crate::context_value_symbol_list_index::matching(next)
        }
        // Font and image values share C++'s integer DataValue payload at this
        // seam; the concrete retained source cell selects the asset subtype
...
impl RuntimeDataBindGraphConverter {
    ...
    fn retained_operation_view_model_cells(&self, cells: &mut Vec<RuntimeViewModelCell>) {
        match self {
            Self::OperationViewModel {
                retained_operation_value: Some(cell),
                ..
            ...
            Self::Group(converters) => {
                for converter in converters {
                    converter.retained_operation_view_model_cells(cells);
                }
            }
            _ => {}
        }
...
// Retained temporarily as disabled source while downstream branches finish
// rebasing the owner move; the production type lives in the mapped converter
// file and no compiled path uses this copy.
#[cfg(any())]
mod pre_fl_d4_interpolator_owner {
    use super::*;
    ...
    impl RuntimeDataBindGraphInterpolatorState {
        ...
        fn convert(
            ...
            if self.advancer.is_none() {
                let Some(input_value) = RuntimeDataBindGraphInterpolatorValue::from_graph(input)
                else {
                    return Some(input.clone());
                };
    ...
    impl RuntimeDataBindGraphInterpolatorAdvancer {
        ...
        fn advance(
            ...
            if self.animation_data(animation_index).to == self.current_value
                || elapsed_seconds == 0.0
            {
...
pub(crate) fn runtime_data_bind_graph_convert_value(
    ...
    match (converter, value) {
        ...
        (
            ...
            crate::data_converter_list_to_length::convert(Some(*value)),
        )),
        (
            RuntimeDataBindGraphConverter::ListToLength,
            RuntimeDataBindGraphValue::List { item_count },
        ) => Some(RuntimeDataBindGraphValue::Number(
            crate::data_converter_list_to_length::convert(Some(*item_count)),
        )),
        (RuntimeDataBindGraphConverter::ListToLength, _) => Some(
        ...
        (
            ...
            crate::data_converter_rounder::convert(*value, *decimals),
        )),
        ...
        (RuntimeDataBindGraphConverter::Group(converters), value) => {
            ...
            for index in crate::data_converter_group::forward_indices(converters.len()) {
...
pub(crate) fn runtime_data_bind_graph_reverse_convert_value(
    ...
    match (converter, value) {
        ...
        (RuntimeDataBindGraphConverter::Group(converters), value) => {
            let mut value = value.clone();
            for index in crate::data_converter_group::reverse_indices(converters.len()) {
                let converter = &converters[index];
        ...
        match token {
            ...
            RuntimeDataBindGraphFormulaToken::Operation { operation_type } => {
                if stack.len() > 1 {
                    ...
                    stack.push(crate::data_converter_formula::binary_operation(
                        left,
                        right,
                        *operation_type,
                    ));
        ...
        "DataConverterGroup" => RuntimeDataBindGraphConverter::Group(
            file.data_converter_group_items_for_object(converter)
                ...
                .filter_map(|item| {
                    ...
                    let converter = runtime_data_bind_graph_converter_for_object(
                        file,
                        converter,
                        visiting,
                        true,
                    ...
                    Some(
                        crate::data_converter_group_item::RuntimeDataConverterGroupItem::import(
                            converter,
                        )
                        .into_owned_converter(),
                    )
                })
                .collect(),
        ),
...
impl RuntimeDataBindGraph {
    ...
    pub(crate) fn sync_key_frame_sources_from(&mut self, prototype: &Self) {
        ...
        for (source, prototype_source) in self.sources.iter_mut().zip(&prototype.sources) {
            ...
            if source.converter != prototype_source.converter {
                source.retain_departed_operation_sources();
                source.converter = prototype_source.converter.clone();
                source.reset_converter_state();
                source.sync_current_converter_operand_sources();
            }
            if source_changed {
    ...
    fn default_view_model_source_node(
        ...
        RuntimeDataBindGraphSourceNode {
            ...
            retained_bind,
            retained_formula_sources: RuntimeRetainedFormulaSources::default(),
            retained_operation_sources: Vec::new(),
            departed_formula_source: None,
    ...
    fn bind_owned_view_model_data_context_source(
        ...
        match resolved {
            Some((value, source_cell, structural_source)) => {
                ...
                if source.departed_formula_source.is_none() {
                    source.remember_departed_formula_source();
                }
                source.retained_bind.reset_preserving_notification();
                source.retain_departed_operation_sources();
                if let Some(source_cell) = source_cell {
                    source.retained_bind.set_source(source_cell.clone());
                    source.finish_formula_source_rebind(Some(&source_cell));
                } else {
                    source.finish_formula_source_rebind(None);
                }
    ...
    fn refresh_retained_source_from_cell(source: &mut RuntimeDataBindGraphSourceNode) -> bool {
        source.collect_retained_formula_source_dirt();
    ...
    pub(crate) fn set_owned_view_model_context_list_source_item_count_for_data_bind(
        ...
        let context_changed =
            crate::data_bind_list_item_consumer::changed(current_context_item_count, item_count);
        let source_changed = self.sources.iter().any(|source| {
            source.path == path
                && matches!(
                    source.default_value,
...
impl RuntimeDataBindGraphSourceNode {
    ...
    fn retain_departed_operation_sources(&mut self) {
        ...
        if let Some(converter) = self.converter.as_ref() {
            converter.retained_operation_view_model_cells(&mut operation_cells);
        }
        for cell in operation_cells {
            if !self
                .retained_operation_sources
                .iter()
                .any(|current| current.ptr_eq(&cell))
            {
                self.retained_operation_sources.push(cell);
            }
        }
        self.retained_bind
            .retain_additional_sources(self.retained_operation_sources.clone());
    }

    fn remember_departed_formula_source(&mut self) {
        if self
            .converter
            .as_ref()
            .is_some_and(runtime_data_bind_graph_converter_contains_formula)
            && let Some(cell) = self.retained_bind.source().cloned()
        {
            self.departed_formula_source = Some(cell);
        }
    }

    fn finish_formula_source_rebind(&mut self, current: Option<&RuntimeViewModelCell>) {
        let Some(departed) = self.departed_formula_source.take() else {
            return;
        };
        if current.is_some_and(|current| current.ptr_eq(&departed)) {
            return;
        }
        if current.is_some() {
            self.retained_formula_sources.retain(vec![departed]);
        }
    }

    fn sync_current_converter_operand_sources(&mut self) {
        ...
        for cell in &operation_cells {
            if !self
                ...
                .any(|current| current.ptr_eq(cell))
            {
                self.retained_operation_sources.push(cell.clone());
            }
        }
        operand_cells.retain(|cell| {
            !operation_cells
                .iter()
                .any(|operation| operation.ptr_eq(cell))
        });
        self.retained_bind.set_additional_sources(operand_cells);
        self.retained_bind
            .retain_additional_sources(self.retained_operation_sources.clone());
    }

    fn collect_retained_formula_source_dirt(&mut self) {
        if self.retained_formula_sources.take_dirt() {
            // Formula::addDirt clears only sourceChange-mode random caches;
            // it never dirties or schedules its parent DataBind.
...
mod tests {
    ...
    fn key_frame_source_sync_retains_departed_operation_operand_like_cpp() {
        ...
        assert!(
            ...
            "C++ leaves the outer DataBind registered on the departed OperationViewModel operand"
        );
        ...
        assert!(occurrence.bind_default_view_model_context());
        assert!(old_operand.set_value(RuntimeViewModelCellValue::Number(4.0)));
        assert!(
            occurrence.collect_retained_source_dirt(),
            "the first departed operand remains registered across converter unbind/default reset"
        );
        assert!(new_operand.set_value(RuntimeViewModelCellValue::Number(8.0)));
        assert!(
            occurrence.collect_retained_source_dirt(),
            "the latest operand also remains registered across converter unbind/default reset"
        );
    }

    #[test]
    fn departed_formula_source_clears_randoms_without_dirtying_outer_bind() {
        let old_source = RuntimeViewModelCell::new(RuntimeViewModelCellValue::Number(2.0));
        let new_source = RuntimeViewModelCell::new(RuntimeViewModelCellValue::Number(5.0));
        let mut graph = graph_with_number_binding(0);
        ...
        graph.sources[0].converter = Some(converter.clone());
        graph.sources[0].converter_state =
            RuntimeDataBindGraphConverterState::for_converter(Some(&converter));
        let RuntimeDataBindGraphConverterState::Formula(state) =
            &mut graph.sources[0].converter_state
        else {
            panic!("formula state");
        };
        state.randoms.push(0.25);

        graph.sources[0]
            .retained_bind
            .set_source(old_source.clone());
        graph.sources[0].remember_departed_formula_source();
        graph.sources[0]
            .retained_bind
            .reset_preserving_notification();
        graph.sources[0].retained_bind.set_source(new_source);
        let current = graph.sources[0].retained_bind.source().cloned();
        graph.sources[0].finish_formula_source_rebind(current.as_ref());

        assert!(old_source.set_value(RuntimeViewModelCellValue::Number(3.0)));
        assert!(
            !graph.collect_retained_source_dirt(),
            "Formula's departed source dirt does not propagate to the parent DataBind"
        );
        let RuntimeDataBindGraphConverterState::Formula(state) = &graph.sources[0].converter_state
        else {
            panic!("formula state");
        };
        assert!(
            state.randoms.is_empty(),
            "the departed Formula source still clears source-change randoms"
        );
    }
