...
Partially open. `crates/rive-graph` now exposes draw target/rule/clipping relationships, and `drawable_order` mirrors C++ `m_Drawables` initialization through foreground reordering, layout proxy insertion, and flattened draw-rule assignment. Remaining work: `sortDrawOrder()`, active draw-target linked-list mutation, clipping-stack proxy operations, draw-rule target ordering, and a renderer-independent draw command stream. This should remain headless and comparable in tests before any GPU work begins.
