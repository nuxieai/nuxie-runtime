# Data Binding Graph Asset Target-To-Source Runtime Contract

## Purpose

Extend graph-owned target-to-source runtime behavior to direct asset binds.

This slice mirrors the direct scalar target-to-source paths for a
default-context asset bind. A mutable `BindablePropertyAsset.propertyValue`
target writes back into its `ViewModelInstanceAssetImage.propertyValue` source,
and a second source-to-target bind observes that updated source on the same
explicit data-context advance.

## In Scope

- Default root view-model context bound with `bind_default_view_model_context`.
- Root-only `DataBindContext.sourcePathIds` of shape `[0, propertyIndex]`.
- Direct `ViewModelInstanceAssetImage.propertyValue` sources.
- Direct `BindablePropertyAsset.propertyValue` targets.
- `DataBindContext.flags` handling for direct asset binds:
...
- Mutating the first two-way asset target marks its graph binding
  target-to-source dirty.
- `advance_data_context` copies that target asset ID into the bound default
  source before source-to-target propagation.
- A second bind to the same source observes the updated asset and drives the
  same transition reports as C++.
- Existing source-to-target default-context asset behavior remains unchanged.
