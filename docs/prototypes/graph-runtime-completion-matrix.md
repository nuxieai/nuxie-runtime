...
| Active draw target linked lists and draw-command emission | Partially graph/runtime-owned | Active target grouping, before/after placement, clipping proxy interleaving, save-operation elision, and imported hidden facts are represented by `sorted_drawable_order`; `rive-runtime` emits the first logical draw command stream for simple hidden/render-opacity filtering and pathless-source empty-clip suppression. Full path/runtime empty-clip handling, paint/geometry payloads, and remaining type-specific drawability remain deferred draw runtime work. |
...
- Full `Shape::isEmpty()`/`PathComposer::worldPath()` empty-clip handling,
  renderer paint allocation, geometry payloads, remaining type-specific
