...
| 17 | `syncSourceTextFromRaw` (275) | Read raw text; for single-line strip and write corrected raw text preserving cursor; move to source; invoke generated `text(source)` callback. | `text_input.rs:317::sync_text_input_source_from_raw` and `set_string_property` | **incomplete**. Rust computes the stripped single-line source but never writes the correction back to the raw editor. The correction contract is raw `textPreserveCursor` write first, then source move, then generated property callback. Blocker: a single-line raw line-break trace that observes raw text, source text, cursor, and callback order. |
...
| # | Pinned definition | Required behavior, side effects, and order | Concrete Rust owner | Candidate disposition and evidence/blocker |
|---:|---|---|---|---|
| 22 | `keyInput` (370) | Ignore releases. For each recognized pressed key, run the exact raw operation, then publish the specified dirt even if the raw value did not change. Undo/redo/backspace/delete sync source then shape dirt; select/navigation use paint dirt; Enter rejects single-line but multiline inserts/syncs and uses **Paint** dirt. | `text_input.rs:382::text_input_key_input`, `text_input.rs:350::text_input_line_range_for_cursor`, `text/raw_text_input.rs:374::cursor_horizontal`, `598::move_cursor_vertical` | **incorrect, with reconstructed navigation adaptation**. Rust gates dirt/source sync on the raw method's `changed` boolean, so recognized no-op keys omit pinned dirt, and multiline Enter uses shape dirt rather than Paint. Pinned `cursorUp`, `cursorDown`, and line-boundary raw operations are reconstructed from retained geometry through `move_cursor_vertical`, `cursor_horizontal`, and a separately computed line range rather than literal raw owners. Blockers: repeated/no-op and Enter dirt traces plus vertical, wrapped-line, and bidi navigation differentials; do not fix from tests without preserving the pinned operation and side-effect order. |
...
| 31 | `worldToLocalWithViewport` (560) | Zero both velocities before inversion; invert or fail; transform point; for active single/multiline axis compute 20px edge distances, set signed speed only when enabled, and clamp coordinates only beyond viewport. | `text_input.rs:611::text_input_move_cursor_to_world_with_auto_scroll`, `constraints.rs:1074::text_input_scroll_viewport` | **incorrect failure ordering, packed with cursor movement**. Rust returns for a missing runtime graph or singular transform before writing local zero velocities back to retained state, so failed conversion can preserve stale edge-scroll velocity. Blocker: trace both velocities before and after failed inversion as well as intermediate local/clamped coordinates on success. |
| 32 | `startDrag` (653) | Set dragging and last world position before conversion; on successful conversion move cursor without selection and always Paint dirt. | `text_input.rs:710::text_input_start_drag` plus `602::text_input_move_cursor_to_world` | **incorrect**. State-before-conversion order is exact, but Paint dirt occurs only when the cursor changes, and delegated conversion can preserve stale velocities when inversion fails instead of zeroing them first. Blockers: same-position start-drag and failed-inversion velocity traces. |
| 33 | `drag` (672) | Update last world position before conversion; auto-scroll conversion; on success extend selection and always Paint dirt. | `text_input.rs:721::text_input_drag` plus `611::text_input_move_cursor_to_world_with_auto_scroll` | **incorrect**. Paint is gated on cursor/selection change, and failed delegated conversion can retain stale edge-scroll velocities. Blockers: successful unchanged-cursor conversion and failed-inversion velocity traces. |
...
| 37 | `advanceDrag` (712) | If not dragging, zero velocities and stop. Require nonzero velocity and constraint. Stop physics, advance and clamp each active axis, then re-hit the finite last position with auto-scroll, move selection, and always Paint on successful conversion; return current dragging state. | `artboard/text/text_input.rs:54::advance_text_input_entry`, `constraints.rs:1187::advance_text_input_scroll`, `text_input.rs:721::text_input_drag` | **incorrect**. The state/physics/clamp/re-hit order exists, but delegated drag omits Paint when the cursor does not change and its failed conversion can leave the just-retained edge-scroll velocities stale rather than clearing them before inversion. Blockers: stationary-cursor edge-scroll and failed re-hit velocity traces. |
