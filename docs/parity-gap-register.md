...
| H5 | Ten upstream files added after the frozen B-6 audit ref carry no `b6_verdict` and are allowlisted by name in `POST_AUDIT_UNAUDITED` (`tools/b6-audit/check.py`): `animation/keyframe_int`, `component_origin`, `core/field_types/core_int_type`, `data_bind/context/context_value_asset_blob`, `layout/{grid_item_placement,grid_track,layout_participant,layout_sizing_style}`, `viewmodel/viewmodel_instance_asset_blob`, `viewmodel/runtime/viewmodel_instance_asset_blob_runtime`. The allowlist keeps the surface visible (any *further* verdict-less row fails the gate) but nothing shrinks it on its own — this row is the owner. Audit them into a verdict and delete each from the set; the row closes when the set is empty. Each pin advance that adds files grows this debt, so clear it before the next sync rather than after. |
