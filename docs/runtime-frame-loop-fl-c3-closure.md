...
- [ ] The active transition is a retained definition handle. Rust may not copy
  transition fire-action, listener-action, interpolator, or duration payloads
  into the layer occurrence.
- [ ] `advance` preserves current advance → mix update → source advance →
...
- [ ] `resetState` removes keyframe binds and drops `stateFrom` only when it is
  distinct from `any` and current, drops current only when distinct from
  `any`, then re-enters the retained entry definition.
- [ ] Drop releases `any`, current, and source occurrences without double
  release under the same identity constraints.
- [ ] Rust's public snapshot clone remains an explicit Rust API adaptation:
...
- [ ] Missing required system state: Any, Entry, and Exit are each rejected at
  the C++ finalization boundary.
- [ ] Duplicate system states: the last authored Any/Entry/Exit wins exactly
  like the ordered C++ switch.
- [ ] Nullable/unknown state: one generic no-op state occurrence retains its
  slot and remains a valid transition target.
- [ ] Bad transition target: negative/sentinel and out-of-range `stateToId`
  fail import without compacting state indices.
- [ ] Empty AnyState: it is constructed and probed; no transition-count
  shortcut remains.
- [ ] Same-state target: no occurrence replacement, actions, or keyframe-bind
  churn occurs.
- [ ] Transition interruption: previous source, outgoing current, new current,
  mix-from, pause/hold, actions, and keyframe binds follow pinned order.
- [ ] Reset aliases: `any == stateFrom`, `any == current`, and
  `stateFrom == current` cannot double-drop or remove binds twice.
- [ ] Zero-time and same-frame chaining: system states and zero-duration
  transitions settle in the same bounded advance as C++.
- [ ] Nested input ordering: duplicate names use first-match lookup; index
  lookup preserves authored slots; initialization applies bool/number and
  deliberately skips trigger.
- [ ] Nested focus and pointer forwarding: shared focus manager is installed
  before tree sync; hit/pointer/drag calls return the child result or the
  pinned empty result.
- [ ] Nested context lifecycle: attach, replacement, clear, dependency release,
  clone, and parent teardown preserve owner order.
- [ ] Clone/remount isolation: definitions are shared where C++ retains them,
  live state occurrences are occurrence-local, and generated nested/layer
  clones remain cold.
- [ ] Permanent structural ratchets: reject the empty-Any shortcut, parallel
