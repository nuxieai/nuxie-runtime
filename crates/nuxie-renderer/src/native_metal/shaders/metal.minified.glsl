...
#define O3 struct S8{
#define P3 };
#define K5(f,w1,a) constant c1*a[[buffer(N1(f))]]
...
#define q1(k0,l) W0.k0.read(c1(l))
...
#define y1(a,e0,F,B,r) __attribute__((visibility("default")))n0 vertex a(uint B[[vertex_id]],uint r[[instance_id]],constant uint&Ng[[buffer(N1(Kc))]],constant CC&n[[buffer(N1(F4))]],constant e0*F[[buffer(0)]],T8 W0,R8 p3){r+=Ng;n0 Z;
#else
...
#define w2
#define x2
#define U8(a) __attribute__((visibility("default")))fragment a(R1 S0,constant CC&n[[buffer(N1(F4))]],n0 Z[[stage_in]],A5 W0,Ua v6,S8 p3){d Y=Z.O0.xy;c1 J=c1(metal::floor(Y));uint D0=J.y*n.m6+J.x;
...
#define Y1 }return S0;
#define Qg(a,...) struct Og{i Pg[[j(0)]];R1 S0;};Og __attribute__((visibility("default")))fragment a(__VA_ARGS__){d Y[[maybe_unused]]=Z.O0.xy;i C1;R1 S0;
#define o2(a) Qg(a,R1 B5,constant CC&n[[buffer(N1(F4))]],n0 Z[[stage_in]],A5 W0,S8 p3)
#define l3 }return{.Pg=C1,.S0=S0};
#endif
#define o4 w0
#define discard discard_fragment()
using namespace metal;template<int P1>e vec<uint,P1>floatBitsToUint(vec<float,P1>x){return as_type<vec<uint,P1>>(x);}template<int P1>e vec<int,P1>floatBitsToInt(vec<float,P1>x){return as_type<vec<int,P1>>(x);}e uint floatBitsToUint(float x){return as_type<uint>(x);}e int floatBitsToInt(float x){return as_type<int>(x);}template<int P1>e vec<float,P1>uintBitsToFloat(vec<uint,P1>x){return as_type<vec<float,P1>>(x);}e float uintBitsToFloat(uint x){return as_type<float>(x);}e E unpackHalf2x16(uint x){return as_type<E>(x);}e uint packHalf2x16(E x){return as_type<uint>(x);}e i unpackUnorm4x8(uint x){return unpack_unorm4x8_to_half(x);}e uint packUnorm4x8(i x){return pack_half_to_unorm4x8(x);}e g0 inverse(g0 l1){g0 Va=g0(l1[1][1],-l1[0][1],-l1[1][0],l1[0][0]);float Rg=(Va[0][0]*l1[0][0])+(Va[0][1]*l1[1][0]);return Va*(1/Rg);}e v mix(v m,v b,n6 G1){v D7;for(int E0=0;E0<3;++E0)D7[E0]=G1[E0]?b[E0]:m[E0];return D7;}e d mix(d m,d b,D4 G1){d D7;for(int E0=0;E0<2;++E0)D7[E0]=G1[E0]?b[E0]:m[E0];return D7;}e d mix(d m,d b,float t){return mix(m,b,d(t));}e float mod(float x,float y){return fmod(x,y);}