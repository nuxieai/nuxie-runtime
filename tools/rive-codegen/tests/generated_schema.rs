...
fn generated_schema_is_reproducible_from_cpp_defs() {
    ...
    let expected_path = workspace_root().join("crates/rive-schema/src/generated/schema.rs");
    let actual_path = std::env::temp_dir().join(format!(
        "rive-rust-generated-schema-{}.rs",
        std::process::id()
    ));

    let output = Command::new(env!("CARGO_BIN_EXE_rive-codegen"))
        .arg("--defs")
        .arg(runtime_dir.join("dev/defs"))
        .arg("--out")
        .arg(&actual_path)
        .output()
        .unwrap_or_else(|err| panic!("failed to run rive-codegen: {err}"));
    ...
    if actual != expected {
        ...
        panic!(
            "checked-in generated schema differs from formatted rive-codegen output: {mismatch}"
        );
    }
}
...
fn invalid_bitmask_passthrough_metadata_is_rejected_like_cpp_generator() {
    ...
    for (name, def, expected_stderr) in cases {
        ...
        assert!(
            !output.status.success(),
            "rive-codegen unexpectedly accepted invalid bitmask fixture {name}"
        );
...
fn duplicate_and_reserved_keys_are_rejected_like_cpp_generator() {
    ...
    for (name, files, expected_stderr) in cases {
        ...
        assert!(
            !output.status.success(),
            "rive-codegen unexpectedly accepted invalid key fixture {name}"
        );
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(
            stderr.contains(expected_stderr),
...
fn run_codegen(defs_dir: &Path, out_path: &Path) -> Output {
    Command::new(env!("CARGO_BIN_EXE_rive-codegen"))
        .arg("--defs")
        .arg(defs_dir)
        .arg("--out")
        .arg(out_path)
