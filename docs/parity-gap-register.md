...
| V16 | **CLOSED 2026-08-03 with a named numeric remainder: `bankcard` applies keyed values in authored order and settles dirty effect/feather owners from the final source path and transform before draw.** Text-style feather/effect descendants now rebuild the retained Text frame rather than refreshing paint alone. | `corpus.toml` `bankcard`, samples 0/1/2; targeted C++/Rust stream comparison including side channel | **Green under `tolerant(0.0141)`:** command/path structure and all side-channel records match. REMAINDER: one inner-feather text contour at t=2 differs by at most 0.014065 after sequential sampling; direct source contours and all other differences remain within 0.00013. |
