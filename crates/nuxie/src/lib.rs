...
use nuxie_runtime::{
    ...
    RuntimeOwnedViewModelInstance, RuntimeOwnedViewModelTransaction, StateMachineEventContext,
};
...
struct ScriptMountResult {
    has_script_target: bool,
    mounted: bool,
}
...
#[cfg(feature = "scripting")]
fn resolved_scripted_primitive_inputs_for_occurrence(
    runtime: &RuntimeFile,
    graph_global_id: u32,
    ...
    for global_id in start..end {
        ...
        let bound =
            instance.bound_script_input_value_for_occurrence(runtime, input, fallback_root)?;
        if let Some(value) = bound.or_else(|| default_script_input_value(input)) {
...
fn prepare_scripted_data_converter_hydration_from_snapshots(
    ...
    for snapshot in snapshots {
        ...
        match snapshot.kind {
            ...
            nuxie_runtime::ScriptListenerInputKind::Artboard => {
                let source = match snapshot.value.clone() {
                ...
                if let nuxie_runtime::ScriptArtboardSource::File(artboard_id) = &source {
                ...
                prepared_inputs.push(PreparedScriptListenerInput::Artboard { name, source });
            }
            nuxie_runtime::ScriptListenerInputKind::ViewModelProperty => {
    ...
    for prepared_input in prepared_inputs {
        match prepared_input {
            ...
            PreparedScriptListenerInput::Artboard { name, source } => {
...
impl nuxie_runtime::PreparedScriptArtboard for PreparedFileScriptArtboard {
    ...
    {
        let artboard = match &self.source {
            ...
            nuxie_runtime::ScriptArtboardSource::Live(bindable) => {
                FileScriptArtboard::new_from_live(
                    Arc::clone(&self.file),
                    bindable,
                    self.parent_context.as_ref(),
                )?
...
impl nuxie_runtime::ScriptArtboardResolver for FileScriptArtboardResolver {
    ...
    > {
        match source {
            ...
            nuxie_runtime::ScriptArtboardSource::Live(_) => {}
        }
        Ok(Box::new(PreparedFileScriptArtboard {
...
impl FileScriptArtboard {
    ...
    fn new_with_view_model(
        ...
        let instance = RuntimeArtboardInstance::from_graph_with_artboards_external_fonts_and_file_catalogs_and_asset_owners(
                &file.runtime,
                graph,
    ...
    fn from_concrete(
        ...
        let view_model = supplied_view_model.or_else(|| {
            ...
            // Read the current generated property from the fresh concrete
            // clone, not immutable source metadata, then create that model in
            // the consumer File exactly like `File::createViewModelInstance`.
            let view_model_key = nuxie_schema::definition_by_name("Artboard")
                .and_then(|definition| {
                    definition
                        .properties
                        .iter()
            ...
            let owned_view_model = view_model_key
                .and_then(|key| instance.debug_uint_property(0, key))
        ...
        Ok(Self {
            file,
            _source_file_pin: source_file_pin,
            source_runtime,
            source_artboards,
            source_external_image_assets,
            source_max_retained_decoded_image_bytes,
...
fn attach_prepared_script_mounts(
    ...
    if let Some(root) = groups.pop_front() {
        attach_interpolators(instance, root.interpolators, &root.file);
    ...
    let mut visitor = |_: usize, _: u32, nested: &mut RuntimeArtboardInstance| {
        if let Some(group) = groups.pop_front() {
            attach_interpolators(nested, group.interpolators, &group.file);
...
fn prepare_scripted_artboard_tree(
    ...
    let mount = mount_scripted_artboard_tree(
        file,
        root_graph,
        instance,
        factory,
...
async fn prepare_scripted_artboard_tree_async(
    ...
    let root_view_model = retained_artboard_root_view_model(instance);
    let mount = mount_scripted_artboard_tree_async(
        file,
        root_graph,
        instance,
        factory,
...
fn advance_scripted_artboard_frame_with_factory(
    ...
    let mount = mount_scripted_artboard_tree(file, root_graph, instance, factory, root_view_model)?;
    let mut input_changed = if let Some(root_view_model) = root_view_model {
        if mount.mounted {
            rehydrate_bound_script_input_tree(
                file,
                root_graph,
                instance,
        ...
        let mut changed = rehydrate_bound_script_input_tree(
            file,
            root_graph,
            instance,
            root_view_model.handle(),
        ...
        changed |= rehydrate_bound_script_input_tree(
            file,
            root_graph,
            instance,
            root_view_model.handle(),
            factory,
            ScriptInputHydrationPhase::PrimitiveBeforeAdvance,
        )?;
        changed
    } else {
        false
...
#[cfg(feature = "scripting")]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ScriptInputHydrationPhase {
    ArtboardMountedLive,
    ArtboardBeforeAdvance,
    PrimitiveBeforeAdvance,
    PrimitiveAfterAdvance,
}

#[cfg(feature = "scripting")]
fn rehydrate_bound_script_input_tree(
    file: &Arc<File>,
    root_graph: &ArtboardGraph,
    instance: &mut RuntimeArtboardInstance,
    root_view_model: &RuntimeOwnedViewModelHandle,
    factory: &mut dyn Factory,
    phase: ScriptInputHydrationPhase,
) -> std::result::Result<bool, nuxie_runtime::ScriptError> {
    let mut changed =
    ...
    let mut visitor = |_: usize, graph_global_id: u32, nested: &mut RuntimeArtboardInstance| {
        ...
        let selected_view_model = nested
            .scripted_interpolator_root_view_model(source_file.runtime(), Some(root_view_model));
        let Some(selected_view_model) = selected_view_model.as_ref() else {
            return Ok(());
        };
        changed |= rehydrate_bound_script_inputs(
            &source_file,
            graph,
            nested,
            selected_view_model,
...
fn rehydrate_bound_script_inputs(
    ...
    for component in graph.components.iter().filter(|component| {
        ...
        if matches!(
            ...
            for (name, value) in resolved_scripted_primitive_inputs_for_occurrence(
                &file.runtime,
                graph.global_id,
                component.local_id,
                instance,
                ...
                hydrated |= instance.sync_bound_script_input_for_global(
                    component.global_id,
                    &name,
                    value,
                    phase == ScriptInputHydrationPhase::PrimitiveAfterAdvance,
                )?;
            }
        }

        // `ScriptInputArtboard::updateArtboard` replaces the retained
        // referenced Artboard when its DataBind changes, and
        // `syncReferencedArtboard` immediately clones that source into the
        // script table (`script_input_artboard.cpp:67-79,123-132`;
        // `scripted_object.cpp:43-59`). Primitive inputs can compare their Lua
        // values directly; artboard userdata instead retains the resolved id
        // on this concrete scripted occurrence.
        if matches!(
            phase,
            ScriptInputHydrationPhase::PrimitiveBeforeAdvance
                | ScriptInputHydrationPhase::PrimitiveAfterAdvance
        ...
        let end = ((start + 1)..file.runtime.object_count())
            ...
            .unwrap_or_else(|| file.runtime.object_count());
        let fallback_context = nuxie_runtime::RuntimeOwnedViewModelContextHandle::root(
            &file.runtime,
            root_view_model.clone(),
        );
        let parent_context =
            instance.scripted_interpolator_artboard_parent_context(Some(&fallback_context));
        for global_id in start..end {
            let Some(input) = file.runtime.object(global_id) else {
                continue;
            };
            if input.type_name != "ScriptInputArtboard"
                || input.uint_property("parentId") != Some(component.local_id as u64)
            {
                continue;
            }
            let resolved_artboard_id = instance
                .bound_script_artboard_input_for_occurrence(
                    &file.runtime,
                    input,
                    Some(root_view_model),
                )?
                .or_else(|| input.uint_property("artboardId"))
                .filter(|artboard_id| *artboard_id != u64::from(u32::MAX));
            let Some(artboard_id) = resolved_artboard_id else {
                continue;
            };
            let Some(name) = input.string_property("name") else {
                continue;
            };
            if instance.script_artboard_input_matches_for_global(
                component.global_id,
                name,
                artboard_id,
            ) {
            ...
            if phase == ScriptInputHydrationPhase::ArtboardMountedLive {
                instance.record_resolved_script_artboard_input_for_global(
                    component.global_id,
                    name,
                    artboard_id,
                );
            ...
            let artboard_index = usize::try_from(artboard_id).map_err(|_| {
                nuxie_runtime::ScriptError::new(format!(
                    "ScriptInputArtboard global {} resolved artboard id {artboard_id} does not fit this platform",
                    input.id,
                ))
            })?;
...
impl<'a> ArtboardInstance<'a> {
    ...
    pub fn mount_scripted_drawables(&mut self, factory: &mut dyn Factory) -> Result<bool> {
        let artboard = self
            ...
            .context("artboard instance graph is unavailable")?;
        let root_view_model = retained_artboard_root_view_model(&self.raw);
        Ok(mount_scripted_artboard_tree(
            &self.script_file,
            artboard,
            &mut self.raw,
            factory,
...
impl OwnedArtboardInstance {
    ...
    pub fn instantiate(file: Arc<File>, artboard_index: usize) -> Result<Self> {
        ...
        #[cfg(feature = "scripting")]
        raw.attach_script_source_file_authority(Rc::new(Arc::clone(&file)));
        Ok(Self {
            raw,
            file,
            artboard_index,
...
impl ViewModelInstance {
    fn commit_mutation(
        &mut self,
        mutate: impl FnOnce(&mut RuntimeOwnedViewModelTransaction, &RuntimeOwnedViewModelHandle) -> bool,
    ) -> bool {
        let Some(mut transaction) = RuntimeOwnedViewModelTransaction::begin() else {
            return false;
        };
        let changed = mutate(&mut transaction, &self.raw);
        if changed {
            transaction.commit();
        }
        changed
    ...
    pub fn set_number(&mut self, name_path: &str, value: f32) -> bool {
        self.commit_mutation(|transaction, owner| transaction.set_number(owner, name_path, value))
    }
    ...
    pub fn set_bool(&mut self, name_path: &str, value: bool) -> bool {
        self.commit_mutation(|transaction, owner| transaction.set_boolean(owner, name_path, value))
    }
    ...
    pub fn set_string(&mut self, name_path: &str, value: &str) -> bool {
        self.commit_mutation(|transaction, owner| {
            transaction.set_string(owner, name_path, value.as_bytes())
        })
    }
    ...
    pub fn set_enum(&mut self, name_path: &str, value: u64) -> bool {
        self.commit_mutation(|transaction, owner| transaction.set_enum(owner, name_path, value))
    }
    ...
    pub fn set_color(&mut self, name_path: &str, argb: u32) -> bool {
        self.commit_mutation(|transaction, owner| transaction.set_color(owner, name_path, argb))
    }
    ...
    pub fn fire_trigger(&mut self, name_path: &str) -> bool {
        self.commit_mutation(|transaction, owner| transaction.fire_trigger(owner, name_path))
    }
...
mod owned_instance_tests {
    ...
    fn facade_two_view_model_file_with_second_property(
        ...
        records.extend([
            ...
            fixture_record(
                ...
                vec![("name", FixtureValue::String("firstOnly".to_owned()))],
            ),
            fixture_record(
                "ViewModel",
                vec![("name", FixtureValue::String("Second".to_owned()))],
            ),
            fixture_record(
            ...
            fixture_record(
                "ViewModelInstance",
                vec![
                    ("name", FixtureValue::String("First defaults".to_owned())),
                    ("viewModelId", FixtureValue::Uint(0)),
                ],
            ),
            fixture_record(
                ...
                vec![
                    ("viewModelPropertyId", FixtureValue::Uint(0)),
                    ("propertyValue", FixtureValue::String("first".to_owned())),
                ],
            ),
            fixture_record(
                "ViewModelInstance",
                vec![
                    ("name", FixtureValue::String("Second defaults".to_owned())),
    ...
    #[cfg(feature = "scripting")]
    fn nested_script_input_context_file() -> File {
        ...
        let runtime = RuntimeFile::from_fixture_records(vec![
            ...
            fixture_record(
                "ScriptedDrawable",
                vec![("parentId", FixtureValue::Uint(0))],
            ),
            fixture_record(
                "ScriptInputNumber",
                vec![
                    ("parentId", FixtureValue::Uint(1)),
                    ("name", FixtureValue::String("local".to_owned())),
            ...
            fixture_record(
                ...
                vec![
                    ("parentId", FixtureValue::Uint(1)),
                    ("name", FixtureValue::String("parent".to_owned())),
    ...
    #[cfg(feature = "scripting")]
    #[test]
    fn nested_script_input_resolution_uses_local_then_parent_data_context() {
        let file = Arc::new(nested_script_input_context_file());
        let mut occurrence = OwnedArtboardInstance::instantiate_default(Arc::clone(&file))
            .expect("nested ScriptInput occurrence");
        let local = RuntimeOwnedViewModelHandle::new(
            RuntimeOwnedViewModelInstance::new(file.runtime(), 0).expect("local model"),
        );
        let parent = RuntimeOwnedViewModelHandle::new(
            RuntimeOwnedViewModelInstance::new(file.runtime(), 1).expect("parent model"),
        );
        assert!(local.borrow_mut().set_number_by_property_index(0, 11.0));
        assert!(parent.borrow_mut().set_number_by_property_index(0, 22.0));
        let parent_context =
            nuxie_runtime::RuntimeOwnedViewModelContextHandle::root(file.runtime(), parent);
        let local_context =
            nuxie_runtime::RuntimeOwnedViewModelContextHandle::root(file.runtime(), local.clone());
        let context = nuxie_runtime::ScriptArtboardParentContext::root(&parent_context)
            .with_local_view_model(&local_context);
        occurrence
            .raw_mut()
            .bind_script_artboard_data_context(file.runtime(), &context);

        let inputs = (0..file.runtime().object_count())
            .filter_map(|id| file.runtime().object(id))
            .filter(|object| object.type_name == "ScriptInputNumber")
            .collect::<Vec<_>>();
        ...
        let values = inputs
            ...
            .map(|input| {
                occurrence
                    .raw()
                    .bound_script_input_value_for_occurrence(file.runtime(), input, Some(&local))
                    .expect("bound ScriptInput")
            })
            .collect::<Vec<_>>();
        assert_eq!(
            values,
            ...
            "the nested occurrence resolves its local model first and falls through to the retained parent node"
        );
    }

    #[cfg(feature = "scripting")]
    struct NestedRehydrateProbe {
        values: Rc<RefCell<Vec<(String, ScriptValue)>>>,
    }

    #[cfg(feature = "scripting")]
    impl ScriptInstance for NestedRehydrateProbe {
        fn has_method(&self, _method: ScriptMethod) -> std::result::Result<bool, ScriptError> {
            Ok(false)
        }
        ...
        fn set_input(
            ...
            self.values.borrow_mut().push((name.to_owned(), value));
            Ok(())
        }
    }
    ...
    #[cfg(feature = "scripting")]
    #[test]
    fn nested_tree_rehydrate_uses_cross_file_source_catalog_and_occurrence_context() {
        let source_file = Arc::new(nested_script_input_context_file());
        let mut source = OwnedArtboardInstance::instantiate_default(Arc::clone(&source_file))
            .expect("cross-File scripted source");
        let local = RuntimeOwnedViewModelHandle::new(
            RuntimeOwnedViewModelInstance::new(source_file.runtime(), 0).expect("local model"),
        );
        let parent = RuntimeOwnedViewModelHandle::new(
            RuntimeOwnedViewModelInstance::new(source_file.runtime(), 1).expect("parent model"),
        );
        assert!(local.borrow_mut().set_number_by_property_index(0, 31.0));
        assert!(parent.borrow_mut().set_number_by_property_index(0, 47.0));
        let parent_context =
            nuxie_runtime::RuntimeOwnedViewModelContextHandle::root(source_file.runtime(), parent);
        let local_context =
            nuxie_runtime::RuntimeOwnedViewModelContextHandle::root(source_file.runtime(), local);
        let source_context = nuxie_runtime::ScriptArtboardParentContext::root(&parent_context)
            .with_local_view_model(&local_context);
        source
            .raw_mut()
            .bind_script_artboard_data_context(source_file.runtime(), &source_context);

        let parent_runtime = RuntimeFile::from_fixture_records(vec![
            fixture_record("Backboard", Vec::new()),
            fixture_record(
                "ViewModel",
                vec![("name", FixtureValue::String("Consumer".to_owned()))],
            ...
            fixture_record(
                "ViewModelPropertyNumber",
                vec![("name", FixtureValue::String("wrong".to_owned()))],
            ),
            fixture_record("Artboard", Vec::new()),
            fixture_record(
                "NestedArtboard",
        ...
        .expect("consumer nested fixture imports");
        let consumer_file =
            Arc::new(File::from_runtime(parent_runtime).expect("consumer nested fixture graph"));
        let consumer_graph = consumer_file
            .graph
            .artboards
            .first()
            .expect("consumer root graph");
        let host_local = consumer_graph
            .nested_artboards
            .first()
            .expect("consumer nested host")
            .local_id;
        let mut consumer = OwnedArtboardInstance::instantiate_default(Arc::clone(&consumer_file))
            .expect("consumer occurrence");
        assert!(
        ...
        let scripted_global = source_file.graph.artboards[0]
            .components
            .iter()
            .find(|component| component.type_name == "ScriptedDrawable")
            .expect("source scripted owner")
            .global_id;
        let values = Rc::new(RefCell::new(Vec::new()));
        consumer
            .raw_mut()
            .try_visit_artboard_tree_instances_mut(&mut |_, _, nested| {
                nested.set_script_instance_for_global(
                    scripted_global,
                    Box::new(NestedRehydrateProbe {
                        values: Rc::clone(&values),
                    }),
                );
                Ok::<(), ScriptError>(())
            })
            .expect("attach nested probe");
        let consumer_root = RuntimeOwnedViewModelHandle::new(
            RuntimeOwnedViewModelInstance::new(consumer_file.runtime(), 0)
                .expect("consumer root model"),
        );
        assert!(
            consumer_root
                .borrow_mut()
                .set_number_by_property_index(0, 99.0)
        );
        let mut factory = script_factory();
        assert!(
            rehydrate_bound_script_input_tree(
            ...
            .expect("cross-File nested rehydrate")
        );
        assert_eq!(
            &*values.borrow(),
            &[
                ("local".to_owned(), ScriptValue::Number(31.0)),
                ("parent".to_owned(), ScriptValue::Number(47.0)),
            ],
    ...
    fn live_scripted_artboard_uses_consumer_file_and_retains_concrete_cross_file_source() {
        ...
        let retained_source = source
            .raw()
            .script_source_file_authority::<Arc<File>>()
            .expect("the concrete occurrence retains its source File");
        assert!(Arc::ptr_eq(retained_source.as_ref(), &source_file));
    ...
    #[cfg(feature = "scripting")]
    #[test]
    fn prepared_live_script_artboard_snapshots_the_occurrence_during_construct() {
        let file = Arc::new(facade_view_model_file());
        let mut first = OwnedArtboardInstance::instantiate_default(Arc::clone(&file))
            .expect("instantiate the first live source");
        assert!(first.raw_mut().set_artboard_dimensions(111.0, 55.0));
        let bindable = nuxie_runtime::RuntimeBindableArtboard::new_with_artboard_instance(
            "source",
            first.raw(),
        );
        let resolver = FileScriptArtboardResolver {
            file: Arc::clone(&file),
        };
        let recipe = nuxie_runtime::ScriptArtboardResolver::prepare_script_artboard(
            &resolver,
            &nuxie_runtime::ScriptArtboardSource::Live(bindable.clone()),
            None,
        )
        .expect("validation retains the live bindable");

        let mut second = OwnedArtboardInstance::instantiate_default(Arc::clone(&file))
            .expect("instantiate the replacement live source");
        assert!(second.raw_mut().set_artboard_dimensions(222.0, 77.0));
        bindable.refresh_artboard_instance(second.raw());

        let scripted = recipe
            .construct()
            .expect("construction snapshots the current occurrence");
        assert_eq!(scripted.width(), 222.0);
        assert_eq!(scripted.height(), 77.0);
    }

    #[cfg(feature = "scripting")]
    #[test]
    fn live_scripted_artboard_selects_consumer_view_model_from_mutated_occurrence() {
        let source_file = Arc::new(facade_two_view_model_file());
        let resolver_file = Arc::new(facade_two_view_model_file());
        let mut source = OwnedArtboardInstance::instantiate_default(Arc::clone(&source_file))
            .expect("instantiate the authored first-model source");
        ...
        assert_eq!(source.raw().debug_uint_property(0, view_model_key), Some(0));
        ...
        let mut scripted =
            FileScriptArtboard::new_from_live(Arc::clone(&resolver_file), &bindable, None)
                .expect("the consumer File wraps the mutated live occurrence");
        let selected = scripted
            .view_model
            .as_ref()
            .expect("the mutated second model is selected")
            .clone();
        assert_eq!(
            selected.property("secondOnly"),
            Some(nuxie_runtime::ScriptViewModelProperty::Number)
        );
        assert_eq!(selected.property("firstOnly"), None);
        let rectangle_width_key = fixture_property(
            "Rectangle",
            "width",
            FixtureValue::Double(0.0),
        )
        .key;
        nuxie_runtime::ScriptArtboard::advance(&mut scripted, 0.0)
            .expect("the selected model advances the cloned occurrence's DataBinds");
        assert_eq!(
            scripted
                .instance
                .double_property(2, rectangle_width_key),
            Some(0.0),
            "the selected second model's numeric property binds to the clone; the immutable first model exposes a string at the same path and could not drive this numeric target"
        );
        assert_eq!(
            source_file
                .runtime()
            ...
            Some(0),
            "the immutable source catalog remains authored to the first model; only the concrete live occurrence changed"
        );
    }
