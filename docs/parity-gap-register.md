...
| V5 | **CLOSED 2026-08-03 (#OR-3): scripted external mutation is differentially covered.** Both runners execute typed `setInput` bool/number/trigger mutations, slash-path view-model bool/number/string/enum/color mutations plus trigger through `--view-model-script`, and logical resize + DPR events. Six exact corpus entries cover every family, a nested view-model path, and cross-stream equal-time ordering. Keyboard/text remain reserved for #FT-TEXT; gamepad and backwards-time scripting remain future grammar additions rather than part of #OR-3's declared close gate. | `docs/side-channel-format.md`; both golden runners; `tools/golden-compare`; `script_verbs_*` corpus entries | **Green:** verb parser/unit tests; six exact scripted corpus entries; corpus-wide `make scripted-golden-compare` at 331/331 exact, 685/685 exact segments, and 684/684 side-channel segments. |
...
| V11 | **`global_variables_test`: Rust misses one root-layer transition at t=0 (found by V4's side channel, 2026-08-02).** C++ `stateChangedCount()`=2 (two layers each entering an AnimationState, coreType 61); Rust `changed_state_count()`=1. Draw streams are identical at the entry's only sample (t=0) but diverge structurally at t>0 (worst numeric delta 32.0 at samples 0.5/1.0) — a real missed transition hidden until now by V2's t=0-only sampling. Localized: the second C++ transition is gated on a `TransitionPropertyViewModelComparator`/`TransitionValueNumberComparator` condition whose Rust-side evaluation timing differs mid-frame (settle probes run unconditionally on main; instrumentation shows the Rust layer never transitions in main advance, components, or settlement). The corpus entry carries `side-channel-diverges:V11`, which suppresses only its side channel; its draw floor still gates. | `corpus.toml` `global_variables_test`; repro: both runners `--samples 0,0.5,1 --side-channel` | Port the pinned mid-frame comparator-evaluation ordering (suspect family: `transition_viewmodel_condition.cpp:49-60` timing vs data-bind passes), then delete the `side-channel-diverges:V11` tag; the entry densifies to multi-sample at close. |
...
| F6 | **Semantics/accessibility** — `semantic_manager` 1,109, `semantic_data` 572, provider, inference registry | 1,926 | PARTIAL (LT1SC) | The retained runtime and LT-1 full diff/action/focus side channel are implemented against `4ac7b327`. The nested focus provider oracle is exact; data_binding_lists still differs on Selected state/removals, and Simpsons still differs on root/pre-layout bounds and Selected updates. The four source rows therefore remain pending. |
