...
pub unsafe fn lua_v_equalval(
    ...
    match ttype!(t1) {
        t if t == lua_Type::LUA_TNIL as i32 => return Ok(1),
        t if t == lua_Type::LUA_TNUMBER as i32 => {
            return Ok(if luai_numeq(nvalue!(t1), nvalue!(t2)) {
                1
            } else {
                0
            });
        ...
        t if t == lua_Type::LUA_TINTEGER as i32 => {
            return Ok(if luai_inteq(lvalue!(t1) as f64, lvalue!(t2) as f64) {
                1
            } else {
                0
            });
        ...
        t if t == lua_Type::LUA_TVECTOR as i32 => {
            return Ok(if luai_veceq(vvalue!(t1).as_ptr(), vvalue!(t2).as_ptr()) {
                1
            } else {
                0
            });
        ...
        t if t == lua_Type::LUA_TBOOLEAN as i32 => {
            return Ok(if bvalue!(t1) == bvalue!(t2) { 1 } else { 0 });
        }
        t if t == lua_Type::LUA_TLIGHTUSERDATA as i32 => {
            return Ok(
                if ((*t1).value.p == (*t2).value.p)
        ...
        t if t == lua_Type::LUA_TUSERDATA as i32 => {
            ...
            if tm.is_null() {
                return Ok(if core::ptr::eq(u1, u2) { 1 } else { 0 });
            }
        }
        t if t == lua_Type::LUA_TCLASS as i32 => {
            return Ok(if core::ptr::eq(classvalue!(t1), classvalue!(t2)) {
                1
            } else {
                0
            });
        ...
        t if t == lua_Type::LUA_TOBJECT as i32 => {
            ...
            if ttisnil!(tm) {
                return Ok(if core::ptr::eq(t1inst, t2inst) { 1 } else { 0 });
            }
        }
        t if t == lua_Type::LUA_TTABLE as i32 => {
            ...
            if tm.is_null() {
                return Ok(if core::ptr::eq(h1, h2) { 1 } else { 0 });
            }
        }
        _ => {
            return Ok(if core::ptr::eq(gcvalue!(t1), gcvalue!(t2)) {
                1
            } else {
                0
            });
    ...
    Ok(if !l_isfalse!((*L).top) { 1 } else { 0 })
}

#[export_name = "luaur_luaV_equalval"]
pub unsafe fn lua_v_equalval_export(
    L: *mut LuaState,
    t1: *const TValue,
    t2: *const TValue,
) -> crate::records::lua_exception::LuaResult<i32> {
