...
use crate::mechanical_port::source::{
    ...
    data_bind::data_values::{
        data_value_asset_image::{DataValueAssetImage, ImageAsset},
...
impl ViewModelInstanceAssetImage {
    ...
    pub fn property_value_changed(&mut self) {
        if let Some(owner) = crate::mechanical_port::source::core::CoreObject::core(self).handle() {
            crate::host_viewmodel::capture_native_change(
                owner,
                crate::RuntimeViewModelChangeValue::Image(self.base.property_value() as u64),
            );
        }
    ...
    pub fn set_value(&mut self, image: Option<Rc<dyn RenderImage>>) {
        ...
        if already_sentinel {
                ...
                crate::mechanical_port::source::core::CoreObject::core(self).handle()
            {
                crate::host_viewmodel::capture_native_change(
                    owner,
                    crate::RuntimeViewModelChangeValue::Image(u32::MAX as u64),
                );
            }
        }
        #[cfg(feature = "tools")]
    ...
    pub fn apply_data_value(
        ...
        if let Some(asset_value) = data_value.as_any().downcast_ref::<DataValueAssetImage>() {
