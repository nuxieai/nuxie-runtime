...
fn main_1() {
    ...
    var phi_1138_: f32;
    var phi_1164_: u32;
    var phi_1162_: vec4<f32>;
    var phi_1179_: u32;
    var phi_1175_: vec4<f32>;
    var phi_1176_: vec3<f32>;
    ...
    if (_e95 == _e96) {
        ...
        phi_1128_ = ((_e96 << bitcast<u32>(17u)) + 65536u);
    }
    let _e102 = phi_1128_;
    let _e103 = i1_1;
    q4_.c2_[_e90] = (_e102 + bitcast<u32>(i32(round((_e103 * 2048f)))));
    phi_1179_ = 0u;
    phi_1175_ = vec4<f32>(0f, 0f, 0f, 0f);
    if (_e95 != _e96) {
        ...
        if ((_e116.x & 768u) != 0u) {
            ...
            phi_789_ = ch;
            if ch {
                phi_789_ = ((_e116.x & 512u) != 0u);
            }
            let _e124 = phi_789_;
            phi_1133_ = _e120;
            if _e124 {
                phi_1133_ = (1f - abs(((fract((_e120 * 0.5f)) * 2f) + -1f)));
            }
            let _e132 = phi_1133_;
            phi_1132_ = _e132;
        }
        ...
        if Yg {
            ...
            if (_e137 != 0u) {
                ...
                if (_e137 == (_e141 >> bitcast<u32>(16i))) {
                    phi_1134_ = min(_e135, unpack2x16float(_e141).x);
                } else {
                    phi_1134_ = 0f;
                }
                let _e149 = phi_1134_;
        ...
        if _e157 {
            ...
            phi_1139_ = min(_e153, clamp((min(_e180.x, _e180.y) + 0.5f), 0f, 1f));
        }
        let _e188 = phi_1139_;
        ...
        if (_e189 <= 1u) {
            ...
            phi_1165_ = 0u;
            if _e194 {
                phi_1165_ = (_e116.y | pack2x16float(vec2<f32>(_e188, 0f)));
            }
            let _e199 = phi_1165_;
            phi_1164_ = _e199;
            phi_1162_ = select(unpack4x8unorm(_e116.y), vec4<f32>(0f, 0f, 0f, 0f), vec4(_e194));
        } else {
            let _e202 = (_e95 * 4u);
        ...
        phi_1179_ = _e235;
        phi_1175_ = vec4<f32>(_e241.x, _e241.y, _e241.z, _e239);
    }
    let _e247 = phi_1179_;
    let _e249 = phi_1175_;
    let _e250 = _e249.xyz;
    ...
    let _e255 = n.A3_;
    if (fh && (_e249.w != 0f)) {
        phi_1176_ = (vec3(((fract((52.982918f * fract(((0.06711056f * _e55.x) + (0.00583715f * _e55.y))))) * _e253) + _e255)) + _e250);
    } else {
        phi_1176_ = _e250;
    }
    let _e271 = phi_1176_;
    let _e277 = vec4<f32>(_e271.x, _e249.y, _e249.z, _e249.w);
    let _e283 = vec4<f32>(_e277.x, _e271.y, _e277.z, _e277.w);
    C1_ = vec4<f32>(_e283.x, _e283.y, _e271.z, _e283.w);
    if (_e247 != 0u) {
        h0_.c2_[_e90] = _e247;
    }
    return;
...
@fragment
fn main(@builtin(position) gl_FragCoord: vec4<f32>, @location(1) @interpolate(flat, either) B0_: u32, @location(0) @interpolate(flat, either) i1_: f32) -> @location(0) vec4<f32> {
