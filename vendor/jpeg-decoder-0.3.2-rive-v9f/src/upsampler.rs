...
impl Upsampler {
    pub fn new(
        ...
        let h_max = components
            .iter()
            .map(|c| c.horizontal_sampling_factor as usize * c.dct_scale_h)
            .max()
            .unwrap();
        let v_max = components
            .iter()
            .map(|c| c.vertical_sampling_factor as usize * c.dct_scale_v)
            .max()
            .unwrap();
        ...
        for component in components {
            let upsampler = choose_upsampler(
                (
                    (component.horizontal_sampling_factor as usize * component.dct_scale_h) as u8,
                    (component.vertical_sampling_factor as usize * component.dct_scale_v) as u8,
                ),
                (h_max as u8, v_max as u8),
                output_width,
                output_height,
            )?;
            upsampler_components.push(UpsamplerComponent {
                ...
                row_stride: component.block_size.width as usize * component.dct_scale_h,
            });
        }
