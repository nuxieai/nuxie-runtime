//! Direct Rust owner for pinned C++ `src/data_bind/data_bind_context.cpp`.
//!
...
/// Target-specific records below remain execution adapters only.
pub(super) type RuntimeArtboardAuthoredDataBindState =
    crate::retained_data_bind::RuntimeAuthoredDataBind<
        ComponentHandle,
        RuntimeOwnedViewModelBindingSource,
        RuntimeArtboardAuthoredSharedConverterState,
    >;
...
pub(super) struct RuntimeArtboardAuthoredDataBindStates {
    ...
    container: RuntimeDataBindContainerQueue,
...
impl RuntimeArtboardAuthoredDataBindStates {
    fn new(mut states: Vec<RuntimeArtboardAuthoredDataBindState>) -> Self {
        ...
        for (data_bind_index, state) in states.iter_mut().enumerate() {
            let occurrence = container.add_data_bind(&mut state.retained, false);
            debug_assert_eq!(occurrence, data_bind_index);
        }
        let mut result = Self {
    ...
    fn take_source_dirt_indices(&mut self) -> Vec<usize> {
        self.container.begin_update(|_| false).unwrap_or_default()
    }

    fn begin_source_dirt_occurrence(&mut self, data_bind_index: usize) {
        self.container.begin_occurrence(data_bind_index);
    }

    fn has_pending_source_dirt(&self) -> bool {
        self.container.has_pending_work() || !self.pending_source_dirt_indices.is_empty()
    }

    fn recycle_source_dirt_indices(&mut self, mut indices: Vec<usize>) {
        indices.clear();
        self.container.finish_update();
    }
...
type RuntimeArtboardAuthoredSharedConverterState =
    crate::retained_data_bind::RuntimeOwnedDataBindConverter<
        RuntimeDataBindGraphConverter,
        RuntimeDataBindGraphConverterState,
    >;
...
#[derive(Debug, Clone)]
pub(crate) struct RuntimeOwnedViewModelBindingSource {
    context: RuntimeOwnedViewModelHandle,
    property_path: Vec<usize>,
...
impl RuntimeOwnedViewModelBindingSource {
    ...
    fn runtime_image(&self) -> Option<RuntimeViewModelImage> {
        self.context
            .borrow()
...
fn runtime_owned_view_model_image_for_data_context(
    file: &RuntimeFile,
    data_context: &RuntimeOwnedDataContext,
    path: &[u32],
    path_is_name_based: bool,
...
impl ArtboardInstance {
    ...
    fn collect_artboard_authored_data_bind_source_dirt(&mut self) -> bool {
        ...
        for data_bind_index in dirty_indices.iter().copied() {
            self.artboard_authored_data_bind_states
                .begin_source_dirt_occurrence(data_bind_index);
            self.publish_artboard_data_bind_target_dirty(data_bind_index);
            let Some((
                to_target,
                path,
                ...
                .and_then(|state| {
                    state
                        ...
                        .map(|primary| {
                            ...
                            let runtime_image = state
                                .source
                                .as_ref()
                                .and_then(RuntimeOwnedViewModelBindingSource::runtime_image);
            ...
            let image_binding_index = self
                .artboard_data_bind_target_queues
                .image_asset_index_for_data_bind(data_bind_index);
            let image_changed = image_binding_index
                .and_then(|index| self.artboard_image_asset_bindings.get(index))
                .filter(|binding| !binding.target.is_font())
                .is_some_and(|binding| {
    ...
    fn bind_owned_view_model_artboard_values(
        ...
        for index in 0..self.artboard_image_asset_bindings.len() {
            let (update, font_update, runtime_image) = {
                ...
                let runtime_image = (!binding.target.is_font())
                    .then(|| {
                        context.runtime_image_by_property_path(
                            binding.owned_context_source_path.as_deref()?,
                        )
                    })
                    .flatten();
                (update, font_update.flatten(), runtime_image)
            };
            if let Some(value) = update {
                ...
                self.artboard_image_asset_bindings[index].runtime_image = runtime_image;
    ...
    fn apply_artboard_image_asset_bindings(&mut self) -> bool {
        ...
        {
            let Some((target, value, font_value, runtime_image)) = self
                .artboard_image_asset_bindings
                .get(index)
                .map(|binding| {
                    let value = self
            ...
            changed |= self.apply_artboard_image_asset_binding_value(
                target,
                &value,
                font_value,
                runtime_image,
...
mod tests {
    ...
    fn retained_source_queue_coalesces_sink_aliases_and_preserves_clone_boundary() {
        ...
        cloned.recycle_source_dirt_indices(clone_pass);
        ...
        let clone_next_pass = cloned.take_source_dirt_indices();
        assert_eq!(clone_next_pass, vec![0]);
        assert_eq!(
            cloned[0].retained.take_source_dirt_with_primary(),
            Some(true)
        );
        cloned.recycle_source_dirt_indices(clone_next_pass);

        assert!(operand.set_value(RuntimeViewModelCellValue::Number(6.0)));
        let clone_operand_pass = cloned.take_source_dirt_indices();
        assert_eq!(clone_operand_pass, vec![0]);
        assert_eq!(
            cloned[0].retained.take_source_dirt_with_primary(),
            Some(false),
            "clone retains a fresh operand sink after carrying in-flight dirt"
        );
        cloned.recycle_source_dirt_indices(clone_operand_pass);
    }
    ...
    fn pending_reconcile_preserves_target_origin_when_operand_repeats_source_dirt() {
        ...
        let mut authored_state = artboard.artboard_authored_data_bind_states[0].clone();
        authored_state.retained = RuntimeRetainedDataBind::new(DATA_BIND_FLAG_TWO_WAY, false);
        artboard.artboard_authored_data_bind_states =
            RuntimeArtboardAuthoredDataBindStates::new(vec![authored_state]);
    ...
    fn image_binding_fixture() -> RuntimeFile {
        ...
        .expect("image binding fixture imports")
    }

    #[derive(Debug)]
    struct BoundTestImage(u32, u32);

    impl nuxie_render_api::RenderImage for BoundTestImage {
    ...
    #[test]
    fn factory_backed_view_model_image_reaches_the_bound_image_draw_owner() {
        ...
        let bound = artboard
            ...
            .get(&1)
            .and_then(RuntimeViewModelImage::render_image)
            .expect("bound Image draw owner retains the runtime image");
