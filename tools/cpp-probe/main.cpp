...
enum class RuntimeStateMachineActionKind
{
    ...
    SetDefaultViewModelSourceNumberByName,
    SetDefaultViewModelSourceBoolean,
    SetDefaultViewModelSourceBooleanByName,
    SetDefaultViewModelSourceString,
    SetDefaultViewModelSourceStringByName,
    SetDefaultViewModelSourceColor,
    SetDefaultViewModelSourceColorByName,
    SetDefaultViewModelSourceEnum,
    SetDefaultViewModelSourceEnumByName,
    SetDefaultViewModelSourceSymbolListIndex,
    SetDefaultViewModelSourceSymbolListIndexByName,
    SetDefaultViewModelSourceAsset,
    SetDefaultViewModelSourceAssetByName,
    SetDefaultViewModelSourceArtboard,
    SetDefaultViewModelSourceArtboardByName,
    ...
    SetOwnedViewModelSourceString,
    SetViewModelInstanceSourceStringByName,
    SetViewModelInstanceSourceColor,
    SetOwnedViewModelSourceColor,
    SetViewModelInstanceSourceColorByName,
    SetViewModelInstanceSourceEnum,
    SetOwnedViewModelSourceEnum,
    ...
    SetOwnedViewModelSourceSymbolListIndex,
    ...
    SetOwnedViewModelSourceList,
    SetViewModelInstanceSourceListByName,
    SetOwnedViewModelSourceViewModel,
    ...
    BindOwnedViewModelBooleanContext,
    BindOwnedViewModelBooleanNamePathContext,
    BindOwnedViewModelImportedIntermediateBooleanNamePathContext,
    BindOwnedViewModelStringContext,
    BindOwnedViewModelStringNamePathContext,
    ...
    BindOwnedViewModelImportedIntermediateEnumNamePathContext,
    BindOwnedViewModelSymbolListIndexContext,
    BindOwnedViewModelSymbolListIndexNamePathContext,
    ...
    BindOwnedViewModelImportedIntermediateTriggerNamePathContext,
...
struct RuntimeStateMachineColorBindingReport
{
    size_t dataBindIndex;
    bool hasSource;
...
struct RuntimeStateMachineEnumBindingReport
{
    size_t dataBindIndex;
    bool hasSource;
    uint32_t sourceValue;
...
struct RuntimeStateMachineSymbolListIndexBindingReport
{
    size_t dataBindIndex;
    bool hasSource;
    uint32_t sourceValue;
...
struct RuntimeStateMachineArtboardBindingReport
{
    size_t dataBindIndex;
    bool hasSource;
    uint32_t sourceValue;
...
std::vector<RuntimeStateMachineNumberBindingReport>
collect_number_binding_reports(rive::StateMachineInstance* stateMachine)
{
    std::vector<RuntimeStateMachineNumberBindingReport> reports;
         ...
         ++dataBindIndex)
    {
        auto dataBind = sourceStateMachine->dataBind(dataBindIndex);
        auto target = dataBind == nullptr ? nullptr : dataBind->target();
        if (target == nullptr || !target->is<rive::BindablePropertyNumber>())
        {
            continue;
        }

        RuntimeStateMachineNumberBindingReport report;
        report.dataBindIndex = dataBindIndex;
        report.hasSource = false;
        report.sourceValue = 0.0f;
        report.hasTarget = false;
        report.targetValue = 0.0f;
...
std::vector<RuntimeStateMachineStringBindingReport>
collect_string_binding_reports(rive::StateMachineInstance* stateMachine)
{
    std::vector<RuntimeStateMachineStringBindingReport> reports;
    if (stateMachine == nullptr)
    {
        return reports;
    }
         ...
         ++dataBindIndex)
    {
        auto dataBind = sourceStateMachine->dataBind(dataBindIndex);
        auto target = dataBind == nullptr ? nullptr : dataBind->target();
        if (target == nullptr || !target->is<rive::BindablePropertyString>())
        {
            continue;
        }

        RuntimeStateMachineStringBindingReport report;
        report.dataBindIndex = dataBindIndex;
        report.hasSource = false;
        report.sourceValue = "";
        report.hasTarget = false;
        ...
        auto bindableProperty = stateMachine->bindablePropertyInstance(
            target->as<rive::BindablePropertyString>());
        auto flags = static_cast<rive::DataBindFlags>(dataBind->flags());
        bool toSource =
            (flags & rive::DataBindFlags::ToSource) ==
            rive::DataBindFlags::ToSource;
        ...
        if (source != nullptr && source->is<rive::ViewModelInstanceString>())
        {
            report.hasSource = true;
            report.sourceValue =
                source->as<rive::ViewModelInstanceString>()->propertyValue();
        }

        if (bindableProperty != nullptr &&
            bindableProperty->is<rive::BindablePropertyString>())
        {
            report.hasTarget = true;
            report.targetValue =
...
std::vector<RuntimeStateMachineColorBindingReport>
collect_color_binding_reports(rive::StateMachineInstance* stateMachine)
{
    std::vector<RuntimeStateMachineColorBindingReport> reports;
    if (stateMachine == nullptr)
    {
        return reports;
    }
         ...
         ++dataBindIndex)
    {
        auto dataBind = sourceStateMachine->dataBind(dataBindIndex);
        auto target = dataBind == nullptr ? nullptr : dataBind->target();
        if (target == nullptr || !target->is<rive::BindablePropertyColor>())
        {
            continue;
        }

        RuntimeStateMachineColorBindingReport report;
        report.dataBindIndex = dataBindIndex;
        report.hasSource = false;
        ...
        auto bindableProperty = stateMachine->bindablePropertyInstance(
            target->as<rive::BindablePropertyColor>());
        auto flags = static_cast<rive::DataBindFlags>(dataBind->flags());
        bool toSource =
            (flags & rive::DataBindFlags::ToSource) ==
            rive::DataBindFlags::ToSource;
        ...
        if (source != nullptr && source->is<rive::ViewModelInstanceColor>())
        {
            report.hasSource = true;
            report.sourceValue = static_cast<uint32_t>(
                source->as<rive::ViewModelInstanceColor>()->propertyValue());
        }

        if (bindableProperty != nullptr &&
            bindableProperty->is<rive::BindablePropertyColor>())
        {
            report.hasTarget = true;
            report.targetValue = static_cast<uint32_t>(
                bindableProperty->as<rive::BindablePropertyColor>()
                    ->propertyValue());
        }
...
std::vector<RuntimeStateMachineEnumBindingReport>
collect_enum_binding_reports(rive::StateMachineInstance* stateMachine)
{
    std::vector<RuntimeStateMachineEnumBindingReport> reports;
    if (stateMachine == nullptr)
    {
        return reports;
    }
         ...
         ++dataBindIndex)
    {
        auto dataBind = sourceStateMachine->dataBind(dataBindIndex);
        auto target = dataBind == nullptr ? nullptr : dataBind->target();
        if (target == nullptr || !target->is<rive::BindablePropertyEnum>())
        {
            continue;
        }

        RuntimeStateMachineEnumBindingReport report;
        report.dataBindIndex = dataBindIndex;
        report.hasSource = false;
        report.sourceValue = 0;
        report.hasTarget = false;
        ...
        auto bindableProperty = stateMachine->bindablePropertyInstance(
            target->as<rive::BindablePropertyEnum>());
        auto flags = static_cast<rive::DataBindFlags>(dataBind->flags());
        bool toSource =
            (flags & rive::DataBindFlags::ToSource) ==
            rive::DataBindFlags::ToSource;
        ...
        if (source != nullptr && source->is<rive::ViewModelInstanceEnum>())
        {
            report.hasSource = true;
            report.sourceValue =
                source->as<rive::ViewModelInstanceEnum>()->propertyValue();
        }

        if (bindableProperty != nullptr &&
            bindableProperty->is<rive::BindablePropertyEnum>())
        {
            report.hasTarget = true;
            report.targetValue =
                bindableProperty->as<rive::BindablePropertyEnum>()
                    ->propertyValue();
        }
...
std::vector<RuntimeStateMachineSymbolListIndexBindingReport>
collect_symbol_list_index_binding_reports(
    rive::StateMachineInstance* stateMachine)
{
    std::vector<RuntimeStateMachineSymbolListIndexBindingReport> reports;
    if (stateMachine == nullptr)
    {
        return reports;
    }
         ...
         ++dataBindIndex)
    {
        ...
        auto target = dataBind == nullptr ? nullptr : dataBind->target();
        if (target == nullptr ||
            !target->is<rive::BindablePropertyInteger>())
        {
            continue;
        }

        RuntimeStateMachineSymbolListIndexBindingReport report;
        report.dataBindIndex = dataBindIndex;
        report.hasSource = false;
        report.sourceValue = 0;
        report.hasTarget = false;
        ...
        auto bindableProperty = stateMachine->bindablePropertyInstance(
            target->as<rive::BindablePropertyInteger>());
        auto flags = static_cast<rive::DataBindFlags>(dataBind->flags());
        bool toSource =
            (flags & rive::DataBindFlags::ToSource) ==
            rive::DataBindFlags::ToSource;
        ...
        if (bindableProperty != nullptr &&
            bindableProperty->is<rive::BindablePropertyInteger>())
        {
            report.hasTarget = true;
            report.targetValue =
                bindableProperty->as<rive::BindablePropertyInteger>()
                    ->propertyValue();
        }
...
std::vector<RuntimeStateMachineArtboardBindingReport>
collect_artboard_binding_reports(rive::StateMachineInstance* stateMachine)
{
    std::vector<RuntimeStateMachineArtboardBindingReport> reports;
    if (stateMachine == nullptr)
    {
        return reports;
    }
         ...
         ++dataBindIndex)
    {
        ...
        auto target = dataBind == nullptr ? nullptr : dataBind->target();
        if (target == nullptr ||
            !target->is<rive::BindablePropertyArtboard>())
        {
            continue;
        }

        RuntimeStateMachineArtboardBindingReport report;
        report.dataBindIndex = dataBindIndex;
        report.hasSource = false;
        report.sourceValue = 0;
        report.hasTarget = false;
        ...
        auto bindableProperty = stateMachine->bindablePropertyInstance(
            target->as<rive::BindablePropertyArtboard>());
        auto flags = static_cast<rive::DataBindFlags>(dataBind->flags());
        bool toSource =
            (flags & rive::DataBindFlags::ToSource) ==
            rive::DataBindFlags::ToSource;
        ...
        if (source != nullptr && source->is<rive::ViewModelInstanceArtboard>())
        {
            report.hasSource = true;
            report.sourceValue =
        ...
        if (bindableProperty != nullptr &&
            bindableProperty->is<rive::BindablePropertyArtboard>())
        {
            report.hasTarget = true;
            report.targetValue =
                                     ...
                                     const LocalIds& localIds)
{
    ...
    for (const auto& action : options.runtimeStateMachineActions)
    {
        ...
        if (action.kind == RuntimeStateMachineActionKind::SetBindableBoolean)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
            ...
            auto target = dataBind == nullptr ? nullptr : dataBind->target();
            if (target != nullptr &&
                target->is<rive::BindablePropertyBoolean>())
            {
                auto bindableProperty = stateMachine->bindablePropertyInstance(
                    target->as<rive::BindablePropertyBoolean>());
                if (bindableProperty != nullptr &&
                ...
                {
                    bindableProperty->as<rive::BindablePropertyBoolean>()
                        ->propertyValue(action.boolValue);
                }
            }
            continue;
        }
        if (action.kind == RuntimeStateMachineActionKind::SetBindableInteger)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind == RuntimeStateMachineActionKind::SetBindableColor)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
            ...
            if (target != nullptr && target->is<rive::BindablePropertyColor>())
            {
                ...
                {
                    bindableProperty->as<rive::BindablePropertyColor>()
                        ->propertyValue(static_cast<int>(action.uintValue));
                }
            }
            continue;
        }
        if (action.kind == RuntimeStateMachineActionKind::SetBindableString)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind == RuntimeStateMachineActionKind::SetBindableEnum)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind == RuntimeStateMachineActionKind::SetBindableAsset)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind == RuntimeStateMachineActionKind::SetBindableArtboard)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind == RuntimeStateMachineActionKind::SetBindableList)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceNumber)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               SetDefaultViewModelSourceNumberByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                ...
                {
                    auto rawSource = viewModelInstance->propertyValue(
                        action.stringValue);
                    if (rawSource == nullptr)
                    {
                             ...
                             propertyIndex++)
                        {
                            ...
                            rawSource = viewModelInstance->propertyValue(
                                static_cast<uint32_t>(propertyIndex));
                            break;
                        }
                    }
                    if (rawSource != nullptr &&
                        rawSource->is<rive::ViewModelInstanceNumber>())
                    {
                        rawSource->as<rive::ViewModelInstanceNumber>()
                            ->propertyValue(action.numberValue);
                    }
                }
            }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetViewModelInstanceSourceNumber)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetOwnedViewModelSourceBoolean)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                ...
                viewModelInstance != nullptr)
            {
                ...
                auto source = context.getViewModelProperty(
                    dataBind->as<rive::DataBindContext>()->sourcePathIds());
                if (source != nullptr &&
                    source->is<rive::ViewModelInstanceBoolean>())
                {
                    source->as<rive::ViewModelInstanceBoolean>()
                        ->propertyValue(action.boolValue);
                }
            }
            continue;
                               ...
                               SetOwnedViewModelSourceBooleanByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                ...
                auto source = runtime.propertyBoolean(action.stringValue);
                if (source != nullptr)
                {
                    source->value(action.boolValue);
                }
            }
            continue;
        }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetViewModelInstanceSourceBoolean)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetViewModelInstanceSourceString)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                ...
                viewModelInstance != nullptr)
            {
                ...
                auto source = context.getViewModelProperty(
                    dataBind->as<rive::DataBindContext>()->sourcePathIds());
                if (source != nullptr &&
                    source->is<rive::ViewModelInstanceString>())
                {
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetOwnedViewModelSourceString)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                ...
                viewModelInstance != nullptr)
            {
                ...
                {
                    source->as<rive::ViewModelInstanceString>()
                        ->propertyValue(action.valueString);
                }
            }
            continue;
        }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetViewModelInstanceSourceColor)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                ...
                viewModelInstance != nullptr)
            {
                ...
                auto source = context.getViewModelProperty(
                    dataBind->as<rive::DataBindContext>()->sourcePathIds());
                if (source != nullptr &&
                    source->is<rive::ViewModelInstanceColor>())
                {
                    source->as<rive::ViewModelInstanceColor>()
                        ->propertyValue(static_cast<int>(action.uintValue));
                }
            }
            continue;
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetOwnedViewModelSourceColor)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               SetViewModelInstanceSourceColorByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                rive::ViewModelInstanceRuntime runtime(
                    rive::ref_rcp(viewModelInstance));
                auto source = runtime.propertyColor(action.stringValue);
                if (source != nullptr)
                {
                    source->value(static_cast<int>(action.uintValue));
                }
            }
            continue;
        }
        if (action.kind ==
            RuntimeStateMachineActionKind::SetViewModelInstanceSourceEnum)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                ...
                viewModelInstance != nullptr)
            {
                ...
                auto source = context.getViewModelProperty(
                    dataBind->as<rive::DataBindContext>()->sourcePathIds());
                if (source != nullptr &&
                    source->is<rive::ViewModelInstanceEnum>())
                {
                    source->as<rive::ViewModelInstanceEnum>()
                        ->propertyValue(action.uintValue);
                }
            }
            continue;
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetOwnedViewModelSourceEnum)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               SetViewModelInstanceSourceEnumByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                rive::ViewModelInstanceRuntime runtime(
                    rive::ref_rcp(viewModelInstance));
                auto source = runtime.propertyEnum(action.stringValue);
                if (source != nullptr)
                {
                    source->valueIndex(action.uintValue);
                }
            }
            continue;
        }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetOwnedViewModelSourceAsset)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               SetViewModelInstanceSourceAssetByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                auto source = viewModelInstance->propertyValue(action.stringValue);
                if (source == nullptr)
                {
                    auto properties = viewModel->properties();
                    for (size_t propertyIndex = 0;
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetViewModelInstanceSourceArtboard)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetViewModelInstanceSourceTrigger)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetOwnedViewModelSourceTrigger)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetViewModelInstanceSourceList)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                ...
                viewModelInstance != nullptr)
            {
                    ...
                    source->is<rive::ViewModelInstanceList>())
                {
                    ...
                    auto itemInstance =
                        rive::make_rcp<rive::ViewModelInstance>();
                    auto itemRuntime =
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetOwnedViewModelSourceList)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetOwnedViewModelSourceViewModel)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                ...
                viewModelInstance != nullptr)
            {
                ...
                auto viewModelSource =
                    source != nullptr &&
                        ...
                        : nullptr;
                auto sourceProperty =
                    viewModelSource != nullptr &&
                            viewModelSource->viewModelProperty() != nullptr &&
                            viewModelSource->viewModelProperty()
                                ->is<rive::ViewModelPropertyViewModel>()
                        ? viewModelSource->viewModelProperty()
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceBoolean)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               SetDefaultViewModelSourceBooleanByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                ...
                else
                {
                    ...
                    if (rawSource != nullptr &&
                        rawSource->is<rive::ViewModelInstanceBoolean>())
                    {
                        rawSource->as<rive::ViewModelInstanceBoolean>()
                            ->propertyValue(action.boolValue);
                    }
                }
            }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceString)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               SetDefaultViewModelSourceStringByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                ...
                else
                {
                    ...
                    if (rawSource != nullptr &&
                        rawSource->is<rive::ViewModelInstanceString>())
                    {
                        rawSource->as<rive::ViewModelInstanceString>()
                            ->propertyValue(action.valueString);
                    }
                }
            }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceColor)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               SetDefaultViewModelSourceColorByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                ...
                else
                {
                    ...
                    if (rawSource != nullptr &&
                        rawSource->is<rive::ViewModelInstanceColor>())
                    {
                        rawSource->as<rive::ViewModelInstanceColor>()
                            ->propertyValue(
                                static_cast<int>(action.uintValue));
                    }
                }
            }
            continue;
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceEnum)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               SetDefaultViewModelSourceEnumByName)
        {
            ...
            if (viewModelInstance != nullptr)
            {
                ...
                else
                {
                    ...
                    if (rawSource != nullptr &&
                        rawSource->is<rive::ViewModelInstanceEnum>())
                    {
                        rawSource->as<rive::ViewModelInstanceEnum>()
                            ->propertyValue(action.uintValue);
                    }
                }
            }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceAsset)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceAssetByName)
        {
            auto viewModel =
                file != nullptr && file->viewModelCount() > 0
                    ? file->viewModel(size_t{0})
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceArtboard)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceTrigger)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceTriggerByName)
        {
            auto viewModel =
                file != nullptr && file->viewModelCount() > 0
                    ? file->viewModel(size_t{0})
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceList)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::SetDefaultViewModelSourceViewModel)
        {
            auto sourceStateMachine = stateMachine->stateMachine();
            auto dataBind =
                sourceStateMachine == nullptr
                               ...
                               BindCreatedDefaultViewModelContext)
        {
            auto viewModelInstance =
                file != nullptr
                    ? file->createViewModelInstance(instance)
                    : nullptr;
            if (viewModelInstance != nullptr)
            {
            ...
            RuntimeStateMachineActionKind::BindOwnedViewModelNumberContext)
        {
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto number = runtime.propertyNumber(property->name());
                if (number != nullptr)
                {
                               ...
                               BindOwnedViewModelNumberNamePathContext)
        {
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto number = runtime.propertyNumber(action.stringValue);
                if (number != nullptr)
                {
                    number->value(action.numberValue);
                }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelBooleanContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto boolean = runtime.propertyBoolean(property->name());
                if (boolean != nullptr)
                {
                    boolean->value(action.boolValue);
                }
                stateMachine->bindViewModelInstance(viewModelInstance);
                activeOwnedViewModelInstances[action.stateMachineIndex] =
                    viewModelInstance;
                               ...
                               BindOwnedViewModelBooleanNamePathContext)
        {
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto boolean = runtime.propertyBoolean(action.stringValue);
                if (boolean != nullptr)
                {
                    boolean->value(action.boolValue);
                }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelStringContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto string = runtime.propertyString(property->name());
                if (string != nullptr)
                {
                               ...
                               BindOwnedViewModelStringNamePathContext)
        {
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto string = runtime.propertyString(action.stringValue);
                if (string != nullptr)
                {
                    string->value(action.valueString);
                }
                stateMachine->bindViewModelInstance(viewModelInstance);
            }
            continue;
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelColorContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto color = runtime.propertyColor(property->name());
                if (color != nullptr)
                {
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelEnumContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto enumValue = runtime.propertyEnum(property->name());
                if (enumValue != nullptr)
                {
                    enumValue->valueIndex(action.uintValue);
                }
                stateMachine->bindViewModelInstance(viewModelInstance);
                activeOwnedViewModelInstances[action.stateMachineIndex] =
                    viewModelInstance;
                               ...
                               BindOwnedViewModelEnumNamePathContext)
        {
            ...
            {
                rive::ViewModelInstanceRuntime runtime(viewModelInstance);
                auto enumValue = runtime.propertyEnum(action.stringValue);
                if (enumValue != nullptr)
                {
                    enumValue->valueIndex(action.uintValue);
                }
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelAssetContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelArtboardContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelViewModelDefaultContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelDeepViewModelContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
            ...
            auto leafProperty =
                middleViewModel != nullptr
                    ? middleViewModel->property(action.viewModelInstanceIndex)
                    : nullptr;
            if (viewModelInstance != nullptr && rootProperty != nullptr &&
                middleProperty != nullptr && leafProperty != nullptr &&
                               ...
                               BindOwnedViewModelImportedIntermediateViewModelContext)
        {
            ...
            auto nestedProperty =
                childViewModel != nullptr
                    ...
                    : nullptr;
            if (viewModelInstance != nullptr && rootProperty != nullptr &&
                childInstance != nullptr && nestedProperty != nullptr &&
                nestedProperty->is<rive::ViewModelPropertyViewModel>())
            {
                auto childRuntime =
                    rive::make_rcp<rive::ViewModelInstanceRuntime>(
        ...
        if (action.kind ==
            RuntimeStateMachineActionKind::BindOwnedViewModelTriggerContext)
        {
            auto viewModel =
                file != nullptr && action.viewModelIndex < file->viewModelCount()
                    ? file->viewModel(action.viewModelIndex)
        ...
        report.colorBindings =
            collect_color_binding_reports(stateMachine.get());
        report.enumBindings =
    ...
    const std::vector<RuntimeStateMachineAdvanceReport>& reports)
{
    ...
    for (size_t i = 0; i < reports.size(); ++i)
    {
        ...
        out << "],\"numberBindings\":[";
        for (size_t j = 0; j < report.numberBindings.size(); ++j)
        {
            if (j != 0)
            {
                out << ',';
            }
            const auto& binding = report.numberBindings[j];
            out << "{\"dataBindIndex\":" << binding.dataBindIndex;
            out << ",\"sourceValue\":";
            if (binding.hasSource)
            {
        ...
        out << "],\"stringBindings\":[";
        for (size_t j = 0; j < report.stringBindings.size(); ++j)
        {
            if (j != 0)
            {
                out << ',';
            }
            const auto& binding = report.stringBindings[j];
            out << "{\"dataBindIndex\":" << binding.dataBindIndex;
            out << ",\"sourceValue\":";
            if (binding.hasSource)
            {
                write_json_string(out, binding.sourceValue);
            }
            else
            {
                out << "null";
        ...
        out << "],\"colorBindings\":[";
        for (size_t j = 0; j < report.colorBindings.size(); ++j)
        {
            if (j != 0)
            {
                out << ',';
            }
            const auto& binding = report.colorBindings[j];
            out << "{\"dataBindIndex\":" << binding.dataBindIndex;
            out << ",\"sourceValue\":";
            if (binding.hasSource)
            {
        ...
        out << "],\"enumBindings\":[";
        for (size_t j = 0; j < report.enumBindings.size(); ++j)
        {
            if (j != 0)
            {
                out << ',';
            }
            const auto& binding = report.enumBindings[j];
            out << "{\"dataBindIndex\":" << binding.dataBindIndex;
            out << ",\"sourceValue\":";
            if (binding.hasSource)
            {
        ...
        out << "],\"symbolListIndexBindings\":[";
        for (size_t j = 0; j < report.symbolListIndexBindings.size(); ++j)
        {
            if (j != 0)
            {
                out << ',';
        ...
        out << "],\"artboardBindings\":[";
        for (size_t j = 0; j < report.artboardBindings.size(); ++j)
        {
            if (j != 0)
            {
                out << ',';
                    ...
                    const ProbeOptions& options)
{
    ...
    if (options.runtimeUpdateArtboardDataBinds)
    {
        artboard->updateDataBinds(true);
    }
    if (options.runtimeAdvanceArtboardAfterBind)
    {
        artboard->advance(0.0f);
    }
...
void write_f12_profiler_oracle(std::ostream& out)
{
    std::vector<uint8_t> bytes;
    {
        rive::VectorBinaryWriter writer(&bytes);
        auto writeInt64 = [](rive::VectorBinaryWriter& target, int64_t value) {
            target.write(reinterpret_cast<const uint8_t*>(&value), sizeof(value));
        };

        // Exact writeProfileHeader sequence from pinned rive_profile.cpp.
        writer.write(uint32_t(0x52505246));
        writer.write(uint32_t(2));
        writeInt64(writer, 1000000);
        writer.writeVarUint(uint32_t(1));
        writer.write(std::string("advance"));
        writer.write(uint32_t(2));
        writer.write(uint32_t(0x112233));
        writer.writeVarUint(uint32_t(1));
        writer.write(std::string("runtime"));

        // Exact writeFrameEvents payload sequence from the same source.
        std::vector<uint8_t> payload;
        rive::VectorBinaryWriter payloadWriter(&payload);
        writeInt64(payloadWriter, 100);
        writeInt64(payloadWriter, 140);
        payloadWriter.writeVarUint(uint32_t(2));
        payloadWriter.write(uint8_t(0));
        payloadWriter.writeVarUint(uint32_t(0));
        writeInt64(payloadWriter, 5);
        payloadWriter.write(uint8_t(1));
        payloadWriter.writeVarUint(uint32_t(0));
        writeInt64(payloadWriter, 35);
        writer.write(uint8_t(0x01));
        writer.writeVarUint(static_cast<uint32_t>(payload.size()));
        writer.write(payload.data(), payload.size());
    }

    rive::TransitionRecord transition;
    transition.artboardId = 0;
    transition.smId = 1;
    transition.layerId = 2;
    ...
    listener.smId = 1;
    listener.listenerNameId = 7;
    listener.listenerType = 6;
    listener.hitEvent = 2;
    ...
    writeBytes(bytes);
    out << ",\"transition\":{";
    out << "\"artboardId\":" << transition.artboardId;
    out << ",\"smId\":" << transition.smId;
    out << ",\"layerId\":" << transition.layerId;
    out << ",\"fromStateId\":" << transition.fromStateId;
    out << ",\"toStateId\":" << transition.toStateId;
    out << ",\"tick\":" << transition.tick;
    out << ",\"path\":[";
    for (size_t i = 0; i < transition.path.size(); ++i)
    {
        if (i != 0)
            out << ',';
        const auto& segment = transition.path[i];
        out << "{\"type\":" << static_cast<unsigned>(segment.type);
        out << ",\"nameId\":" << segment.nameId;
        out << ",\"index\":" << segment.index << '}';
    }
    out << "]},\"listener\":{";
    out << "\"artboardId\":" << listener.artboardId;
    out << ",\"smId\":" << listener.smId;
    out << ",\"listenerNameId\":" << listener.listenerNameId;
    out << ",\"listenerType\":" << listener.listenerType;
    out << ",\"hitEvent\":" << listener.hitEvent;
...
int main(int argc, const char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (is_arg(argv[i], "--runtime-advance-artboard-after-bind"))
        {
            options.runtimeAdvanceArtboardAfterBind = true;
            continue;
        }

        if (is_arg(argv[i], "--runtime-update-artboard-data-binds"))
        {
            options.runtimeUpdateArtboardDataBinds = true;
            continue;
        }
        ...
        if (is_arg(argv[i], "--runtime-update-state-machine-data-binds"))
        {
            if (i + 1 >= argc)
            {
                std::cerr << "--runtime-update-state-machine-data-binds requires stateMachineIndex\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-state-machine-bindable-bool"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-state-machine-bindable-bool requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::SetBindableBoolean;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.inputIndex = 0;
            action.dataBindIndex =
        ...
        if (is_arg(argv[i], "--runtime-set-state-machine-bindable-integer"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-state-machine-bindable-integer requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::SetBindableInteger;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.inputIndex = 0;
            action.dataBindIndex =
        ...
        if (is_arg(argv[i], "--runtime-set-state-machine-bindable-color"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-state-machine-bindable-color requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::SetBindableColor;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.inputIndex = 0;
            action.dataBindIndex =
        ...
        if (is_arg(argv[i], "--runtime-set-state-machine-bindable-string"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-state-machine-bindable-string requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::SetBindableString;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.inputIndex = 0;
            action.dataBindIndex =
        ...
        if (is_arg(argv[i], "--runtime-set-state-machine-bindable-enum"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-state-machine-bindable-enum requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::SetBindableEnum;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.inputIndex = 0;
            action.dataBindIndex =
        ...
        if (is_arg(argv[i], "--runtime-set-state-machine-bindable-asset"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-state-machine-bindable-asset requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::SetBindableAsset;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.inputIndex = 0;
            action.dataBindIndex =
        ...
        if (is_arg(argv[i], "--runtime-set-state-machine-bindable-artboard"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-state-machine-bindable-artboard requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::SetBindableArtboard;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.inputIndex = 0;
            action.dataBindIndex =
        ...
        if (is_arg(argv[i], "--runtime-set-state-machine-bindable-list"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-state-machine-bindable-list requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::SetBindableList;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.inputIndex = 0;
            action.dataBindIndex =
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-number"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-number requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-number-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-number-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-bool"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-bool requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-bool-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-bool-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-string"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-string requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-string-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-string-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-color"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-color requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-color-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-color-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-enum"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-enum requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-enum-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-enum-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-symbol-list-index"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-symbol-list-index requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-symbol-list-index-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-symbol-list-index-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-asset"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-asset requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-asset-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-asset-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-artboard"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-artboard requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-artboard-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-artboard-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-trigger"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-trigger requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-default-view-model-source-trigger-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-trigger-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-default-view-model-source-list"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-list requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
                   ...
                   "--runtime-set-default-view-model-source-viewmodel"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-default-view-model-source-viewmodel requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-view-model-instance-source-number"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-number requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-owned-view-model-source-bool"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-bool requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(
                argv[i],
                "--runtime-set-owned-view-model-source-bool-by-name"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-bool-by-name requires stateMachineIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
                   ...
                   "--runtime-set-view-model-instance-source-number-by-name"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-number-by-name requires stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-view-model-instance-source-bool"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-bool requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-set-view-model-instance-source-bool-by-name"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-bool-by-name requires stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-view-model-instance-source-string"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-string requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-owned-view-model-source-string"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-string requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-view-model-instance-source-color"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-color requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-owned-view-model-source-color"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-color requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-view-model-instance-source-color-by-name"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-color-by-name requires stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-view-model-instance-source-enum"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-enum requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-owned-view-model-source-enum"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-enum requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-view-model-instance-source-enum-by-name"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-enum-by-name requires stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-set-view-model-instance-source-symbol-list-index"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-symbol-list-index requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-set-owned-view-model-source-symbol-list-index"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-symbol-list-index requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-owned-view-model-source-asset"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-asset requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-view-model-instance-source-asset-by-name"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-asset-by-name requires stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
                RuntimeStateMachineActionKind::SetViewModelInstanceSourceAssetByName;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.viewModelIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
        ...
        if (is_arg(argv[i],
                   "--runtime-set-view-model-instance-source-artboard"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-artboard requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
                   ...
                   "--runtime-set-view-model-instance-source-artboard-by-name"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-artboard-by-name requires stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i], "--runtime-set-view-model-instance-source-trigger"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-trigger requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-owned-view-model-source-trigger"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-trigger requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-set-view-model-instance-source-trigger-by-name"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-trigger-by-name requires stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
                RuntimeStateMachineActionKind::SetViewModelInstanceSourceTriggerByName;
            action.stateMachineIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
            action.viewModelIndex =
                static_cast<size_t>(std::strtoull(argv[++i], nullptr, 10));
        ...
        if (is_arg(argv[i], "--runtime-set-view-model-instance-source-list"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-set-view-model-instance-source-list requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-owned-view-model-source-list"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-list requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i], "--runtime-set-owned-view-model-source-viewmodel"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-set-owned-view-model-source-viewmodel requires stateMachineIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-relink-view-model-instance-source-viewmodel"))
        {
            if (i + 5 >= argc)
            {
                std::cerr << "--runtime-relink-view-model-instance-source-viewmodel requires stateMachineIndex viewModelIndex instanceIndex dataBindIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-created-default-view-model-state-machine-context"))
        {
            if (i + 1 >= argc)
            {
                std::cerr << "--runtime-bind-created-default-view-model-state-machine-context requires stateMachineIndex\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-bind-owned-view-model-number-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-number-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-number-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-number-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-imported-intermediate-number-name-path-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-number-name-path-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-bind-owned-view-model-bool-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-bool-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
            ...
            action.viewModelInstanceIndex = 0;
            action.seconds = 0.0f;
            action.boolValue = std::strtoull(argv[++i], nullptr, 10) != 0;
            action.numberValue = 0.0f;
            options.runtimeStateMachineActions.push_back(action);
            continue;
        }
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-bool-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-bool-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-imported-intermediate-bool-name-path-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-bool-name-path-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-bind-owned-view-model-string-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-string-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-string-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-string-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-imported-intermediate-string-name-path-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-string-name-path-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-bind-owned-view-model-color-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-color-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-color-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-color-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
                   ...
                   "--runtime-bind-owned-view-model-enum-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-enum-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-enum-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-enum-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-imported-intermediate-enum-name-path-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-enum-name-path-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-bind-owned-view-model-asset-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-asset-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-asset-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-asset-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-symbol-list-index-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-symbol-list-index-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-imported-intermediate-symbol-list-index-name-path-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-symbol-list-index-name-path-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
                   ...
                   "--runtime-bind-owned-view-model-artboard-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-artboard-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-bind-owned-view-model-viewmodel-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-viewmodel-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(argv[i],
                   "--runtime-bind-owned-view-model-deep-viewmodel-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-deep-viewmodel-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex middlePropertyIndex leafPropertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(argv[i],
                   "--runtime-bind-owned-view-model-imported-intermediate-viewmodel-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-viewmodel-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex nestedPropertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-artboard-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-artboard-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-imported-intermediate-artboard-name-path-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-artboard-name-path-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
                   ...
                   "--runtime-bind-owned-view-model-viewmodel-default-state-machine-context"))
        {
            if (i + 3 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-viewmodel-default-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
                   ...
                   "--runtime-bind-owned-view-model-trigger-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-trigger-state-machine-context requires stateMachineIndex viewModelIndex propertyIndex value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind =
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-imported-intermediate-trigger-name-path-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-trigger-name-path-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-list-name-path-state-machine-context"))
        {
            if (i + 4 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-list-name-path-state-machine-context requires stateMachineIndex viewModelIndex propertyPath itemCount\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
        ...
        if (is_arg(
                argv[i],
                "--runtime-bind-owned-view-model-imported-intermediate-list-name-path-state-machine-context"))
        {
            if (i + 6 >= argc)
            {
                std::cerr << "--runtime-bind-owned-view-model-imported-intermediate-list-name-path-state-machine-context requires stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath itemCount\n";
                return 2;
            }
            RuntimeStateMachineAction action;
            action.kind = RuntimeStateMachineActionKind::
    ...
    if (filename == nullptr)
    {
        ...
        std::cerr << "additional runtime flag: --runtime-bind-owned-view-model-imported-intermediate-number-name-path-state-machine-context stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
        std::cerr << "additional runtime flag: --runtime-bind-owned-view-model-bool-name-path-state-machine-context stateMachineIndex viewModelIndex propertyPath value\n";
        std::cerr << "additional runtime flag: --runtime-bind-owned-view-model-imported-intermediate-bool-name-path-state-machine-context stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-bind-owned-view-model-imported-intermediate-enum-name-path-state-machine-context stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-bind-owned-view-model-imported-intermediate-artboard-name-path-state-machine-context stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-bind-owned-view-model-imported-intermediate-trigger-name-path-state-machine-context stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-bind-owned-view-model-imported-intermediate-list-name-path-state-machine-context stateMachineIndex viewModelIndex rootPropertyIndex childInstanceIndex propertyPath itemCount\n";
        ...
        std::cerr << "additional runtime flag: --runtime-bind-created-default-view-model-state-machine-context stateMachineIndex\n";
        ...
        std::cerr << "additional runtime flag: --runtime-set-owned-view-model-source-bool stateMachineIndex dataBindIndex value\n";
        std::cerr << "additional runtime flag: --runtime-set-owned-view-model-source-bool-by-name stateMachineIndex propertyName value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-set-owned-view-model-source-string stateMachineIndex dataBindIndex value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-set-owned-view-model-source-color stateMachineIndex dataBindIndex value\n";
        std::cerr << "additional runtime flag: --runtime-set-view-model-instance-source-color-by-name stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-set-owned-view-model-source-enum stateMachineIndex dataBindIndex value\n";
        std::cerr << "additional runtime flag: --runtime-set-view-model-instance-source-enum-by-name stateMachineIndex viewModelIndex instanceIndex propertyName value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-set-owned-view-model-source-symbol-list-index stateMachineIndex dataBindIndex value\n";
        ...
        std::cerr << "additional runtime flag: --runtime-set-owned-view-model-source-list stateMachineIndex dataBindIndex value\n";
        std::cerr << "additional runtime flag: --runtime-set-owned-view-model-source-viewmodel stateMachineIndex dataBindIndex value\n";
