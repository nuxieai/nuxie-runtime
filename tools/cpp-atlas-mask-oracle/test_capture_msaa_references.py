...
class CaptureMsaaReferencesTest(unittest.TestCase):
    ...
    def test_runs_bounded_parallel_capture_and_installs_provenance(self) -> None:
        ...
        self.assertNotIn("registry_sha256=", provenance)
        self.assertIn("runtime_revision=" + RUNTIME_REVISION + "\n", provenance)
        self.assertIn("dawn_revision=" + DAWN_REVISION + "\n", provenance)
        self.assertIn("provenance_version=2\n", provenance)
        self.assertEqual(provenance.count("case_identity_sha256="), 1)
        self.assertIn("frame_width=2\n", provenance)
        self.assertIn("frame_height=3\n", provenance)
        self.assertIn("sample_count=4\n", provenance)
