//! Pinned `src/script_input_number.cpp` property semantics.
...
pub(crate) fn value_property_key() -> Option<u16> {
    crate::properties::property_key_for_name("ScriptInputNumber", "propertyValue")
}
...
pub(crate) fn authored_target(
    ...
    (value_property_key().map(u32::from) == Some(property_key)).then(|| {
        RuntimeDataBindGraphValue::Number(input.double_property("propertyValue").unwrap_or(0.0))
    })
}
