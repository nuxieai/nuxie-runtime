...
| V16 | **PARTIAL 2026-08-03: authored keyed-object/property order is retained, but `bankcard` still has a Rive-owned inner-feather numeric remainder.** The rejected suffix-settlement prototype made effect/feather repair a global post-wave scan, contrary to RF-29's explicit dependency-node requirement, and therefore was removed. | `corpus.toml` milestone `V16-feather-remainder`, samples 0/1/2; targeted C++/Rust stream comparison including side channel | **Row remainder open:** one inner-feather text contour at t=2 differs by at most 0.014065 after sequential sampling. Keep the row divergent until ShapePaint/Feather are explicit retained dependency nodes and the stream is exact; do not hide Rive-owned output behind tolerance. |
| V17 | **PARTIAL 2026-08-03: the Skin contract is reconciled; `bullet_man` retains a separately named TrimPath remainder.** Skin depends on each tendon bone and every peer-constraint parent, snapshots `bone.world * inverseBind` in tendon order, and Weight/CubicWeight deformation reads only that Skin-owned buffer. The first remaining t=0.5 mismatch is path 19 in the Skin-free `Sparks` child: Rust trims the authored two-point path to endpoints near 6.45/10.43 while C++ remains near 15.29/15.31. | focused Skin dependency/buffer/deformation tests; `corpus.toml` milestone `V17-trim-remainder`, samples 0/0.5/1 | **Skin gate green; row remainder open:** localize the nested `On_fire` TrimPath timing/output seam, keep the 0.0005 contract, then restore the corpus row to `exact`. |
