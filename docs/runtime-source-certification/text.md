...
animated advance all call that owner; controlled sizing reads the retained
tuple instead of repolling a newer Taffy result. Focused evidence
...
real direct-child propagation boundary. The disabled empty body remains red.
No consumer moved; topology remains four pass, three executable expected-red,
and 11 pending.
...
| 10 | `controlSize` (160; disabled 1446) | On any width/height/scale/direction change, retain all five values then `markShapeDirty(false)`; disabled is inert. | Retained owner `components.rs::RuntimeTextState::retain_control_size`; callback `text/text.rs::control_size`; direct/participant callers `draw.rs::ArtboardInstance::retain_runtime_layout_component_bounds`, `artboard.rs::ArtboardInstance::invalidate_runtime_layout_text_host`; retained consumer `draw.rs::ArtboardInstance::runtime_text_layout_constraint`; evidence `text.rs::cxx_control_size_retains_all_fields_before_no_layout_shape_dirt`, `draw.rs::layout_size_change_does_not_control_text_behind_a_solo` | **exact enabled-path correction candidate; disabled path missing**: one occurrence-owned tuple compares and publishes all five fields before no-layout shape dirt; direct and participant Taffy seams call it, and controlled consumers prefer that retained value. The disabled no-op body remains red. |
