...
//! every rive-render-api trait call to a caller-provided C vtable.
//!
...
/// C mirror of [`ImageSampler`] using the `RiveImageWrap`/`RiveImageFilter`
/// enum values.
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct RiveImageSampler {
    pub wrap_x: u8,
    pub wrap_y: u8,
    pub filter: u8,
}

impl From<ImageSampler> for RiveImageSampler {
...
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct RiveRenderCallbacks {
    pub user_data: *mut c_void,

    // Factory calls.
    pub make_render_path:
        Option<unsafe extern "C" fn(*mut c_void, *const RiveRawPathView, u8) -> u64>,
    ...
    pub render_path_add_raw_path:
        Option<unsafe extern "C" fn(*mut c_void, u64, *const RiveRawPathView)>,
    ...
    pub draw_image: Option<unsafe extern "C" fn(*mut c_void, u64, RiveImageSampler, u8, f32)>,
    #[allow(clippy::type_complexity)]
    pub draw_image_mesh: Option<
        unsafe extern "C" fn(*mut c_void, u64, RiveImageSampler, u64, u64, u64, u32, u32, u8, f32),
    >,
    pub modulate_opacity: Option<unsafe extern "C" fn(*mut c_void, f32)>,
}

impl Default for RiveRenderCallbacks {
...
macro_rules! call {
    ($callbacks:expr, $field:ident $(, $arg:expr)*) => {
        if let Some(callback) = $callbacks.$field {
            // SAFETY: the caller of `rive_artboard_instance_draw` guarantees
...
fn with_raw_path_view<R>(path: &RawPath, action: impl FnOnce(*const RiveRawPathView) -> R) -> R {
...
impl CallbackFactory {
    pub(crate) fn new(callbacks: RiveRenderCallbacks) -> Self {
        Self { callbacks }
    }
}
