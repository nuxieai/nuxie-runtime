...
fn native_input_semantic_text_updates_and_redacts_secure_values() {
    ...
    let input = artboard.native_handle().with_artboard(|artboard| {
        artboard
            ...
            .cloned()
            .expect("fixture has a native input")
    });
    ...
    assert!(
        observations
            .iter()
            .all(|text| !text.value.contains("changed"))
    );
...
fn nested_native_input_inspection_keeps_occurrence_values_and_redaction_separate() {
    ...
    let children = artboard.native_handle().with_artboard(|artboard| {
        artboard
            ...
            .filter_map(|object| {
                object
                    .with_downcast::<NestedArtboard, _>(|nested| nested.artboard_instance_default())
                    .flatten()
            })
            .collect::<Vec<_>>()
    });
    ...
    for (index, child) in children.iter().enumerate() {
        ...
        assert!(CoreRegistry::set_bool_handle(
            &input,
            i32::from(TextInputBase::OBSCURED_PROPERTY_KEY),
            index == 0
        ));
    }
    artboard.advance(0.0).unwrap();
    let observations = artboard.semantic_text_with_bounds();
    let secure = observations
        .iter()
        .find(|text| text.value == "[REDACTED]")
        .unwrap();
    let plain = observations
        .iter()
        .find(|text| text.value == "occurrence-1")
        .unwrap();
