...
spec = importlib.util.spec_from_file_location('opacity_audit', Path(__file__).with_name('opacity-gallery-reference.py'))
...
with tempfile.TemporaryDirectory(prefix='opacity-audit-test-') as folder:
    folder = Path(folder)
    cases = []
    asset_request = None
    for index, ((name, width), ref) in enumerate(refs.items()):
        ...
        Path(str(prefix) + '.requirements.json').write_text(json.dumps(fixture['runtimeRequirements']))
        for ext in ['.riv']:
    ...
    assert result['reviewedPairs'] == 264 and not result['remaining']
    ...
    result = module.audit(root, folder)
    assert result['reviewedPairs'] == 263 and len(result['remaining']) == 1
    target.unlink()
...
print(json.dumps(dict(status='verifier-controls-pass', exactPairs=264, changedAssetRejected=True, changedImageUnreviewed=True, incompleteGalleryRejected=True)))
