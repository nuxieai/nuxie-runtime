...
impl ScrollBarConstraint {
    ...
    pub fn draggables(&mut self) -> Vec<Box<dyn DraggableProxy>> {
        ...
        if self.base.parent().parent().is_some_and(|parent| parent.is::<LayoutComponent>()) {
            let track = self.base.parent_mut().parent_mut().unwrap().as_mut::<LayoutComponent>().unwrap();
    ...
    pub fn constrain(&mut self, component: &mut TransformComponent) {
        let Some(scroll_pointer) = self.scroll_constraint else { return; };
        ...
        if self.base.constrains_horizontal() {
            ...
            thumb_offset_x = if scroll.max_offset_x() == 0.0 { 0.0 } else { scroll.clamped_offset_x() / scroll.max_offset_x() * max_thumb_offset };
            if thumb_offset_x < 0.0 { thumb_width += thumb_offset_x; thumb_offset_x = 0.0; }
            else if thumb_offset_x > max_thumb_offset { thumb_width -= thumb_offset_x - max_thumb_offset; thumb_offset_x = if self.base.auto_size() { thumb_offset_x } else { max_thumb_offset }; }
            if self.base.auto_size() { self.thumb_mut().set_forced_width(thumb_width); }
        }
        if self.base.constrains_vertical() {
            ...
            thumb_offset_y = if scroll.max_offset_y() == 0.0 { 0.0 } else { scroll.clamped_offset_y() / scroll.max_offset_y() * max_thumb_offset };
            if thumb_offset_y < 0.0 { thumb_height += thumb_offset_y; thumb_offset_y = 0.0; }
            else if thumb_offset_y > max_thumb_offset { thumb_height -= thumb_offset_y - max_thumb_offset; thumb_offset_y = if self.base.auto_size() { thumb_offset_y } else { max_thumb_offset }; }
            if self.base.auto_size() { self.thumb_mut().set_forced_height(thumb_height); }
        }
        let target_transform = Mat2D::multiply(*component.world_transform(), Mat2D::from_translate(thumb_offset_x, thumb_offset_y));
        TransformConstraint::constrain_world(component, *component.world_transform(), self.components_a, target_transform, self.components_b, self.base.strength());
    }

    pub fn scroll_constraint_mut(&mut self) -> &mut ScrollConstraint { unsafe { &mut *self.scroll_constraint.unwrap() } }
    pub fn set_scroll_constraint(&mut self, constraint: &mut ScrollConstraint) { self.scroll_constraint = Some(constraint); }

    pub fn build_dependencies(&mut self) {
        unsafe { (*self.scroll_constraint.unwrap()).add_dependent(self.base.as_component_mut_ptr()) };
    ...
    pub fn on_added_dirty(&mut self, context: &mut CoreContext) -> StatusCode {
        ...
        if result != StatusCode::Ok { return result; }
        let object = context.resolve_mut(self.base.scroll_constraint_id());
        let Some(scroll) = object.and_then(|object| object.as_mut::<ScrollConstraint>()) else { return StatusCode::MissingObject; };
    ...
    pub fn hit_track(&mut self, world_position: Vec2D) {
        let Some(scroll_pointer) = self.scroll_constraint else { return; };
        let Some(inverse_world) = self.track().world_transform().inverted() else { return; };
        ...
        if self.base.constrains_horizontal() {
            ...
            scroll.set_scroll_offset_x(math_types::clamp(local_position.x / track_range * max_offset, max_offset, 0.0));
        }
        if self.base.constrains_vertical() {
            ...
            scroll.set_scroll_offset_y(math_types::clamp(local_position.y / track_range * max_offset, max_offset, 0.0));
        }
    }
    ...
    pub fn drag_thumb(&mut self, delta: Vec2D, time_stamp: f32) {
        ...
        if self.base.constrains_horizontal() {
            ...
            scroll.set_scroll_offset_x(math_types::clamp(thumb_offset / track_range * max_offset, max_offset, 0.0));
        }
        if self.base.constrains_vertical() {
            let inner_height = self.track().inner_height();
            ...
            scroll.set_scroll_offset_y(math_types::clamp(thumb_offset / track_range * max_offset, max_offset, 0.0));
        }
        let delta = Vec2D::new(scroll.offset_x() - previous_x, scroll.offset_y() - previous_y);
        if let Some(physics) = scroll.physics_mut() { physics.accumulate(delta, time_stamp); }
    }

    pub fn thumb(&self) -> &LayoutComponent { self.base.parent().as_ref::<LayoutComponent>().unwrap() }
    pub fn thumb_mut(&mut self) -> &mut LayoutComponent { self.base.parent_mut().as_mut::<LayoutComponent>().unwrap() }
    pub fn track(&self) -> &LayoutComponent { self.base.parent().parent().unwrap().as_ref::<LayoutComponent>().unwrap() }
    pub fn validate(&self, context: &CoreContext) -> bool { context.resolve(self.base.scroll_constraint_id()).is_some_and(|object| object.is::<ScrollConstraint>()) }
}
