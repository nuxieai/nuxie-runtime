...
FEATURE_ROWS = {
    ...
    "src/constraints/scrolling/elastic_scroll_physics.cpp": (
        "ported",
        "crates/nuxie-runtime/src/constraints/scrolling/elastic_scroll_physics.rs; crates/nuxie-runtime/src/constraints/scrolling/scroll_physics.rs",
        "Phase 2 direct owner and Phase 3 paired audit: exact helper/variant lifecycle is retained; run uses pinned comparison semantics for NaN and reversed bounds rather than f32::clamp.",
    ),
...
FEATURE_ROWS.update(
    {
        ...
        "src/constraints/scrolling/scroll_bar_constraint.cpp": (
            "ported",
            "crates/nuxie-runtime/src/constraints/scrolling/scroll_bar_constraint.rs; crates/nuxie-runtime/src/constraints.rs; crates/nuxie-runtime/src/components.rs",
            "Phase 2 direct owner and Phase 3 paired audit: constrain, track-hit, and thumb-drag behavior matches the pin, including fminf/fmaxf clamp semantics; safe nullable-owner handling is explicit.",
        ),
        "src/constraints/scrolling/scroll_bar_constraint_proxy.cpp": (
            "ported",
            "crates/nuxie-runtime/src/constraints/scrolling/scroll_bar_constraint_proxy.rs; crates/nuxie-runtime/src/constraints/draggable_constraint.rs",
            "Phase 2 direct owner and Phase 3 paired audit: Thumb/Track lifecycle matches the pin and proxy construction now enforces the pinned LayoutComponent owner checks.",
        ),
        "src/constraints/scrolling/scroll_constraint.cpp": (
            "ported",
            "crates/nuxie-runtime/src/constraints.rs; crates/nuxie-runtime/src/constraints/scrolling/scroll_constraint.rs",
            "Phase 3 paired owner audit covers retained metrics/intents, constraint rendezvous, drag/physics lifecycle, computed velocity/active observables, directional snap, focus scrolling, and generated computed-property setter behavior.",
        ),
        "src/constraints/scrolling/scroll_constraint_proxy.cpp": (
            ...
            "Phase 3 paired audit: viewport proxy threshold, drag, physics, and interactive lifecycle match the pin; noninteractive start preserves retained drag state exactly.",
        ),
        "src/constraints/scrolling/scroll_physics.cpp": (
            "ported",
            "crates/nuxie-runtime/src/constraints/scrolling/scroll_physics.rs",
            "Phase 3 paired audit: base accumulate/reset/prepare/run/stop lifecycle matches the deterministic pinned oracle; supplied host timestamps are the explicit Rust host-clock adaptation.",
        ),
