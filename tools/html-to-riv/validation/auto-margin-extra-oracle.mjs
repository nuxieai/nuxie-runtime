// Supplemental L07 root and realistic composition references.
...
const fixtures = JSON.parse(fs.readFileSync(path.join(here, 'auto-margin-extra-cases.json'), 'utf8'));
const out = path.resolve('output/playwright/html-to-riv/auto-margin-extra-oracle');
...
try {
  ...
  for (const fixture of fixtures) {
    ...
    for (const width of [240, 390, 768]) {
      ...
      if (fixture.name === 'auto-margin-composition-cards') {
        for (const [body, button] of [['bbody', 'baction'], ['pbody', 'paction']]) {
          if (boxes[button].y + boxes[button].height > 320 || boxes[button].y - boxes[body].y - boxes[body].height <= 6) {
            throw new Error(`Card fixture must expose its action and positive auto space at ${width}`);
          }
        }
      }
