...
| C03 `DenseHashMap` pointer-key constructor | No port required. | `DenseHashMap::new(empty_key)` already accepts pointer keys, and its existing `Default`/`DenseDefault` path supplies a null pointer for raw-pointer keys. No Rust API split is needed. |
