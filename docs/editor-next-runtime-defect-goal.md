...
- Open Editor-owned supported-WebGPU stale-golden evidence: `LOC-012` is
  qualified at `stale-oracle`, but changing the expected image requires an
  explicit user decision before closure. The required-WebGPU visual/spacing
  gate passes 2/2 with the reviewed authored image; P19-C08 and COR-07 remain
  live because the historical artifact cannot be regenerated byte-for-byte
  and carries no C++/Rust parity claim.
...
- `F-ED-10`: `LOC-012` has qualified Editor-owned required-WebGPU
  ...
  and COR-07; make no runtime/renderer repair or C++ parity claim.
