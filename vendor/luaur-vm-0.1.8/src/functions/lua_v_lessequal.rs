...
pub unsafe fn lua_v_lessequal(
    ...
    if ttype!(l) != ttype!(r) {
        return luaG_ordererror(L, l, r, TMS::TM_LE);
    } else if ttisnumber!(l) {
        return Ok(luai_numle(nvalue!(l), nvalue!(r)) as i32);
    } else if ttisstring!(l) {
        return Ok((luaV_strcmp(tsvalue!(l), tsvalue!(r)) <= 0) as i32);
    } else if {
...
pub unsafe fn lua_v_lessequal_export(
    L: *mut lua_State,
    l: *const TValue,
    r: *const TValue,
) -> crate::records::lua_exception::LuaResult<i32> {
