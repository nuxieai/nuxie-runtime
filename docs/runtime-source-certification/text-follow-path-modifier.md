...
| 2 | `onAddedClean` (34) | For non-null Shape/Path target, add its follow-path flag, then call `Super::onAddedClean`. Unsupported/null target adds no flag. | `nuxie-runtime/src/artboard.rs:2928::initialize_path_target_flags`, TextFollow branch `:2983-3008`; inherited occurrence target resolution at `:2894::initialize_text_target_modifiers` and state `components.rs:999::RuntimeTextTargetState` | **adapted exact**: occurrence construction publishes the same target flag before normal update traversal. Runtime target cpp-probe covers missing and resolved targets. |
| 3 | `update` (52) | Collect Shape paths in authored order or the single Path; rewind `m_worldPath`; append each raw path transformed by that path's `pathTransform`. Empty/null/unsupported target leaves an empty retained world path. | `text/text_follow_path_modifier.rs:184::update_text_follow_path_world_path`; `artboard.rs:9402-9415`; occurrence state `components.rs:994::RuntimeTextFollowPathState`; inherited target state `components.rs:999::RuntimeTextTargetState` | **corrected exact**: world commands are retained on the modifier occurrence at its component update boundary and consume the inherited occurrence's frozen target rather than re-reading an authored descriptor. Rust path geometry applies the retained path transform before ordered append. |
...
fresh empty state (`components.rs:1029::clone_for_occurrence`). Its inherited
`TextTargetModifier::m_Target` is separately occurrence-owned at
`components.rs:999::RuntimeTextTargetState` and is re-resolved from the copied
generated ID during clone construction. `m_worldPath`
...
- `text.rs:7479::cxx_text_follow_path_retains_local_measure_when_text_inverse_fails`
  ...
  preservation.
- `text.rs:7525::cxx_text_follow_path_callbacks_publish_path_only` drives all
  six live generated property write routes and observes exact Text Path dirt.
- `text.rs:7564::cxx_text_follow_path_numeric_order_matches_min_max_clamp_and_nested_fmod`
