# Data Binding Graph Formula Random Remaining Fallbacks Target-To-Source Call Count Runtime Contract

## Purpose

Pin Rust's host-supplied formula random-stream accounting for enum, color,
string, and trigger target-to-source fallback sources.

For these non-number sources flowing through a direct `DataConverterFormula`
random token into number targets, C++ keeps the source unchanged and reports
...
- Boolean target-to-source call counts, covered by
...
- Setting a host random stream resets the remaining fallback call count to
  zero.
...
- Enum, color, string, and trigger sources all preserve those counts for
  random modes `0`, `1`, and `2`.
- The C++ probe's `randomTotalCalls` reports match the expected C++ remaining
  fallback reverse-path semantics and Rust's host-stream call count for every
  enum, color, string, and trigger target-to-source report.
