...
| 14 | `computeBoundsInfo` (304) | Accumulate paragraph lines, widths, baseline origin, paragraph spacing, ellipsis line/height, and non-fixed trim in source order. | `text.rs:4375::static_layout_info`; `3100::static_line_metrics`; `3189::static_text_total_height`; `text.rs:5539::cxx_paragraph_spacing_order_covers_empty_paragraphs_trim_measure_fit_and_render` | **adapted correction candidate**: the flattened line representation now adds spacing after each paragraph, retains final spacing for fixed-height/ellipsis calculations, and removes exactly one final spacing before auto-bounds trim. The shaped-line representation remains adapted. |
...
| 31 | `paragraphSpacingChanged` (1017; disabled 1433) | Paint dirty. | `text/text.rs:115::double_property_changed`; consumption `text.rs:3076::paragraph_spacing`, `3100::static_line_metrics`, `3189::static_text_total_height` | **exact enabled-path correction candidate**: Paint publication is unchanged and the rebuilt layout now consumes spacing in pinned bounds/fit/measure/render order. The disabled-feature counterpart remains absent. |
...
| 37 | static `BreakLines` (1085) | Break each paragraph; no-wrap/auto width use -1; auto-width computes global max; then compute spacing/alignment for all paragraphs. | `text.rs:4227::StaticTextSlice::layout_static_text_lines`; `text/line_breaker.rs:1::split_static_text_lines`; `52::static_text_line_iteration`; downstream metrics `text.rs:3100::static_line_metrics` | **adapted**: custom HarfRust/renderer annotation breaker replaces `GlyphLine::BreakLines`; paragraph boundaries now drive the downstream spacing positions, but the shaped line-break representation remains nonliteral. |
