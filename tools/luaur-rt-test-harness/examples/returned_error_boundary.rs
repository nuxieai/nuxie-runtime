//! Qualifies typed protected-call cleanup. Run with CARGO_PROFILE_DEV_PANIC=abort.
    ...
    unsafe {
        ...
        for _ in 0..3 {
            ...
            let status = luaD_pcall(state, Some(fail), core::ptr::null_mut(), top, 0)
                .expect("protected failure returns its status");
            ...
            assert_eq!(
                luaD_pcall(state, Some(succeed), core::ptr::null_mut(), top, 0)
                    .expect("protected success returns its status"),
                0
            );
            assert_eq!(
                std::ffi::CStr::from_ptr(
