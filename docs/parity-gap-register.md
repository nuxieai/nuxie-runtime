...
| V11 | **CLOSED 2026-08-03 — `global_variables_test` is exact at t=0/0.5/1, including the side channel.** Name-based comparator sources now resolve through the retained file resolver before the bounded update/data-bind probe; Rust reports the same two t=0 layer changes as C++. | fresh scripted golden capture; `global_variables_mid_frame_view_model_comparator_matches_cpp_layer_count`; `transition_viewmodel_condition.cpp:49-60`; `state_machine_instance.cpp:2665-2697` | **Green:** corpus row restored to `exact`; obsolete draw/side-channel divergence tags removed. |
| V12 | **`db_health_tracker`: post-zero Rust advance does not complete.** V2 densification to the authored 1.0s boundary found that the scripted Rust runner consumes a full CPU and does not return within 180 seconds for `--samples 0,0.5,1`; the scripted C++ runner returns a 2.3 MB stream. The former t=0-only row was exact. The samples remain enrolled and the row is parked `not-yet` with `post-zero-runtime-hang:V12`, so the gate cannot mistake the missing comparison for exactness. | `corpus.toml` `db_health_tracker`; repro: `gtimeout 180 rust-golden-runner-scripted --file .../db_health_tracker.riv --execute-scripts --samples 0,0.5,1 --side-channel` exits 124 after 180s | Localize the non-terminating post-zero frame path, make the three-sample Rust runner complete, compare its stream, then remove the V12 tag and restore `exact`. |
...
| V22 | **CLOSED 2026-08-03 — stale densification runner/capture artifact.** A fresh pinned side-channel capture has the same 147-line structure at t=0/0.5/1 and a maximum numeric delta of `3.9e-6`, well inside the `1.3e-4` contract; the recorded 245×250-vs-490×362.5 layout mismatch is absent. | fresh ordinary golden capture; `corpus.toml` milestone V22, samples 0/0.5/1 | **Green:** row reclassified to `exact`; retain the dense samples so stale-runner regressions remain visible. |
...
| V25 | **CLOSED 2026-08-03 — `group_effect` is exact at t=0/0.5/1.** A true scripted path-effect advance now publishes `ScriptUpdate` at the effect's dependency slot, invalidating its retained effect suffix before the parent paint rebuilds the authored group/target chain. | fresh scripted golden capture; `true_scripted_path_effect_advance_schedules_effect_invalidation`; `scripted_path_effect.cpp:111-132,199-207`; `shape_paint.cpp:115-152` | **Green:** corpus row restored to `exact`. |
...
| V27 | **CLOSED 2026-08-03 — stale densification runner/capture artifact.** Fresh pinned ordinary runners produce byte-identical 417,801-byte streams at t=0/0.5/1, including every image vertex/index-buffer command; the recorded buffer-command phase mismatch is absent. | fresh ordinary golden capture; `corpus.toml` milestone V27, samples 0/0.5/1 | **Green:** row reclassified to `exact`; retain the dense samples and image-buffer commands in the contract. |
...
| V30 | **CLOSED 2026-08-03 — `path_effect_with_feathers` is exact at t=0/0.5/1.** The V25 scripted-effect invalidation reaches `ShapePaint` before feather preparation, so the inner path and clip are rebuilt from the current effect path rather than the stale source geometry. | fresh scripted golden capture; `true_scripted_path_effect_advance_schedules_effect_invalidation`; `scripted_path_effect.cpp:111-132,199-207`; `shape_paint.cpp:115-152` | **Green:** corpus row restored to `exact`; retain the dense feather samples. |
...
| V35 | **CLOSED 2026-08-03 — stale runner/harness capture artifact.** Fresh rebuilt pinned runners agree at t=0/0.5/1 with the side channel enabled; the recorded 100-vs-75 radius mismatch is absent, and the prior C++ diagnostic-path crash does not reproduce with the provenance-bound runner. | fresh ordinary golden capture; `corpus.toml` milestone V35, samples 0/0.5/1 | **Green:** row reclassified to `exact` with full side-channel coverage; retain the dense source-switch samples. |
| V36 | **CLOSED 2026-08-03 — stale densification runner/capture artifact.** A fresh pinned side-channel capture has matching command structure at t=0/0.5/1 and a maximum numeric delta of `3.0e-5`; the recorded empty-path-vs-compound-path mismatch is absent. | fresh ordinary golden capture; `corpus.toml` milestone V36, samples 0/0.5/1 | **Green:** row reclassified to `exact`; retain the dense path samples so stale-runner regressions remain visible. |
...
| F6 | **Semantics/accessibility** — `semantic_manager` 1,109, `semantic_data` 572, provider, inference registry | 1,926 | PARTIAL (SEMRES) | The retained runtime and LT-1 full diff/action/focus side channel are implemented against `4ac7b327`. Nested focus and Simpsons are exact, including Selected propagation, root/pre-layout bounds, and update timing. data_binding_lists now matches Selected changes, removals, hierarchy patches, and every post-action frame; only its four initial mounted Text glyph bounds differ in the sampled corpus. The exact Simpsons differential promotes the inference-registry row; data, manager, and provider remain pending on that Text-bound remainder plus owner-pushed generic WorldTransform/Path semantic dirt (see `SEMRES-report.md`). |
| F7 | **Unported Lua bindings** — `lua_gpu` 3,734, `lua_promise` 1,323, `lua_scripted_context` 583, `lua_buffer_ext` 538, `lua_audio` 507, `lua_data_value` 503, `lua_image_decode` 467, + mesh/color/image/blob/state/data_context/gradient/input | ~9,800 | PARTIAL (by design) | The GPU-prefixed `lua_gpu.cpp` surface is implemented as a pending wgpu-adapted candidate; that mixed file's Canvas 2D and `Image:view` members remain residue. Other named Lua families retain their own correspondence status and corpus gates. |
