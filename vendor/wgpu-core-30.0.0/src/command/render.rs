...
use core::{convert::Infallible, fmt, hash::Hash, num::NonZeroU32, ops::Range, str};
...
/// A set that keeps ordinary render-pass attachment validation on the stack,
/// while retaining hash-set behavior for views that address many array layers.
enum InlineHashSet<T, const N: usize> {
    Inline(ArrayVec<T, N>),
    Overflow(crate::FastHashSet<T>),
}

impl<T, const N: usize> Default for InlineHashSet<T, N> {
    fn default() -> Self {
        Self::Inline(ArrayVec::new())
    }
}

impl<T: Eq + Hash, const N: usize> InlineHashSet<T, N> {
    /// Returns `true` if `value` was not already present.
    fn insert(&mut self, value: T) -> bool {
        match self {
            Self::Overflow(overflow) => overflow.insert(value),
            Self::Inline(inline) => {
                ...
                let mut overflow = crate::FastHashSet::with_capacity_and_hasher(
                    inline.len() + 1,
                    Default::default(),
                );
                overflow.extend(inline.drain(..));
                let inserted = overflow.insert(value);
                debug_assert!(inserted, "inline entries and the new value are unique");
...
type AttachmentSubresource = (crate::track::TrackerIndex, u32, u32);

// One slice for each possible color attachment and resolve target stays inline.
// D2-array attachments can exceed this count and promote to the hash set above.
const INLINE_ATTACHMENT_SUBRESOURCES: usize = hal::MAX_COLOR_ATTACHMENTS * 2;
type AttachmentSubresourceSet =
    InlineHashSet<AttachmentSubresource, INLINE_ATTACHMENT_SUBRESOURCES>;

fn validate_color_attachment_bytes_per_sample_once(
    validated: &mut bool,
    ...
    validation::validate_color_attachment_bytes_per_sample(attachment_formats, limit)?;
    *validated = true;
    Ok(())
}
...
impl RenderPassInfo {
    ...
    fn start(
        ...
        let mut attachment_set = AttachmentSubresourceSet::default();
        let mut color_attachment_bytes_per_sample_validated = false;
        ...
        for (index, attachment) in color_attachments.iter().enumerate() {
            ...
            // Keep this at the first active color attachment to preserve the
            // existing validation-error order, but do not rescan all formats
            // for every later attachment.
            validate_color_attachment_bytes_per_sample_once(
                &mut color_attachment_bytes_per_sample_validated,
                color_attachments
                    .iter()
                    .flatten()
            ...
            fn check_attachment_overlap(
                attachment_set: &mut AttachmentSubresourceSet,
                view: &TextureView,
...
/// Returns whether this render pass can be appended to an already-open native
/// command buffer without changing the order of any work.
///
/// wgpu normally records a pass body first, discovers its resource transitions
/// and other pre-pass work while validating that body, then reorders two native
/// command buffers. The HAL capability explicitly identifies the narrow case
/// where transitions need no native work and command-buffer continuation does
/// not bypass a backend prologue. The other kinds of pre-pass work are real and
/// must remain before the body. This predicate is intentionally sufficient
/// rather than necessary: rejecting a pass only loses coalescing, while
/// accepting a pass with real pre-pass work would be a correctness bug.
fn can_append_render_pass_to_open_command_buffer(
    hal_can_coalesce_render_pass_command_buffers: bool,
    has_pending_discards: bool,
    has_timestamp_writes: bool,
    has_occlusion_query_set: bool,
    indirect_validation_enabled: bool,
    commands: &[ArcRenderCommand],
) -> bool {
    // Keep the command scan entirely out of the common path for every backend
    // whose HAL encoder retains the default-false capability.
    if !hal_can_coalesce_render_pass_command_buffers {
        return false;
    }

    let mut has_query_command = false;
    let mut may_inject_indirect_validation = false;
    for command in commands {
        match command {
            ...
            | ArcRenderCommand::EndPipelineStatisticsQuery => {
                has_query_command = true;
            }
            ArcRenderCommand::DrawIndirect { .. }
            | ArcRenderCommand::MultiDrawIndirectCount { .. }
            // A render bundle may itself contain indirect draws. Conservatively
            // reject all bundles while indirect validation is enabled instead of
            // coupling this classifier to the bundle's validated command stream.
            | ArcRenderCommand::ExecuteBundle(_) => {
                may_inject_indirect_validation = indirect_validation_enabled;
            }
    ...
    pre_pass_work_allows_appending(
        has_pending_discards,
        has_timestamp_writes,
        has_occlusion_query_set,
        has_query_command,
        may_inject_indirect_validation,
    )
}

fn pre_pass_work_allows_appending(
    has_pending_discards: bool,
    has_timestamp_writes: bool,
    has_occlusion_query_set: bool,
    has_query_command: bool,
    may_inject_indirect_validation: bool,
) -> bool {
    !has_pending_discards
        && !has_timestamp_writes
        && !has_occlusion_query_set
...
fn assert_append_pre_pass_invariants(
    append_to_open_command_buffer: bool,
    has_discard_init_fixups: bool,
    has_query_resets: bool,
    has_indirect_validation_batches: bool,
) {
    if !append_to_open_command_buffer {
        return;
    }

    // These are safety postconditions for the sufficient classifier above,
    // not debugging aids. Keep them enabled in optimized builds: if future
    // validation starts injecting any real pre-pass work into an accepted
    // command stream, abort before recording it after the pass body.
    assert!(
        !has_discard_init_fixups,
        "render-pass command-buffer continuation produced discard init fixups"
    );
    assert!(
        !has_query_resets,
        "render-pass command-buffer continuation produced query resets"
    );
    assert!(
        !has_indirect_validation_batches,
        "render-pass command-buffer continuation produced indirect validation work"
    );
}
...
pub(super) fn encode_render_pass(
    ...
    let hal_can_coalesce_render_pass_command_buffers = parent_state
        .raw_encoder
        .can_coalesce_render_pass_command_buffers();
    let append_to_open_command_buffer = can_append_render_pass_to_open_command_buffer(
        hal_can_coalesce_render_pass_command_buffers,
        parent_state.texture_memory_actions.has_pending_discards(),
        timestamp_writes.is_some(),
        occlusion_query_set.is_some(),
    ...
    let raw_encoder = if append_to_open_command_buffer {
        parent_state.raw_encoder.open().map_pass_err(pass_scope)?
    } else {
        parent_state
            .raw_encoder
            .close_if_open()
    ...
    {
        let transit = if append_to_open_command_buffer {
            encoder.open().map_pass_err(pass_scope)?
        } else {
            encoder
                .open_pass(hal_label(
        ...
        assert_append_pre_pass_invariants(
            append_to_open_command_buffer,
            !pending_discard_init_fixups.is_empty(),
            !pending_query_resets.is_empty(),
            !indirect_draw_validation_batcher.is_empty(),
        );
...
#[cfg(test)]
mod render_pass_validation_tests {
    use super::*;

    #[test]
    fn attachment_subresource_set_rejects_duplicates_inline() {
        let mut set = InlineHashSet::<u32, 2>::default();

        assert!(set.insert(10));
        assert!(!set.insert(10));
        assert!(set.insert(20));
        assert!(!set.insert(20));
        assert!(matches!(set, InlineHashSet::Inline(_)));
    }

    #[test]
    fn attachment_subresource_set_preserves_membership_after_promotion() {
        let mut set = InlineHashSet::<u32, 2>::default();

        assert!(set.insert(10));
        assert!(set.insert(20));
        assert!(set.insert(30));
        assert!(matches!(set, InlineHashSet::Overflow(_)));
    ...
    #[test]
    fn color_attachment_bytes_per_sample_validation_is_cached_after_success() {
        ...
        // This would exceed the limit if validation ran a second time.
        validate_color_attachment_bytes_per_sample_once(
            &mut validated,
    ...
    #[test]
    fn failed_color_attachment_bytes_per_sample_validation_is_not_cached() {
        let mut validated = false;

        assert!(validate_color_attachment_bytes_per_sample_once(
            &mut validated,
            [wgt::TextureFormat::Rgba32Float],
            0,
        )
...
#[cfg(test)]
mod render_pass_command_buffer_coalescing_tests {
    use super::*;

    #[test]
    fn only_pre_pass_free_scopes_can_append() {
        for work_bits in 0_u8..32 {
            let actual = pre_pass_work_allows_appending(
                work_bits & 1 != 0,
                work_bits & 2 != 0,
                work_bits & 4 != 0,
                work_bits & 8 != 0,
            ...
            assert_eq!(actual, work_bits == 0, "work_bits={work_bits:05b}");
        }
    }

    #[test]
    fn direct_draw_and_debug_commands_remain_appendable() {
        let commands = [
            ArcRenderCommand::PushDebugGroup { color: 0, len: 0 },
            ArcRenderCommand::PopDebugGroup,
            ArcRenderCommand::InsertDebugMarker { color: 0, len: 0 },
            ArcRenderCommand::Draw {
                vertex_count: 3,
                instance_count: 1,
                first_vertex: 0,
        ...
        assert!(can_append_render_pass_to_open_command_buffer(
            true, false, false, false, true, &commands,
        ));
    }

    #[test]
    fn query_commands_force_the_ordered_pre_pass_path() {
        for command in [
            ArcRenderCommand::BeginOcclusionQuery { query_index: 0 },
            ArcRenderCommand::EndOcclusionQuery,
            ArcRenderCommand::EndPipelineStatisticsQuery,
        ] {
            assert!(!can_append_render_pass_to_open_command_buffer(
    ...
    #[test]
    fn indirect_validation_toggle_does_not_reject_direct_draws() {
        let commands = [ArcRenderCommand::Draw {
            vertex_count: 3,
            instance_count: 1,
            first_vertex: 0,
            first_instance: 0,
        ...
        for indirect_validation_enabled in [false, true] {
            assert!(can_append_render_pass_to_open_command_buffer(
                true,
                false,
                false,
                false,
                indirect_validation_enabled,
                &commands,
            ));
        }
    }
    ...
    #[test]
    fn default_false_hal_capability_rejects_before_command_classification() {
        let commands = [ArcRenderCommand::Draw {
            vertex_count: 3,
            instance_count: 1,
            first_vertex: 0,
    ...
    #[test]
    fn release_invariants_allow_only_empty_pre_pass_work_for_continuation() {
        assert_append_pre_pass_invariants(true, false, false, false);
        assert_append_pre_pass_invariants(false, true, true, true);

        for work_bits in 1_u8..8 {
            ...
            assert!(result.is_err(), "work_bits={work_bits:03b}");
        }
    }
}
